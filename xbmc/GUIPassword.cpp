...
bool CGUIPassword::CheckStartUpLock()
{
  ...
  if (iMasterLockRetriesLeft == -1)
    iMasterLockRetriesLeft = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
        CSettings::SETTING_MASTERLOCK_MAXRETRIES);
  ...
  else
  {
    ...
    {
      iVerifyPasswordResult =
          VerifyPassword(profileManager->GetMasterProfile().getLockMode(), strPassword, strHeader);
      if (iVerifyPasswordResult != 0)
      {
        ...
        HELPERS::ShowOKDialogLines(CVariant{12360}, CVariant{12367}, CVariant{strLabel},
                                   CVariant{""});
      }
      else
...
bool CGUIPassword::IsMasterLockUnlocked(bool bPromptUser, bool& bCanceled)
{
  ...
  if ((LOCK_MODE_EVERYONE < profileManager->GetMasterProfile().getLockMode() && !bMasterUser) &&
      !bPromptUser)
  {
  ...
  int iVerifyPasswordResult =
      VerifyPassword(profileManager->GetMasterProfile().getLockMode(), strPassword, strHeading);
...
void CGUIPassword::UpdateMasterLockRetryCount(bool bResetCount)
{
  ...
  if (!bResetCount)
  {
    ...
    HELPERS::ShowOKDialogLines(CVariant{20075}, CVariant{12345}, CVariant{std::move(dlgLine1)},
                               CVariant{0});
  }
  else
    g_passwordManager
        .iMasterLockRetriesLeft = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
        CSettings::
            SETTING_MASTERLOCK_MAXRETRIES); // user entered correct mastercode, reset retries to max allowed
}
  ...
  for (const char* const strType : strTypes)
  {
    ...
    for (IVECSOURCES it = shares->begin(); it != shares->end(); ++it)
      if (it->m_iLockMode != LOCK_MODE_EVERYONE) // remove old info
      {
        ...
        CMediaSourceSettings::GetInstance().UpdateSource(strType, it->strName, "lockmode",
                                                         "0"); // removes locks from xml
      }
  }
  CMediaSourceSettings::GetInstance().Save();
  CGUIMessage msg(GUI_MSG_NOTIFY_ALL, 0, 0, GUI_MSG_UPDATE_SOURCES);
...
int CGUIPassword::VerifyPassword(LockType btnType,
                                 const std::string& strPassword,
