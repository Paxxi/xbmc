...
int MysqlDatabase::connect(bool create_new)
{
  ...
  try
  {
                           ...
                           atoi(port.c_str()), NULL, compression ? CLIENT_COMPRESS : 0) != NULL)
    {
      ...
      {
        CLog::Log(LOGERROR, "Unable to set utf8 charset: %s [%d](%s)", db.c_str(),
    ...
    CLog::Log(LOGERROR, "Unable to open database: %s [%d](%s)", db.c_str(), mysql_errno(conn),
              mysql_error(conn));
  ...
  {
    CLog::Log(LOGERROR, "Unable to open database: %s (%u)", db.c_str(), GetLastError());
  }
  return DB_CONNECTION_NONE;
}
...
int MysqlDatabase::copy(const char* backup_name)
{
  ...
  if (res)
  {
    ...
    while ((row = mysql_fetch_row(res)) != NULL)
    {
      ...
      sprintf(sql, "CREATE TABLE `%s`.%s LIKE %s", backup_name, row[0], row[0]);
      ...
      // copy the table data
      sprintf(sql, "INSERT INTO `%s`.%s SELECT * FROM %s", backup_name, row[0], row[0]);
...
int MysqlDatabase::query_with_reconnect(const char* query)
{
  ...
  while (((result = mysql_real_query(conn, query, strlen(query))) != MYSQL_OK) &&
         ...
         (attempts-- > 0))
  {
    CLog::Log(LOGINFO, "MYSQL server has gone. Will try %d more attempt(s) to reconnect.",
              attempts);
    active = false;
    connect(true);
  }
...
bool MysqlDatabase::mysqlStrAccumAppend(StrAccum* p, const char* z, int N)
{
  ...
  {
    CLog::Log(LOGDEBUG,
              "This query part contains a like, we will double backslash in the next field: %s",
              testString.c_str());
    isLike = true;
  }
...
MysqlDataset::MysqlDataset(MysqlDatabase* newDb)
  : Dataset(newDb)
{
  haveError = false;
  db = newDb;
