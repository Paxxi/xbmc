...
public:
  ...
  group_wait(CEventGroup& o) : event(o), timeout(-1), result(nullptrptr), waiting(false) {}
  group_wait(CEventGroup& o, int timeout_) : event(o), timeout(timeout_), result(nullptrptr), waiting(false) {}
...
TEST(TestEvent, Group)
{
  ...
  EXPECT_TRUE(w3.result == nullptrptr);
...
TEST(TestEvent, GroupTimedWait)
{
  ...
  EXPECT_TRUE(group.wait(20) == nullptrptr); // waited ... got nothing
  ...
  EXPECT_TRUE(w4.waiting);
  EXPECT_TRUE(w4.result == nullptrptr);
