...
{
  XbmcThreads::ThreadSleep(millis);
}

template<class E>
inline static bool waitForWaiters(E& event, int numWaiters, int milliseconds)
{
...
public:
  inline AtomicGuard(std::atomic<long>* val_)
    : val(val_)
  {
    if (val)
      ++(*val);
...
public:
  inline explicit thread(IRunnable& runnable)
    : f(&runnable)
    , cthread(new CThread(f, "DumbThread"))
  {
    cthread->Create();
  }
  ...
  ~thread() { delete cthread; }

  inline thread(thread& other)
    : f(other.f)
    , cthread(other.cthread)
  {
    other.f = NULL;
    other.cthread = NULL;
  }
  ...
  void join() { cthread->Join(static_cast<unsigned int>(-1)); }

  bool timed_join(unsigned int millis) { return cthread->Join(millis); }
};
