  ...
  while (iter != g_callQueue.end())
  {
    ...
    if (p->handler->isStateOk(p->cb->getObject()))
    {
      ...
      //  longer are accessing the g_callQueue so it's fine to do this now
      {
        ...
        CLog::Log(LOGDEBUG, "%sNEWADDON executing callback 0x%lx", _tg.getSpaces(),
                  (long)(p->cb.get()));
#endif
        ...
        if (!p->cb->getObject()->isDeallocating())
        {
          ...
          catch (XbmcCommons::Exception& e)
          {
            e.LogThrowMessage();
          }
          catch (...)
          {
            CLog::Log(LOGERROR, "Unknown exception while executing callback 0x%lx",
                      (long)(p->cb.get()));
          }
        }
      }
...
void RetardedAsyncCallbackHandler::clearPendingCalls(void* userData)
{
  ...
  while (iter != g_callQueue.end())
  {
    ...
    {
#ifdef ENABLE_XBMC_TRACE_API
      CLog::Log(LOGDEBUG, "%sNEWADDON removing callback 0x%lx for PyThreadState 0x%lx from queue",
