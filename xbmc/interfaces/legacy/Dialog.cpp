...
std::unique_ptr<std::vector<int>> Dialog::multiselect(
                ...
                const String& line3)
{
  ...
  if (line2.empty() && line3.empty())
    ...
    return HELPERS::ShowOKDialogLines(CVariant{heading}, CVariant{line1}, CVariant{line2},
                                      CVariant{line3});
}
    ...
    const String& heading, const String& defaultt, int type, int option, int autoclose)
{
  ...
  case INPUT_ALPHANUM:
  {
    ...
    if (!CGUIKeyboardFactory::ShowAndGetInput(value, CVariant{heading}, true, bHiddenInput,
                                              autoclose))
      value = emptyString;
  }
  break;
