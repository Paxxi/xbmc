...
public:
  WindowXMLInterceptor(WindowXML* _window, int windowid, const char* xmlfile)
    : InterceptorDialog<CGUIMediaWindow>("CGUIMediaWindow", _window, windowid, xmlfile)
    , xwin(_window)
  {
  }
  ...
  void AllocResources(bool forceLoad = false) override
  {
    ...
    if (up())
      ...
      checkedv(AllocResources(forceLoad));
  }
  void FreeResources(bool forceUnLoad = false) override
  {
    ...
    if (up())
      ...
      checkedv(FreeResources(forceUnLoad));
  }
  bool OnClick(int iItem, const std::string& player = "") override
  {
    XBMC_TRACE;
    return up() ? CGUIMediaWindow::OnClick(iItem, player) : checkedb(OnClick(iItem));
  }
  ...
  void Process(unsigned int currentTime, CDirtyRegionList& dirtyregions) override
  {
    XBMC_TRACE;
    if (up())
      CGUIMediaWindow::Process(currentTime, dirtyregions);
    else
      checkedv(Process(currentTime, dirtyregions));
  }
  ...
  {
    XBMC_TRACE;
    return up() ? CGUIWindow::OnAction(action) : checkedb(OnAction(action));
  }
...
protected:
  ...
  {
    XBMC_TRACE;
    return up() ? CGUIMediaWindow::LoadXML(strPath, strPathLower)
                : xwin->LoadXML(strPath, strPathLower);
  }
  ...
  void GetContextButtons(int itemNumber, CContextButtons& buttons) override
  {
    ...
    if (up())
      ...
      xwin->GetContextButtons(itemNumber, buttons);
  }
  bool Update(const std::string& strPath, bool) override
  {
    XBMC_TRACE;
    return up() ? CGUIMediaWindow::Update(strPath) : xwin->Update(strPath);
  }
  ...
  : Window(true)
{
  ...
  if (!XFILE::CFile::Exists(strSkinPath))
  {
    ...
    if (XFILE::CFile::Exists(basePath))
    {
      ...
      std::shared_ptr<ADDON::CSkinInfo> skinInfo =
  ...
  interceptor =
      new WindowXMLInterceptor(this, lockingGetNextAvailableWindowId(), strSkinPath.c_str());
...
bool WindowXML::OnAction(const CAction& action)
{
  ...
  invokeCallback(new CallbackFunction<WindowXML, AddonClass::Ref<Action>>(
      this, &WindowXML::onAction, inf.get()));
  PulseActionEvent();
  return ret;
}
  ...
  case GUI_MSG_CLICKED:
  {
    ...
    if (iControl && iControl != interceptor->GetID()) // pCallbackWindow &&  != this->GetID())
    {
          ...
          //   and we try to do GetControlType on it. So check to make sure it exists
      {
        ...
        {
          invokeCallback(
              new CallbackFunction<WindowXML, int>(this, &WindowXML::onDoubleClick, iControl));
          PulseActionEvent();
          return true;
        }
...
WindowXMLDialog::WindowXMLDialog(const String& xmlFilename,
                                 const String& scriptPath,
                                 const String& defaultSkin,
                                 const String& defaultRes)
