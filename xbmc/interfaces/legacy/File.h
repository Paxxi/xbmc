...
public:
  inline File(const String& filepath, const char* mode = NULL)
  ...
  inline String read(unsigned long numBytes = 0)
  {
    ...
    return b.getString(numBytes == 0 ? b.remaining()
                                     : std::min((unsigned long)b.remaining(), numBytes));
  }
#endif
  ...
  seek(...);
#else
  inline long long seek(long long seekBytes, int iWhence = SEEK_SET)
  {
    DelayedCallGuard dg(languageHook);
