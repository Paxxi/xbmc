...
// ============================================================
// ============================================================
ControlFadeLabel::ControlFadeLabel(long x,
                                   long y,
                                   long width,
                                   long height,
                                   const char* font,
...
CGUIControl* ControlFadeLabel::Create()
{
  ...
  label.align = align;
  pGUIControl =
      new CGUIFadeLabelControl(iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth,
                               (float)dwHeight, label, true, 0, true, false);
...
ControlTextBox::ControlTextBox(
    long x, long y, long width, long height, const char* font, const char* _textColor)
  : strFont("font13")
  , textColor(0xffffffff)
{
  dwPosX = x;
...
CGUIControl* ControlTextBox::Create()
{
  ...
  pGUIControl = new CGUITextBox(iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth,
                                (float)dwHeight, label);
  ...
  , focusedColor(0xffffffff)
{
  ...
  strTextureFocus =
      focusTexture ? focusTexture : XBMCAddonUtils::getDefaultImage("button", "texturefocus");
  strTextureNoFocus =
      noFocusTexture ? noFocusTexture : XBMCAddonUtils::getDefaultImage("button", "texturenofocus");
                             ...
                             const String& label2)
{
  ...
  if (pGUIControl)
  {
    ...
    static_cast<CGUIButtonControl*>(pGUIControl)
        ->PythonSetLabel(strFont, strText, textColor, shadowColor, focusedColor);
...
CGUIControl* ControlButton::Create()
{
  ...
  pGUIControl = new CGUIButtonControl(
      ...
      CTextureInfo(strTextureFocus), CTextureInfo(strTextureNoFocus), label);

  CGUIButtonControl* pGuiButtonControl = static_cast<CGUIButtonControl*>(pGUIControl);
...
// ============================================================
// ============================================================
ControlImage::ControlImage(long x,
                           long y,
                           long width,
                           long height,
                           const char* filename,
...
{
  if (cColorDiffuse)
    sscanf(cColorDiffuse, "%x", &colorDiffuse);
  else
    colorDiffuse = 0;
  ...
  if (pGUIControl)
    static_cast<CGUIImage*>(pGUIControl)
        ->SetColorDiffuse(GUILIB::GUIINFO::CGUIInfoColor(colorDiffuse));
}

CGUIControl* ControlImage::Create()
{
  pGUIControl = new CGUIImage(iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth,
                              (float)dwHeight, CTextureInfo(strFileName));
...
// ============================================================
// ============================================================
ControlProgress::ControlProgress(long x,
                                 long y,
                                 long width,
                                 long height,
                                 ...
                                 const char* textureoverlay)
{
  ...
  // if texture is supplied use it, else get default ones
  strTextureBg = texturebg ? texturebg : XBMCAddonUtils::getDefaultImage("progress", "texturebg");
  strTextureLeft =
      textureleft ? textureleft : XBMCAddonUtils::getDefaultImage("progress", "lefttexture");
  strTextureMid =
      texturemid ? texturemid : XBMCAddonUtils::getDefaultImage("progress", "midtexture");
  strTextureRight =
      textureright ? textureright : XBMCAddonUtils::getDefaultImage("progress", "righttexture");
  strTextureOverlay = textureoverlay
                          ? textureoverlay
...
CGUIControl* ControlProgress::Create()
{
  pGUIControl = new CGUIProgressControl(
      iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth, (float)dwHeight,
      CTextureInfo(strTextureBg), CTextureInfo(strTextureLeft), CTextureInfo(strTextureMid),
      CTextureInfo(strTextureRight), CTextureInfo(strTextureOverlay));

  if (pGUIControl && colorDiffuse)
    static_cast<CGUIProgressControl*>(pGUIControl)
        ->SetColorDiffuse(GUILIB::GUIINFO::CGUIInfoColor(colorDiffuse));
                             ...
                             int orientation)
{
  ...
  // if texture is supplied use it, else get default ones
  strTextureBack =
      textureback ? textureback : XBMCAddonUtils::getDefaultImage("slider", "texturesliderbar");
  strTexture = texture ? texture : XBMCAddonUtils::getDefaultImage("slider", "textureslidernib");
...
{
  pGUIControl =
      new CGUISliderControl(iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth,
                            (float)dwHeight, CTextureInfo(strTextureBack), CTextureInfo(strTexture),
...
{
  pGUIControl = new CGUIControlGroup(iParentId, iControlId, (float)dwPosX, (float)dwPosY,
...
// ============================================================
// ============================================================
ControlRadioButton::ControlRadioButton(long x,
                                       long y,
                                       long width,
                                       long height,
                                       const String& label,
                                       const char* focusOnTexture,
                                       const char* noFocusOnTexture,
                                       const char* focusOffTexture,
                                       const char* noFocusOffTexture,
                                       const char* focusTexture,
                                       const char* noFocusTexture,
                                       long _textOffsetX,
                                       long _textOffsetY,
                                       ...
                                       const char* _shadowColor,
                                       const char* _focusedColor,
                                       const char* disabledOnTexture,
                                       const char* disabledOffTexture)
  : strFont("font13")
  , textColor(0xffffffff)
  , disabledColor(0x60ffffff)
  , textOffsetX(_textOffsetX)
  ...
  , focusedColor(0xffffffff)
{
  ...
  {
    strTextureRadioOnFocus = strTextureRadioOnNoFocus =
  ...
  {
    strTextureRadioOffFocus = strTextureRadioOffNoFocus =
...
void ControlRadioButton::setRadioDimension(long x, long y, long width, long height)
{
  ...
  if (pGUIControl)
  {
    XBMCAddonUtils::GuiLock lock(languageHook, false);
    static_cast<CGUIRadioButtonControl*>(pGUIControl)
        ->SetRadioDimensions((float)dwPosX, (float)dwPosY, (float)dwWidth, (float)dwHeight);
  }
}
...
CGUIControl* ControlRadioButton::Create()
{
  ...
  pGUIControl = new CGUIRadioButtonControl(
      iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth, (float)dwHeight,
      CTextureInfo(strTextureFocus), CTextureInfo(strTextureNoFocus), label,
      CTextureInfo(strTextureRadioOnFocus), CTextureInfo(strTextureRadioOnNoFocus),
      CTextureInfo(strTextureRadioOffFocus), CTextureInfo(strTextureRadioOffNoFocus),
      CTextureInfo(strTextureRadioOnDisabled), CTextureInfo(strTextureRadioOffDisabled));

  CGUIRadioButtonControl* pGuiButtonControl = static_cast<CGUIRadioButtonControl*>(pGUIControl);
...
void Control::setAnimations(const std::vector<Tuple<String, String>>& eventAttr)
{
  ...
  const CRect animRect((float)dwPosX, (float)dwPosY, (float)dwPosX + dwWidth,
                       (float)dwPosY + dwHeight);
  XBMCAddonUtils::GuiLock lock(languageHook, false);
  if (pGUIControl)
  {
...
void Control::setNavigation(const Control* up,
                            const Control* down,
                            const Control* left,
                            const Control* right)
{
...
void ControlSpin::setTextures(const char* up,
                              const char* down,
                              const char* upFocus,
...
CGUIControl* ControlLabel::Create()
{
  ...
  label.align = align;
  label.angle = (float)-iAngle;
  pGUIControl = new CGUILabelControl(iParentId, iControlId, (float)dwPosX, (float)dwPosY,
                                     (float)dwWidth, (float)dwHeight, label, false, bHasPath);
  static_cast<CGUILabelControl*>(pGUIControl)->SetLabel(strText);
  return pGUIControl;
}

void ControlLabel::setLabel(const String& label,
                            const char* font,
                            const char* textColor,
                            const char* disabledColor,
                            const char* shadowColor,
                            const char* focusedColor,
                            const String& label2)
{
  strText = label;
  CGUIMessage msg(GUI_MSG_LABEL_SET, iParentId, iControlId);
...
{
  strTextureFocus =
      focusTexture ? focusTexture : XBMCAddonUtils::getDefaultImage("edit", "texturefocus");

  strTextureNoFocus =
      noFocusTexture ? noFocusTexture : XBMCAddonUtils::getDefaultImage("edit", "texturenofocus");
...
CGUIControl* ControlEdit::Create()
{
  ...
  label.disabledColor = disabledColor;
  label.align = align;
  pGUIControl = new CGUIEditControl(iParentId, iControlId, (float)dwPosX, (float)dwPosY,
                                    (float)dwWidth, (float)dwHeight, CTextureInfo(strTextureFocus),
                                    CTextureInfo(strTextureNoFocus), label, strText);

  if (bIsPassword)
    static_cast<CGUIEditControl*>(pGUIControl)
        ->SetInputType(CGUIEditControl::INPUT_TYPE_PASSWORD, 0);
  return pGUIControl;
}

void ControlEdit::setLabel(const String& label,
                           const char* font,
                           const char* textColor,
                           const char* disabledColor,
                           const char* shadowColor,
...
void ControlEdit::setType(int type, const String& heading)
{
  if (pGUIControl)
  {
    XBMCAddonUtils::GuiLock(languageHook, false);
    static_cast<CGUIEditControl*>(pGUIControl)
        ->SetInputType(static_cast<CGUIEditControl::INPUT_TYPE>(type), CVariant{heading});
  }
}
...
ControlList::ControlList(long x,
  ...
  , space(_space)
  , itemTextOffsetX(_itemTextXOffset)
  , itemTextOffsetY(_itemTextYOffset)
  , alignmentY(_alignmentY)
{
  ...
  strTextureButton = cbuttonTexture
                         ? cbuttonTexture
                         : XBMCAddonUtils::getDefaultImage("listcontrol", "texturenofocus");

  strTextureButtonFocus = cbuttonFocusTexture
                              ? cbuttonFocusTexture
...
CGUIControl* ControlList::Create()
{
  ...
  pGUIControl =
      new CGUIListContainer(iParentId, iControlId, (float)dwPosX, (float)dwPosY, (float)dwWidth,
                            (float)dwHeight - pControlSpin->dwHeight - 5, label, label2,
                            CTextureInfo(strTextureButton), CTextureInfo(strTextureButtonFocus),
                            (float)itemHeight, (float)imageWidth, (float)imageHeight, (float)space);

  return pGUIControl;
}

void ControlList::addItem(const Alternative<String, const XBMCAddon::xbmcgui::ListItem*>& item,
                          bool sendMessage)
{
  XBMC_TRACE;
...
{
  CLog::Log(LOGWARNING, "ControlList::setImageDimensions was called but ... it currently isn't "
                        "defined to do anything.");
  /*
        PyXBMCGUILock();
        if (self->pGUIControl)
        {
...
void ControlList::setItemHeight(long height)
{
  CLog::Log(
      LOGWARNING,
      "ControlList::setItemHeight was called but ... it currently isn't defined to do anything.");
  /*
        PyXBMCGUILock();
        if (self->pGUIControl)
        {
...
{
  CLog::Log(LOGWARNING,
            "ControlList::setSpace was called but ... it currently isn't defined to do anything.");
  /*
        PyXBMCGUILock();
        if (self->pGUIControl)
        {
...
{
  CLog::Log(LOGWARNING, "ControlList::setPageControlVisible was called but ... it currently isn't "
                        "defined to do anything.");
