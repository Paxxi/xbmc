  ...
  : m_offscreen(offscreen)
{
  ...
  if (!thumbnailImage.empty())
    CLog::Log(LOGWARNING,
              "Using thumbnailImage in ListItem constructor results in NOP. Use setArt.");
  if (!path.empty())
    item->SetPath(path);
}
...
void ListItem::setRating(std::string type,
                         float rating,
                         int votes /* = 0 */,
                         bool defaultt /* = false */)
{
...
void ListItem::setProperty(const char* key, const String& value)
{
  ...
  { // special case for start offset - don't actually store in a property,
    ...
    item->m_lStartOffset = CUtil::ConvertSecsToMilliSecs(
        atof(value.c_str())); // we store the offset in frames, or 1/75th of a second
  }
...
void ListItem::setInfo(const char* type, const InfoLabelDict& infoLabels)
{
  ...
  if (strcmpi(type, "video") == 0)
  {
    ...
    for (const auto& it : infoLabels)
    {
      ...
      {
        if (alt.which() != second)
          throw WrongTypeException("When using \"cast\" or \"castandrole\" you need to supply a "
                                   "list of tuples for the value in the dictionary");
      ...
      {
        if (alt.which() != second)
          throw WrongTypeException("When using \"artist\" you need to supply a list of strings for "
  ...
  else if (StringUtils::EqualsNoCase(type, "game"))
  {
    ...
    for (const auto& it : infoLabels)
    {
      ...
      {
        if (alt.which() != second)
          throw WrongTypeException("When using \"genres\" you need to supply a list of strings for "
                                   "the value in the dictionary");
...
void ListItem::addAvailableArtwork(std::string url,
                                   std::string art_type,
                                   std::string preview,
                                   std::string referrer,
                                   std::string cache,
...
{
  XBMCAddonUtils::GuiLock lock(languageHook, m_offscreen);
  GetVideoInfoTag()->m_strPictureURL.AddElement(url, art_type, preview, referrer, cache, post, isgz,
                                                season);
}
...
void ListItem::addContextMenuItems(const std::vector<Tuple<String, String>>& items,
                                   bool replaceItems /* = false */)
{
  for (size_t i = 0; i < items.size(); ++i)
  {
    ...
    if (tuple.GetNumValuesSet() != 2)
      throw ListItemException("Must pass in a list of tuples of pairs of strings. One entry in the "
                              "list only has %d elements.",
                              tuple.GetNumValuesSet());
                                                  ...
                                                  std::string value)
{
  if (alt.which() == first)
  {
    ...
    return StringUtils::Split(
        value, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_videoItemSeparator);
  }
  ...
  {
    if (el.which() == second)
      throw WrongTypeException("When using \"%s\" you need to supply a string or list of strings "
                               "for the value in the dictionary",
                               tag.c_str());
