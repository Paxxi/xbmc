...
Player::Player(int _playerCore)
{
  ...
  if (_playerCore != 0)
    CLog::Log(LOGERROR, "xbmc.Player: Requested non-default player. This behavior is deprecated, "
                        "plugins may no longer specify a player");
...
{
  XBMC_TRACE;
  invokeCallback(new CallbackFunction<Player, int, int>(
      this, &Player::onPlayBackSeek, static_cast<int>(time), static_cast<int>(seekOffset)));
}
