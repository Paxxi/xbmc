...
public:
  inline explicit MaybeLock(CCriticalSection* p_lock)
    : lock(p_lock)
  {
    if (lock)
      lock->lock();
...
public:
  inline SingleLockWithDelayGuard(CCriticalSection& ccrit, LanguageHook* lh)
    : dcg(lh)
    , lock(ccrit)
  {
    lock.lock();
...
CGUIWindow* ProxyExistingWindowInterceptor::get()
{
  XBMC_TRACE;
  return cguiwindow;
}
...
{
  XBMC_TRACE;
  if (iOldWindowId != iWindowId && iWindowId != ACTIVE_WINDOW)
...
Control* Window::GetControlById(int iControlId, CCriticalSection* gc)
{
  ...
  case CGUIControl::GUICONTROL_EDIT:
    ...
    if (li.font)
      ((ControlEdit*)pControl)->strFont = li.font->GetFontName();
    ((ControlButton*)pControl)->align = li.align;
    break;
  default:
    break;
...
bool Window::OnAction(const CAction& action)
{
  ...
  invokeCallback(
      new CallbackFunction<Window, AddonClass::Ref<Action>>(this, &Window::onAction, inf.get()));
  PulseActionEvent();
...
bool Window::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_CLICKED:
  {
    ...
    if (inf.isNotNull())
    {
      ...
      {
        invokeCallback(new CallbackFunction<Window, AddonClass::Ref<Control>>(
            this, &Window::onControl, inf.get()));
        PulseActionEvent();
...
void Window::onControl(Control* action)
{
  XBMC_TRACE; /* do nothing by default */
}
void Window::onClick(int controlId)
{
  XBMC_TRACE; /* do nothing by default */
}
void Window::onDoubleClick(int controlId)
{
  XBMC_TRACE; /* do nothing by default */
}
void Window::onFocus(int controlId)
{
  XBMC_TRACE; /* do nothing by default */
}
void Window::onInit()
...
void Window::removeControls(std::vector<Control*> pControls)
{
  ...
  DelayedCallGuard dg(languageHook);
  int count = 1;
  int size = pControls.size();
  for (std::vector<Control*>::iterator iter = pControls.begin(); iter != pControls.end();
       count++, ++iter)
...
void Window::doAddControl(Control* pControl, CCriticalSection* gcontext, bool wait)
{
...

  {
    ...
    do
      pControl->iControlId = ++iCurrentControlId;
