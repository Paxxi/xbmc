...
public:
  inline explicit PyObjectDecrementor(PyObject* pyobj)
    : obj(pyobj)
  {
  }
                            ...
                            const char* methodname)
{
  ...
  buf = "";
  throw XBMCAddon::WrongTypeException("argument \"%s\" for method \"%s\" must be unicode or str",
...
PythonToCppException::PythonToCppException()
                                             ...
                                             const char* methodNameForErrorString)
{
  if (pythonObj->magicNumber != XBMC_PYTHON_TYPE_MAGIC_NUMBER)
    throw XBMCAddon::WrongTypeException(
        "Non api type passed to \"%s\" in place of the expected type \"%s.\"",
  ...
  if (!isParameterRightType(typeInfo->swigType, expectedType, methodNamespacePrefix))
  {
    ...
    if (typeInfo->parentType)
      ...
      throw XBMCAddon::WrongTypeException(
          "Incorrect type passed to \"%s\", was expecting a \"%s\" but received a \"%s\"",
...
void prepareForReturn(XBMCAddon::AddonClass* c)
{
  ...
  if (c)
  {
    ...
    XBMCAddon::Python::PythonLanguageHook::GetIfExists(state->interp)
        ->RegisterAddonClassInstance(c);
  }
}
                             ...
                             bool incrementRefCount)
{
  ...
  PyTypeObject* typeObj =
      pytype == NULL ? const_cast<PyTypeObject*>(&(typeInfo->pythonType)) : pytype;
