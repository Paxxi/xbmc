...
/**
   ...
   */
void PyXBMCGetUnicodeString(std::string& buf,
                            PyObject* pObject,
...
struct PyHolder
{
  PyObject_HEAD int32_t magicNumber;
...
/**
   ...
   */
inline XBMCAddon::AddonClass* retrieveApiInstance(PyObject* pythonObj,
                                                  ...
                                                  const char* typenameForErrorString)
{
  if (pythonObj == NULL || pythonObj == Py_None)
    return NULL;
  if (reinterpret_cast<PyHolder*>(pythonObj)->magicNumber != XBMC_PYTHON_TYPE_MAGIC_NUMBER ||
      !PyObject_TypeCheck(pythonObj, const_cast<PyTypeObject*>((&(typeToCheck->pythonType)))))
    throw XBMCAddon::WrongTypeException("Incorrect type passed to \"%s\", was expecting a \"%s\".",
                                        methodNameForErrorString, typenameForErrorString);
  return reinterpret_cast<PyHolder*>(pythonObj)->pSelf;
}

bool isParameterRightType(const char* passedType,
                          const char* expectedType,
                          const char* methodNamespacePrefix,
                          bool tryReverse = true);

XBMCAddon::AddonClass* doretrieveApiInstance(const PyHolder* pythonObj,
                                             const TypeInfo* typeInfo,
                                             const char* expectedType,
                                             const char* methodNamespacePrefix,
                                             const char* methodNameForErrorString);
...
/**
   ...
   */
inline XBMCAddon::AddonClass* retrieveApiInstance(const PyObject* pythonObj,
                                                  const char* expectedType,
                                                  const char* methodNamespacePrefix,
                                                  const char* methodNameForErrorString)
{
  return (pythonObj == NULL || pythonObj == Py_None)
             ...
             : doretrieveApiInstance(reinterpret_cast<const PyHolder*>(pythonObj),
                                     ...
                                     expectedType, methodNamespacePrefix, methodNameForErrorString);
}
...
/**
   ...
   */
PyObject* makePythonInstance(XBMCAddon::AddonClass* api,
                             PyTypeObject* pythonType,
...
{
  static inline int compare(PyObject* obj1,
                            PyObject* obj2,
                            const char* swigType,
                            const char* methodNamespacePrefix,
                            const char* methodNameForErrorString)
  {
    ...
    {
      T* o1 =
          (T*)retrieveApiInstance(obj1, swigType, methodNamespacePrefix, methodNameForErrorString);
      T* o2 =
          (T*)retrieveApiInstance(obj2, swigType, methodNamespacePrefix, methodNameForErrorString);
