...
struct closestRes
{
  explicit closestRes(const RESOLUTION_INFO& target)
    : m_target(target){};
  bool operator()(const RESOLUTION_INFO& i, const RESOLUTION_INFO& j)
  {
    float diff = fabs(i.DisplayRatio() - m_target.DisplayRatio()) -
                 fabs(j.DisplayRatio() - m_target.DisplayRatio());
    if (diff < 0)
      return true;
    if (diff > 0)
      return false;
...
std::string CSkinInfo::GetSkinPath(const std::string& strFile,
                                   RESOLUTION_INFO* res,
                                   const std::string& strBaseDir /* = "" */) const
{
  if (m_resolutions.empty())
    return ""; // invalid skin
...
{
  int windowID = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
      CSettings::SETTING_LOOKANDFEEL_STARTUPWINDOW);
  assert(m_startupWindows.size());
  for (std::vector<CStartupWindow>::const_iterator it = m_startupWindows.begin();
       it != m_startupWindows.end(); ++it)
  {
    if (windowID == (*it).m_id)
      return windowID;
  }
...
bool CSkinInfo::IsInUse() const
{
  ...
  return CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
             CSettings::SETTING_LOOKANDFEEL_SKIN) == ID();
}
...
void CSkinInfo::OnPostInstall(bool update, bool modal)
{
       ...
       HELPERS::ShowYesNoDialogText(CVariant{Name()}, CVariant{24099}) == DialogResponse::YES))
  {
    ...
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
            CSettings::SETTING_LOOKANDFEEL_SKIN) == ID())
...
void CSkinInfo::SettingOptionsSkinColorsFiller(SettingConstPtr setting,
                                               ...
                                               void* data)
{
  ...
  list.push_back(StringSettingOption(g_localizeStrings.Get(15109),
...
void CSkinInfo::SettingOptionsSkinFontsFiller(SettingConstPtr setting,
                                              std::vector<StringSettingOption>& list,
                                              std::string& current,
                                              void* data)
{
...
void CSkinInfo::SettingOptionsSkinThemesFiller(SettingConstPtr setting,
                                               std::vector<StringSettingOption>& list,
                                               std::string& current,
                                               void* data)
{
...
void CSkinInfo::SettingOptionsStartupWindowsFiller(SettingConstPtr setting,
                                                   ...
                                                   void* data)
{
  ...
  for (std::vector<CStartupWindow>::const_iterator it = startupWindows.begin();
       it != startupWindows.end(); ++it)
  {
...
bool CSkinInfo::SettingsFromXML(const CXBMCTinyXML& doc, bool loadDefaults /* = false */)
{
  ...
  for (const auto& setting : settings)
  {
    if (setting->GetType() == "string")
      m_strings.insert(std::pair<int, CSkinSettingStringPtr>(
          number++, std::dynamic_pointer_cast<CSkinSettingString>(setting)));
    else if (setting->GetType() == "bool")
      m_bools.insert(std::pair<int, CSkinSettingBoolPtr>(
          number++, std::dynamic_pointer_cast<CSkinSettingBool>(setting)));
    else
...
bool CSkinInfo::SettingsToXML(CXBMCTinyXML& doc) const
{
  ...
  for (const auto& it : m_bools)
  {
    if (!it.second->Serialize(settingsElement))
      CLog::Log(LOGWARNING, "CSkinInfo: failed to save string setting \"%s\"",
  ...
  {
    if (!it.second->Serialize(settingsElement))
      CLog::Log(LOGWARNING, "CSkinInfo: failed to save bool setting \"%s\"",
                it.second->name.c_str());
  }
