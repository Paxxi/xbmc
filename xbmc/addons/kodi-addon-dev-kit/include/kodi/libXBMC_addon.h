...
typedef struct CB_AddOn
{
  ...
  char* (*GetFilePropertyValue)(const void* addonData,
                                void* file,
                                XFILE::FileProperty type,
                                const char* name);
  char** (*GetFilePropertyValues)(const void* addonData,
                                  void* file,
                                  XFILE::FileProperty type,
                                  const char* name,
                                  int* numPorperties);
  ...
  bool (*GetDirectory)(const void* addonData,
                       const char* strPath,
                       const char* mask,
                       VFSDirEntry** items,
  ...
  bool (*CURLAddOption)(const void* addonData,
                        void* file,
...
public:
  ...
  bool RegisterMe(void* handle)
  {
    ...
    if (m_Handle)
      m_Callbacks =
          (KodiAPI::AddOn::CB_AddOnLib*)m_Handle->AddOnLib_RegisterMe(m_Handle->addonData);
    if (!m_Callbacks)
      fprintf(stderr,
              "libXBMC_addon-ERROR: AddOnLib_RegisterMe can't get callback table from Kodi !!!\n");
  ...
  /*!
     ...
     */
  bool WakeOnLan(const char* mac) { return m_Callbacks->WakeOnLan(mac); }
  ...
  /*!
     ...
     */
  char* UnknownToUTF8(const char* str) { return m_Callbacks->UnknownToUTF8(str); }
  ...
  /*!
     ...
     */
  char* GetDVDMenuLanguage() { return m_Callbacks->GetDVDMenuLanguage(m_Handle->addonData); }

  /*!
     * @brief Free the memory used by str
     * @param str The string to free
     */
  void FreeString(char* str) { m_Callbacks->FreeString(m_Handle->addonData, str); }
  ...
  /*!
     ...
     */
  void FlushFile(void* file) { m_Callbacks->FlushFile(m_Handle->addonData, file); }
  ...
  /*!
     ...
     */
  void CloseFile(void* file) { m_Callbacks->CloseFile(m_Handle->addonData, file); }
  ...
  /*!
    ...
    */
  char** GetFilePropertyValues(void* file,
                               XFILE::FileProperty type,
                               const char* name,
                               int* numValues)
  {
  ...
  /*!
     ...
     */
  bool GetDirectory(const char* strPath,
                    const char* mask,
                    VFSDirEntry** items,
                    unsigned int* num_items)
  {
