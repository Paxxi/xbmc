...
extern "C"
{
  ...
  {
    bool(__cdecl* get_keyboard_input)(void* ctx,
                                      const char* heading,
                                      char** input,
                                      bool hidden_input);
    void(__cdecl* set_error_dialog)(
  ...
  typedef struct KodiToAddonFuncTable_VFSEntry /* internal */
  {
    ...
    void*(__cdecl* open_for_write)(const AddonInstance_VFSEntry* instance,
                                   const VFSURL* url,
                                   bool overwrite);
...
public:
  ...
  public:
    ...
    explicit CVFSCallbacks(const VFSGetDirectoryCallbacks* cb)
      : m_cb(cb)
    {
    }
