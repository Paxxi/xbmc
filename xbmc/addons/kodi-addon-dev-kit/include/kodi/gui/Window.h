...
public:
  ...
  {
    GUIHANDLE handle = m_interface->kodi_gui->window->get_list_item(m_interface->kodiBase,
                                                                    m_controlHandle, listPos);
    if (!handle)
      return ListItemPtr();
  ...
  void SetIndependentCallbacks(
      ...
      void (*CBGetContextButtons)(GUIHANDLE cbhdl,
                                  int itemNumber,
...
private:
  static bool CBOnInit(GUIHANDLE cbhdl) { return static_cast<CWindow*>(cbhdl)->OnInit(); }
  ...
  static void CBGetContextButtons(GUIHANDLE cbhdl,
                                  int itemNumber,
                                  gui_context_menu_pair* buttons,
                                  unsigned int* size)
  {
