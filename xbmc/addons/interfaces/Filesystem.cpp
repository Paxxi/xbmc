...
extern "C"
{
  ...
  {
    addonInterface->toKodi->kodi_filesystem = static_cast<AddonToKodiFuncTable_kodi_filesystem*>(
    ...
    addonInterface->toKodi->kodi_filesystem->translate_special_protocol =
  ...
  bool Interface_Filesystem::can_open_directory(void* kodiBase, const char* url)
  {
    ...
    {
      CLog::Log(LOGERROR, "Interface_Filesystem::%s - invalid data (addon='%p', url='%p')",
                __FUNCTION__, kodiBase, url);
      return false;
    }
                                           ...
                                           unsigned int* num_items)
  {
        ...
        num_items == nullptr)
    {
      CLog::Log(LOGERROR,
                ...
                "items='%p', num_items='%p'",
                __FUNCTION__, kodiBase, path, mask, static_cast<void*>(items),
                                            ...
                                            unsigned int num_items)
  {
    ...
    {
      CLog::Log(LOGERROR, "Interface_Filesystem::%s - invalid data (addon='%p', items='%p')",
  ...
  bool Interface_Filesystem::file_exists(void* kodiBase, const char* filename, bool useCache)
  {
    ...
    {
      CLog::Log(LOGERROR, "Interface_Filesystem::%s - invalid data (addon='%p', filename='%p')",
                __FUNCTION__, kodiBase, filename);
      return false;
    }
                                         ...
                                         const char* newFileName)
  {
    ...
    if (addon == nullptr || filename == nullptr || newFileName == nullptr)
    {
      CLog::Log(
          LOGERROR,
          "Interface_Filesystem::%s - invalid data (addon='%p', filename='%p', newFileName='%p')",
          __FUNCTION__, kodiBase, filename, newFileName);
      return false;
    }
  ...
  bool Interface_Filesystem::copy_file(void* kodiBase, const char* filename, const char* dest)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "Interface_Filesystem::%s - invalid data (addon='%p', filename='%p', dest='%p')",
                __FUNCTION__, kodiBase, filename, dest);
      return false;
    }
  ...
  char* Interface_Filesystem::translate_special_protocol(void* kodiBase, const char* strSource)
  {
    ...
    {
      CLog::Log(LOGERROR, "Interface_Filesystem::%s - invalid data (addon='%p', strSource='%p)",
                __FUNCTION__, kodiBase, strSource);
      return nullptr;
    }
  ...
  ssize_t Interface_Filesystem::read_file(void* kodiBase, void* file, void* ptr, size_t size)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "Interface_Filesystem::%s - invalid data (addon='%p', file='%p', ptr='%p')",
                                              ...
                                              int lineLength)
  {
    ...
    {
      CLog::Log(LOGERROR, "Interface_Filesystem::%s - invalid data (addon='%p', file='%p')",
      ...
      void* kodiBase, void* file, int type, const char* name, const char* value)
  {
    ...
    if (addon == nullptr || file == nullptr || name == nullptr || value == nullptr)
    {
      CLog::Log(
          LOGERROR,
          "Interface_Filesystem::%s - invalid data (addon='%p', file='%p', name='%p', value='%p')",
          __FUNCTION__, kodiBase, file, name, value);
      return false;
    }
