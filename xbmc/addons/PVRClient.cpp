...
ADDON_STATUS CPVRClient::Create(int iClientId)
{
  ...
  if ((status = CAddonDll::Create(ADDON_INSTANCE_PVR, &m_struct, &m_struct.props)) ==
      ADDON_STATUS_OK)
 ...
 */
void CPVRClient::WriteClientChannelInfo(const CPVRChannelPtr& xbmcChannel,
                                        PVR_CHANNEL& addonChannel)
{
  ...
  strncpy(addonChannel.strInputFormat, xbmcChannel->InputFormat().c_str(),
          sizeof(addonChannel.strInputFormat) - 1);
}
...
bool CPVRClient::GetAddonProperties(void)
{
  ...
  strFriendlyName =
      StringUtils::Format("%s:%s", strBackendName.c_str(), strConnectionString.c_str());
        ...
        if (retval == PVR_ERROR_NO_ERROR)
        {
          ...
          for (int i = 0; i < size; ++i)
          {
            ...
            timerTypes.emplace_back(
                CPVRTimerTypePtr(new CPVRTimerType(types_array[i], m_iClientId)));
          }
        }
        return retval;
      },
...
{
  return DoAddonCall(
      __FUNCTION__, [](const AddonInstance* addon) { return addon->OpenDialogChannelScan(); },
...
PVR_ERROR CPVRClient::GetEPGForChannel(int iChannelUid, CPVREpg* epg, time_t start, time_t end)
{
  return DoAddonCall(
      ...
      [this, iChannelUid, epg, start, end](const AddonInstance* addon) {
        ...
        return addon->GetEPGForChannel(&handle, iChannelUid, start ? start - iPVRTimeCorrection : 0,
...
{
  return DoAddonCall(
      __FUNCTION__, [iDays](const AddonInstance* addon) { return addon->SetEPGTimeFrame(iDays); },
      m_clientCapabilities.SupportsEPG());
}
                                                ...
                                                CPVRStreamProperties& props)
{
  return DoAddonCall(__FUNCTION__, [&tag, &props](const AddonInstance* addon) {
    ...
    PVR_ERROR error =
        addon->GetEPGTagStreamProperties(&addonTag, properties.get(), &iPropertyCount);
    if (error == PVR_ERROR_NO_ERROR)
...
PVR_ERROR CPVRClient::GetEpgTagEdl(const CConstPVREpgInfoTagPtr& epgTag,
...
PVR_ERROR CPVRClient::DeleteAllRecordingsFromTrash()
{
  return DoAddonCall(
      __FUNCTION__,
      [](const AddonInstance* addon) { return addon->DeleteAllRecordingsFromTrash(); },
      m_clientCapabilities.SupportsRecordingsUndelete());
}
...
PVR_ERROR CPVRClient::ReadRecordedStream(void* lpBuf, int64_t uiBufSize, int& iRead)
{
  ...
  return DoAddonCall(__FUNCTION__, [&lpBuf, uiBufSize, &iRead](const AddonInstance* addon) {
    iRead =
        addon->ReadRecordedStream(static_cast<unsigned char*>(lpBuf), static_cast<int>(uiBufSize));
    return (iRead == -1) ? PVR_ERROR_NOT_IMPLEMENTED : PVR_ERROR_NO_ERROR;
  });
}
...
{
  iPosition = -1;
  return DoAddonCall(__FUNCTION__,
...
PVR_ERROR CPVRClient::SeekTime(double time, bool backwards, double* startpts)
{
  return DoAddonCall(__FUNCTION__, [time, backwards, &startpts](const AddonInstance* addon) {
    return addon->SeekTime(time, backwards, startpts) ? PVR_ERROR_NO_ERROR
...
PVR_ERROR CPVRClient::GetDescrambleInfo(PVR_DESCRAMBLE_INFO& descrambleinfo) const
{
  return DoAddonCall(
      __FUNCTION__,
      [&descrambleinfo](const AddonInstance* addon) {
...
{
  return DoAddonCall(__FUNCTION__,
                     [&times](const AddonInstance* addon) { return addon->GetStreamTimes(times); });
}
                                                  ...
                                                  const PVR_CHANNEL_GROUP_MEMBER* member)
{
  ...
  CPVRChannelPtr channel = CServiceBroker::GetPVRManager().ChannelGroups()->GetByUniqueID(
  ...
  else if (group->IsRadio() == channel->IsRadio())
  {
    ...
    group->AddToGroup(channel, CPVRChannelNumber(member->iChannelNumber, member->iSubChannelNumber),
                      member->iOrder, true);
  }
}
...
void CPVRClient::cb_recording(void* kodiInstance,
                              const char* strName,
...
void CPVRClientCapabilities::InitRecordingsLifetimeValues()
{
  ...
  else if (SupportsRecordingsLifetimeChange())
  {
    ...
    for (int i = 1; i < 366; ++i)
    {
      m_recordingsLifetimeValues.push_back(std::make_pair(
          StringUtils::Format(g_localizeStrings.Get(17999).c_str(), i), i)); // "%s days"
    }
  }
  else
  {
