...
public:
  ...
  static bool DoGetKeyboardInput(void* context,
                                 const char* heading,
                                 char** input,
                                 bool hidden_input);
...
public:
  ...
  explicit CVFSEntryIFileDirectoryWrapper(VFSEntryPtr ptr)
    : CVFSEntryIDirectoryWrapper(ptr)
  {
  }
  ...
  bool ContainsFiles(const CURL& url) override { return m_addon->ContainsFiles(url, m_items); }
  ...
  //! \brief Check if directory exists.
  //! \param[in] url URL to check.
  bool Exists(const CURL& url) override { return CVFSEntryIDirectoryWrapper::Exists(url); }

  //! \brief Delete directory.
  //! \param[in] url URL to delete.
  bool Remove(const CURL& url) override { return CVFSEntryIDirectoryWrapper::Remove(url); }

  //! \brief Create directory.
  //! \param[in] url URL to delete.
  bool Create(const CURL& url) override { return CVFSEntryIDirectoryWrapper::Create(url); }
