...
void CGUIDialogNumeric::OnInitWindow()
{
  ...
  case INPUT_MODE::TIME:
    data["type"] = "time";
    break;
  case INPUT_MODE::DATE:
    data["type"] = "date";
    break;
  case INPUT_MODE::IP_ADDRESS:
    data["type"] = "ip";
    break;
  case INPUT_MODE::PASSWORD:
    data["type"] = "numericpassword";
    break;
  case INPUT_MODE::NUMBER:
    data["type"] = "number";
    break;
  case INPUT_MODE::TIME_SECONDS:
    data["type"] = "seconds";
    break;
  default:
    data["type"] = "keyboard";
  ...
  CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::Input, "xbmc",
...
void CGUIDialogNumeric::OnBackSpace()
{
  ...
  if (m_mode == INPUT_MODE::NUMBER || m_mode == INPUT_MODE::PASSWORD)
  { // just go back one character
    if (!m_number.empty())
      m_number.erase(m_number.length() - 1);
  }
  else if (m_mode == INPUT_MODE::IP_ADDRESS)
  {
    if (m_ip[m_block])
      m_ip[m_block] /= 10;
    else if (m_block)
  ...
  else if (m_mode == INPUT_MODE::TIME)
  {
    if (m_block == 0)
      m_datetime.wHour /= 10;
  ...
  else if (m_mode == INPUT_MODE::TIME_SECONDS)
  {
    if (m_block == 0)
      m_datetime.wHour /= 10;
    else if (m_block == 1)
  ...
  else if (m_mode == INPUT_MODE::DATE)
  {
    if (m_block == 0)
      m_datetime.wDay /= 10;
    else if (m_block == 1)
...
{
  if (m_mode == INPUT_MODE::IP_ADDRESS && m_block == 0 && m_ip[0] == 0)
    return;

  if (m_block < m_lastblock)
    m_block++;
  m_dirty = false;
  if (m_mode == INPUT_MODE::DATE)
...
void CGUIDialogNumeric::FrameMove()
{
  ...
  unsigned int end = 0;
  if (m_mode == INPUT_MODE::PASSWORD)
    strLabel.assign(m_number.length(), '*');
  else if (m_mode == INPUT_MODE::NUMBER)
    strLabel = m_number;
  else if (m_mode == INPUT_MODE::TIME)
  { // format up the time
...
void CGUIDialogNumeric::SetMode(INPUT_MODE mode, const SYSTEMTIME& initial)
{
  ...
  m_block = 0;
  m_lastblock = 0;
  if (m_mode == INPUT_MODE::TIME || m_mode == INPUT_MODE::TIME_SECONDS || m_mode == INPUT_MODE::DATE)
  {
    m_datetime = initial;
    m_lastblock = (m_mode != INPUT_MODE::TIME) ? 2 : 1;
  }
}
...
void CGUIDialogNumeric::SetMode(INPUT_MODE mode, const std::string& initial)
{
  ...
  {
    CDateTime dateTime;
    if (m_mode == INPUT_MODE::TIME || m_mode == INPUT_MODE::TIME_SECONDS)
    {
      ...
      else
      {
        ...
        if (m_mode == INPUT_MODE::TIME_SECONDS && tmp.length() <= 5)
          tmp = "00:" + tmp;
    ...
    m_lastblock = (m_mode == INPUT_MODE::DATE) ? 2 : 1;
  }
  else if (m_mode == INPUT_MODE::IP_ADDRESS)
  {
    m_lastblock = 3;
  ...
  else if (m_mode == INPUT_MODE::NUMBER || m_mode == INPUT_MODE::PASSWORD)
    m_number = initial;
}

SYSTEMTIME CGUIDialogNumeric::GetOutput() const
{
  assert(m_mode == INPUT_MODE::TIME || m_mode == INPUT_MODE::TIME_SECONDS || m_mode == INPUT_MODE::DATE);
  return m_datetime;
}
...
bool CGUIDialogNumeric::ShowAndGetSeconds(std::string& timeString, const std::string& heading)
{
  ...
  pDialog->SetMode(INPUT_MODE::TIME_SECONDS, time);
...
bool CGUIDialogNumeric::ShowAndGetTime(SYSTEMTIME& time, const std::string& heading)
{
  ...
  if (!pDialog)
    return false;
  pDialog->SetMode(INPUT_MODE::TIME, time);
  pDialog->SetHeading(heading);
  pDialog->Open();
  if (!pDialog->IsConfirmed() || pDialog->IsCanceled())
    return false;
...
bool CGUIDialogNumeric::ShowAndGetDate(SYSTEMTIME& date, const std::string& heading)
{
  ...
  if (!pDialog)
    return false;
  pDialog->SetMode(INPUT_MODE::DATE, date);
  pDialog->SetHeading(heading);
  pDialog->Open();
  if (!pDialog->IsConfirmed() || pDialog->IsCanceled())
    return false;
...
bool CGUIDialogNumeric::ShowAndGetIPAddress(std::string& IPAddress, const std::string& heading)
{
  ...
  if (!pDialog)
    return false;
  pDialog->SetMode(INPUT_MODE::IP_ADDRESS, IPAddress);
  pDialog->SetHeading(heading);
  pDialog->Open();
  if (!pDialog->IsConfirmed() || pDialog->IsCanceled())
    return false;
...
bool CGUIDialogNumeric::ShowAndGetNumber(std::string& strInput,
                                         ...
                                         unsigned int iAutoCloseTimeoutMs /* = 0 */)
{
  ...
  pDialog->SetMode(INPUT_MODE::NUMBER, strInput);
...
bool CGUIDialogNumeric::ShowAndVerifyNewPassword(std::string& strNewPassword)
{
  ...
  InputVerificationResult ret =
      ShowAndVerifyInput(strUserInput, g_localizeStrings.Get(12340), false);
...
int CGUIDialogNumeric::ShowAndVerifyPassword(std::string& strPassword,
                                             const std::string& strHeading,
                                             int iRetries)
{
  ...
  if (iRetries > 0)
  {
    ...
    strTempHeading = StringUtils::Format("%s. %s %i %s", strHeading.c_str(),
...
InputVerificationResult CGUIDialogNumeric::ShowAndVerifyInput(std::string& strToVerify,
                                                              ...
                                                              bool bVerifyInput)
{
  ...
  pDialog->SetMode(INPUT_MODE::PASSWORD, strInput);
  pDialog->Open();
  ...
  return StringUtils::EqualsNoCase(strToVerify, md5pword2) ? InputVerificationResult::SUCCESS
