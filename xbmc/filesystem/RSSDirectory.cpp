...
static bool IsPathToMedia(const std::string& strPath)
{
  return URIUtils::HasExtension(
      strPath, CServiceBroker::GetFileExtensionProvider().GetVideoExtensions() + '|' +
                          ...
                          const std::string& path)
{
  ...
  else if (name == "category")
  {
    ...
    else if (scheme == "urn:boxee:title-type")
    {
      ...
      vtag->m_genre = StringUtils::Split(
          text,
  ...
  else if (name == "credit")
  {
    ...
    else if (role == "author" || role == "writer")
    ...
    vtag->m_studio = StringUtils::Split(
        text, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_videoItemSeparator);
...
static void ParseItem(CFileItem* item, TiXmlElement* root, const std::string& path)
{
  ...
  const char* prio[] = {"media:content",   "voddler:trailer", "rss:enclosure", "svtplay:broadcasts",
                        "svtplay:xmllink", "rss:link",        "rss:guid",      NULL};
  ...
  int maxrate = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
  ...
  if (best != resources.end())
  {
    ...
    if (best->mime == "application/rss+xml" &&
        StringUtils::StartsWithNoCase(item->GetPath(), "http://"))
      item->SetPath("rss://" + item->GetPath().substr(7));

    if (best->mime == "application/rss+xml" &&
        StringUtils::StartsWithNoCase(item->GetPath(), "https://"))
...
bool CRSSDirectory::GetDirectory(const CURL& url, CFileItemList& items)
{
  ...
  {
    if (it->second > CDateTime::GetCurrentDateTime() && items.Load())
      return true;
    m_cache.erase(it);
  }
  lock.Leave();
  ...
  {
    CLog::Log(LOGERROR, "failed to load xml from <%s>. error: <%d>", strPath.c_str(),
              xmlDoc.ErrorId());
    return false;
  }
  if (xmlDoc.Error())
  {
    CLog::Log(LOGERROR, "error parsing xml doc from <%s>. error: <%d>", strPath.c_str(),
              xmlDoc.ErrorId());
    return false;
  }
  ...
  for (child = channelXmlNode->FirstChildElement("item"); child;
