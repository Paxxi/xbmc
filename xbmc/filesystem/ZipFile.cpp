  ...
  if (mZipItem.method == 8)
  {
    ...
    case SEEK_SET:
      ...
      if (iFilePosition < m_iFilePos)
      {
        ...
        {
          unsigned int iToRead = (iFilePosition - m_iFilePos) > blockSize ? blockSize : static_cast<int>(iFilePosition - m_iFilePos);
    ...
    case SEEK_END:
      ...
      while( static_cast<int>(m_ZStream.total_out) < mZipItem.usize + iFilePosition)
      {
        unsigned int iToRead = (mZipItem.usize + iFilePosition - m_ZStream.total_out > blockSize) ?
          blockSize : static_cast<int>(mZipItem.usize + iFilePosition - m_ZStream.total_out);
        if (Read(buf.get(), iToRead) != iToRead)
          return -1;
      }
      return m_iFilePos;
...
ssize_t CZipFile::Read(void* lpBuf, size_t uiBufSize)
{
  ...
  if (mZipItem.method == 0) // uncompressed. just read from file, but mind our boundaries.
  {
...
int CZipFile::UnpackFromMemory(std::string& strDest, const std::string& strInput, bool isGZ)
{
  ...
  while( iPos + LHDR_SIZE < strInput.size() || isGZ)
  {
    ...
    if (isGZ)
    {
      ...
      m_ZStream.next_in = reinterpret_cast<Bytef*>(const_cast<char*>(strInput.data()));
      temp = std::make_unique<char>(8192);
      toRead = 8191;
    }
    else
    {
      m_ZStream.avail_in = mZipItem.csize;
      m_ZStream.next_in = reinterpret_cast<Bytef*>(const_cast<char*>(strInput.data())) + iPos + LHDR_SIZE + mZipItem.flength + mZipItem.elength;
      // init m_zipitem
      strDest.reserve(mZipItem.usize);
      temp.reset(new char[mZipItem.usize + 1]);
      toRead = mZipItem.usize;
    }
    int iCurrResult;
    while( (iCurrResult = Read(temp.get(), toRead)) > 0)
    {
      strDest.append(temp.get(),temp.get() + iCurrResult);
      iResult += iCurrResult;
    }
    Close();
