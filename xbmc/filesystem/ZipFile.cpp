...
int64_t CZipFile::Seek(int64_t iFilePosition, int iWhence)
{
  ...
  if (mZipItem.method == 0) // this is easy
  {
    ...
    case SEEK_END:
      ...
      m_iZipFilePos = m_iFilePos;
      iResult =
          mFile.Seek(mZipItem.offset + mZipItem.usize + iFilePosition, SEEK_SET) - mZipItem.offset;
      return iResult;
      break;
    default:
      return -1;
  ...
  if (mZipItem.method == 8)
  {
    ...
    case SEEK_END:
      ...
      {
        ssize_t iToRead = (mZipItem.usize + iFilePosition - m_ZStream.total_out > blockSize)
                              ? blockSize
                              : mZipItem.usize + iFilePosition - m_ZStream.total_out;
        if (Read(buf.get(), iToRead) != iToRead)
          return -1;
      }
      return m_iFilePos;
  ...
  if (mZipItem.method == 8) // deflated
  {
    ...
    while ((iDecompressed < uiBufSize) && ((m_iZipFilePos < mZipItem.csize) || (m_bFlush)))
    {
      ...
      m_bFlush = ((iMessage == Z_OK) && (m_ZStream.avail_out == 0))
                     ? true
                     : false; // more info in input buffer
...
int CZipFile::UnpackFromMemory(std::string& strDest, const std::string& strInput, bool isGZ)
{
  ...
  while (iPos + LHDR_SIZE < strInput.size() || isGZ)
  {
    ...
    else
    {
      ...
      m_ZStream.next_in = const_cast<Bytef*>((const Bytef*)strInput.data()) + iPos + LHDR_SIZE +
                          mZipItem.flength + mZipItem.elength;
      // init m_zipitem
