...
static std::string GetContentMapping(NPT_String& objectClass)
{
  ...
  static const SClassMapping mapping[] = {{"object.item.videoItem.videoBroadcast", "episodes"},
...
+---------------------------------------------------------------------*/
const char* CUPnPDirectory::GetFriendlyName(const CURL& url)
{
...
bool CUPnPDirectory::GetResource(const CURL& path, CFileItem& item)
{
  ...
  {
    CLog::Log(LOGERROR, "CUPnPDirectory::GetResource - no items returned for object %s",
              object.c_str());
    return false;
  }
...
|   CUPnPDirectory::GetDirectory
+---------------------------------------------------------------------*/
bool CUPnPDirectory::GetDirectory(const CURL& url, CFileItemList& items)
{
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
          CSettings::SETTING_SERVICES_UPNP))
    return false;
  ...
  else
  {
    ...
    while (entry)
    {
      ...
      if ((!video &&
           (*entry)->m_ObjectClass.type.CompareN("object.item.videoitem", 21, true) == 0) ||
          (!audio &&
           (*entry)->m_ObjectClass.type.CompareN("object.item.audioitem", 21, true) == 0) ||
    ...
    if (content == "unknown")
    {
      ...
      items.AddSortMethod(SortByLabel, SortAttributeIgnoreFolders, 551,
                          LABEL_MASKS("%L", "%I", "%L", ""));
