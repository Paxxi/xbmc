...
using DirectorizeEntryToFileItemFunction = CFileItemPtr (*)(const TEntry& entry,
...
static void Directorize(const CURL& url,
                        const DirectorizeEntries<TEntry>& entries,
                        DirectorizeEntryToFileItemFunction<TEntry> converter,
                        CFileItemList& items)
{
  ...
  for (const auto& entry : entries)
  {
    ...
    entryFileName.clear();
    for (auto filePathToken = filePathTokens.begin(); filePathToken != filePathTokens.end();
         ++filePathToken)
    {
