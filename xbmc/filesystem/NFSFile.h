...
public:
  ...
  bool splitUrlIntoExportAndPath(const CURL& url,
                                 std::string& exportPath,
...
private:
  ...
  int getContextForExport(
      const std::string&
          exportname); //get context for given export and add to open contexts map - sets m_pNfsContext (my return a already mounted cached context)
