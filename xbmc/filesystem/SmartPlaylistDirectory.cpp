...
bool CSmartPlaylistDirectory::GetDirectory(const CSmartPlaylist& playlist,
                                           CFileItemList& items,
                                           ...
                                           bool filter /* = false */)
{
  ...
  bool isGrouped =
      !group.empty() && !StringUtils::EqualsNoCase(group, "none") && !playlist.IsGroupMixed();
  ...
  if (playlist.GetType() == "movies" || playlist.GetType() == "tvshows" ||
...
std::string CSmartPlaylistDirectory::GetPlaylistByName(const std::string& name,
                                                       const std::string& playlistType)
{
  ...
  if (CSmartPlaylist::IsMusicType(playlistType))
    filesExist =
        CDirectory::GetDirectory("special://musicplaylists/", list, ".xsp", DIR_FLAG_DEFAULTS);
  else // all others are video
    filesExist =
        CDirectory::GetDirectory("special://videoplaylists/", list, ".xsp", DIR_FLAG_DEFAULTS);
  if (filesExist)
  {
