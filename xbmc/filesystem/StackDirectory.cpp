...
std::string CStackDirectory::GetStackedTitlePath(const std::string& strPath)
{
  ...
  const std::vector<std::string>& strRegExps =
...
std::string CStackDirectory::GetStackedTitlePath(const std::string& strPath, VECCREGEXP& RegExps)
{
  ...
  CFileItemList files;
  std::string strStackTitlePath, strCommonDir = URIUtils::GetParentPath(strPath);
  ...
  if (files.Size() > 1)
  {
    ...
    while (itRegExp != RegExps.end())
    {
      ...
      {
        std::string Title1 = itRegExp->GetMatch(1), Volume1 = itRegExp->GetMatch(2),
                    Ignore1 = itRegExp->GetMatch(3), Extension1 = itRegExp->GetMatch(4);
        if (offset)
          Title1 = File1.substr(0, itRegExp->GetSubStart(2));
        if (itRegExp->RegFind(File2, offset) != -1)
        {
          std::string Title2 = itRegExp->GetMatch(1), Volume2 = itRegExp->GetMatch(2),
                      Ignore2 = itRegExp->GetMatch(3), Extension2 = itRegExp->GetMatch(4);
          if (offset)
