...
#define GETN8(p) \
  ((uint64_t)data[p] | ((uint64_t)data[(p) + 1] << 8) | ((uint64_t)data[(p) + 2] << 16) | \
   ((uint64_t)data[(p) + 3] << 24) | ((uint64_t)data[(p) + 4] << 32) | \
   ((uint64_t)data[(p) + 5] << 40) | ((uint64_t)data[(p) + 6] << 48) | \
  ...
  while (p < dirp->dir_length)
  {
    ...
    if (TagID == 257)
    {
      ...
      else
      {
        ...
        strncpy((char*)dirp->entry.d_name, filename, sizeof(dirp->entry.d_name) - 1);
...
int udf25::DVDReadLBUDF(uint32_t lb_number, size_t block_count, unsigned char* data, int encrypted)
{
  ...
  {
    CLog::Log(LOGERROR,
              "udf25::DVDReadLBUDF -  Block was not complete, setting to wanted %u (got %u)",
...
int udf25::UDFFindPartition(int partnum, struct Partition* part)
{
  ...
  do
  {
    ...
    do
    {
      ...
      if ((TagID == 5) && (!part->valid))
      {
        ...
        UDFPartition(LogBlock, &part->Flags, &part->Number, part->Contents, &part->Start,
                     &part->Length);
  ...
  } while ((lbnum < part->Start + part->Length) && (TagID != 8) && (TagID != 256));
...
int udf25::UDFMapICB(struct AD ICB, struct Partition* partition, struct FileAD* File)
{
  ...
  } while (lbnum <= partition->Start + ICB.Location + (ICB.Length - 1) / DVD_VIDEO_LB_LEN);
...
int udf25::UDFScanDir(const struct FileAD& Dir,
                      char* FileName,
                      struct Partition* partition,
                      struct AD* FileICB,
                      int cache_file_info)
{
  ...
  if (DVDUDFCacheLevel(-1) > 0)
  {
    ...
    { /* Assuming dirs don't use chains? */
...
UDF_FILE udf25::UDFFindFile(const char* filename, uint64_t* filesize)
{
  ...
  *filesize = 0;
  ...
  if (!(GetUDFCache(PartitionCache, 0, &partition) && GetUDFCache(RootICBCache, 0, &RootICB)))
  {
    ...
    return NULL; /* Root dir should be dir */
  {
    ...
    while (token != NULL)
    {
      if (!UDFScanDir(File, token, &partition, &ICB, cache_file_info))
        return NULL;
