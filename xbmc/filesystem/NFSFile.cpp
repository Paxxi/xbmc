...
struct nfs_context* CNfsConnection::getContextFromMap(const std::string& exportname,
                                                      bool forceCacheHit /* = false*/)
{
  ...
  if (it != m_openContextMap.end())
  {
    ...
    if ((now - it->second.lastAccessedTime) < CONTEXT_TIMEOUT || forceCacheHit)
    {
      ...
      if (!forceCacheHit) // only log it if this isn't the resetkeepalive on each read ;)
        CLog::Log(LOGDEBUG, "NFS: Refreshing context for %s, old: %" PRId64 ", new: %" PRId64,
...
bool CNfsConnection::Connect(const CURL& url, std::string& relativePath)
{
      ...
      (XbmcThreads::SystemClockMillis() - m_lastAccessedTime) > CONTEXT_TIMEOUT)
  {
    ...
    if (contextRet ==
        CONTEXT_INVALID) //we need a new context because sharename or hostname has changed
    {
      return false;
    }
...
int64_t CNFSFile::GetPosition()
{
  ...
  if (gNfsConnection.GetNfsContext() == NULL || m_pFileHandle == NULL)
    return 0;
...
bool CNFSFile::Open(const CURL& url)
{
  ...
  if (ret != 0)
  {
    CLog::Log(LOGINFO, "CNFSFile::Open: Unable to open file : '%s'  error : '%s'",
...
ssize_t CNFSFile::Read(void* lpBuf, size_t uiBufSize)
{
  ...
  gNfsConnection.resetKeepAlive(
      m_exportPath, m_pFileHandle); //triggers keep alive timer reset for this filehandle

  //something went wrong ...
  if (numberOfBytesRead < 0)
    CLog::Log(LOGERROR, "%s - Error( %" PRId64 ", %s )", __FUNCTION__, (int64_t)numberOfBytesRead,
              nfs_get_error(m_pNfsContext));
...
int64_t CNFSFile::Seek(int64_t iFilePosition, int iWhence)
{
  ...
  if (ret < 0)
  {
    CLog::Log(LOGERROR, "%s - Error( seekpos: %" PRId64 ", whence: %i, fsize: %" PRId64 ", %s)",
              __FUNCTION__, iFilePosition, iWhence, m_fileSize, nfs_get_error(m_pNfsContext));
    return -1;
  }
...
int CNFSFile::Truncate(int64_t iSize)
{
  ...
  if (ret < 0)
  {
    CLog::Log(LOGERROR, "%s - Error( ftruncate: %" PRId64 ", fsize: %" PRId64 ", %s)", __FUNCTION__,
              iSize, m_fileSize, nfs_get_error(m_pNfsContext));
    return -1;
  }
  return ret;
}
...
ssize_t CNFSFile::Write(const void* lpBuf, size_t uiBufSize)
{
  ...
  size_t chunkSize = gNfsConnection.GetMaxWriteChunkSize() > 32768
                         ? 32768
                         : (size_t)gNfsConnection.GetMaxWriteChunkSize();
  ...
  while (leftBytes)
  {
    ...
    writtenBytes = nfs_write(m_pNfsContext, m_pFileHandle, chunkSize,
...
bool CNFSFile::Delete(const CURL& url)
{
  ...
  if (ret != 0)
  {
    CLog::Log(LOGERROR, "%s - Error( %s )", __FUNCTION__,
              nfs_get_error(gNfsConnection.GetNfsContext()));
  }
  return (ret == 0);
}
...
bool CNFSFile::OpenForWrite(const CURL& url, bool bOverWrite)
{
  ...
  if (bOverWrite)
  {
    CLog::Log(LOGWARNING, "FileNFS::OpenForWrite() called with overwriting enabled! - %s",
              filename.c_str());
  ...
  if (ret || m_pFileHandle == NULL)
  {
    ...
    CLog::Log(LOGERROR, "CNFSFile::Open: Unable to open file : '%s' error : '%s'", filename.c_str(),
              nfs_get_error(gNfsConnection.GetNfsContext()));
    m_pNfsContext = NULL;
    m_exportPath.clear();
    return false;
