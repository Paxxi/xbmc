...
#define TAG_X_RESOLUTION \
  0x011A // Not processed. Format rational64u (see http://search.cpan.org/src/EXIFTOOL/Image-ExifTool-6.76/html/TagNames/EXIF.html)
...
#define TAG_COLOUR_SPACE \
  0xA001 // Not processed. Format int16u. Values: 1-RGB; 2-Adobe RGB 65535-Uncalibrated
                            ...
                            int NestingLevel)
{
  ...
  for (int de = 0; de < NumDirEntries; de++)
  {
    ...
    case TAG_FOCALPLANEXRES:
      m_FocalPlaneXRes = ConvertAnyFormat(ValuePtr, Format);
      break;
    case TAG_THUMBNAIL_OFFSET:
      m_ExifInfo->ThumbnailOffset = (unsigned)ConvertAnyFormat(ValuePtr, Format);
      break;
    case TAG_THUMBNAIL_LENGTH:
      m_ExifInfo->ThumbnailSize = (unsigned)ConvertAnyFormat(ValuePtr, Format);
      break;
    ...
    case TAG_INTEROP_OFFSET:
    {
      const unsigned char* const SubdirStart =
          OffsetBase + (unsigned)Get32(ValuePtr, m_MotorolaOrder);
...
//--------------------------------------------------------------------------
bool CExifParse::Process(const unsigned char* const ExifSection,
                         ...
                         ExifInfo_t* info)
{
  ...
  const char* pos =
      (const char*)(ExifSection + sizeof(short)); // position data pointer after length field
    ...
    if (m_ExifInfo->FocalLength35mmEquiv == 0)
    {
      ...
      if (m_ExifInfo->CCDWidth != 0.0)
      {
        m_ExifInfo->FocalLength35mmEquiv =
            (int)(m_ExifInfo->FocalLength / m_ExifInfo->CCDWidth * 36 + 0.5);
      }
    }
  }
  return true;
...
//--------------------------------------------------------------------------
void CExifParse::GetLatLong(const unsigned int Format,
                            ...
                            char* latLongString)
{
  ...
  else
  {
    ...
    if (Values[0] < 0 || Values[0] > 180 || Values[1] < 0 || Values[1] >= 60 || Values[2] < 0 ||
        Values[2] >= 60)
    {
...
//--------------------------------------------------------------------------
void CExifParse::ProcessGpsInfo(const unsigned char* const DirStart,
                                int ByteCountUnused,
                                const unsigned char* const OffsetBase,
                                unsigned ExifLength)
{
