...
//
//--------------------------------------------------------------------------
bool CIptcParse::Process(const unsigned char* const Data,
                         ...
                         IPTCInfo_t* info)
{
  ...
  if (memcmp(pos, IptcSignature1, strlen(IptcSignature1) - 1) != 0)
    return false;
  pos += sizeof(IptcSignature1); // move data pointer to the next field

  if (memcmp(pos, IptcSignature2, strlen(IptcSignature2) - 1) != 0)
    return false;
  pos += sizeof(IptcSignature2) - 1; // move data pointer to the next field

  while (memcmp(pos, IptcSignature3, sizeof(IptcSignature3)) != 0)
  { // loop on valid Photoshop blocks
    ...
    if (memcmp(pos, IptcSignature2, sizeof(IptcSignature2) - 1) != 0)
      return false;
    pos += sizeof(IptcSignature2) - 1; // move data pointer to the next field
  }
  ...
  while (pos < (const char*)(Data + itemlen - 5))
  {
    ...
    if (signature == 0x1C02)
    {
      ...
      case IPTC_RECORD_VERSION:
        tag = info->RecordVersion;
        break;
      case IPTC_SUPLEMENTAL_CATEGORIES:
        tag = info->SupplementalCategories;
        break;
      case IPTC_KEYWORDS:
        tag = info->Keywords;
        break;
      ...
      case IPTC_TRANSMISSION_REFERENCE:
        tag = info->TransmissionReference;
        break;
      case IPTC_DATE:
        tag = info->Date;
        break;
      ...
      case IPTC_REFERENCE_SERVICE:
        tag = info->ReferenceService;
        break;
      case IPTC_COUNTRY_CODE:
        tag = info->CountryCode;
        break;
      case IPTC_TIME_CREATED:
        tag = info->TimeCreated;
        break;
      case IPTC_SUB_LOCATION:
        tag = info->SubLocation;
        break;
      case IPTC_IMAGE_TYPE:
        tag = info->ImageType;
        break;
      default:
