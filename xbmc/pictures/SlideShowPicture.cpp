...
void CSlideShowPic::Process(unsigned int currentTime, CDirtyRegionList &dirtyregions)
{
  ...
  { // do start transition
    ...
    { // fade in at 1x speed
      alpha = static_cast<color_t>(static_cast<float>(m_iCounter) / static_cast<float>(m_transitionStart.length) * 255.0f);
    }
...
//    CLog::Log(LOGDEBUG,"Transitioning");
    ...
    { // fade out at 1x speed
      alpha = 255 - static_cast<color_t>(static_cast<float>(m_iCounter - m_transitionEnd.start) / static_cast<float>(m_transitionEnd.length) * 255.0f);
    }
