...
bool CSlideShowPic::DisplayEffectNeedChange(DISPLAY_EFFECT newDispEffect) const
{
  if (m_displayEffect == newDispEffect)
    return false;
  if (newDispEffect == EFFECT_RANDOM && m_displayEffect != EFFECT_NONE &&
                                        ...
                                        TRANSITION_EFFECT transEffect)
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
          CSettings::SETTING_SLIDESHOW_HIGHQUALITYDOWNSCALING))
  ...
  if (m_displayEffect != EFFECT_NO_TIMEOUT)
    fadeTime = std::min(0.2f * CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                                   CSettings::SETTING_SLIDESHOW_STAYTIME),
                        3.0f);
  m_transitionStart.length = (int)(CServiceBroker::GetWinSystem()->GetGfxContext().GetFPS() *
  ...
  int iFrames = std::max((int)(CServiceBroker::GetWinSystem()->GetGfxContext().GetFPS() *
                                   ...
                                   CDirtyRegionList& dirtyregions)
{
  ...
  if (memcmp(cur_x, new_x, count) || memcmp(cur_y, new_y, count) || m_bIsDirty)
  {
...
{
  if (!m_pImage || !m_bIsLoaded || m_bIsFinished)
    return;
  ...
  { // do end transition
    ...
    { // fade out at 1x speed
      alpha = 255 - (UTILS::Color)((float)(m_iCounter - m_transitionEnd.start) /
                                   (float)m_transitionEnd.length * 255.0f);
    }
    else if (m_transitionEnd.type == FADEIN_FADEOUT)
    { // keep solid, then fade out at 2x speed
      alpha = (UTILS::Color)((float)(m_transitionEnd.length - m_iCounter + m_transitionEnd.start) /
  ...
  if (m_displayEffect != EFFECT_NO_TIMEOUT || m_iCounter < m_transitionStart.length ||
      m_iCounter >= m_transitionEnd.start ||
      (m_iCounter >= m_transitionTemp.start &&
       m_iCounter < m_transitionTemp.start + m_transitionTemp.length))
  {
  ...
  float fComp = 1.0f + 0.01f * CServiceBroker::GetSettingsComponent()
                                   ->GetAdvancedSettings()
  ...
  if (m_displayEffect != EFFECT_NO_TIMEOUT && fScreenRatio < fSourceAR * fComp &&
      fSourceAR < fScreenRatio * fComp)
    bFillScreen = true;
  if ((!bFillScreen && fScreenWidth * fPixelRatio > fScreenHeight * fSourceAR) ||
      (bFillScreen && fScreenWidth * fPixelRatio < fScreenHeight * fSourceAR))
    fScaleNorm = fScreenHeight / (m_fHeight * fPixelRatio);
  bFillScreen = false;
  if (m_displayEffect != EFFECT_NO_TIMEOUT && fScreenRatio < fSourceInvAR * fComp &&
      fSourceInvAR < fScreenRatio * fComp)
    bFillScreen = true;
  if ((!bFillScreen && fScreenWidth * fPixelRatio > fScreenHeight * fSourceInvAR) ||
      (bFillScreen && fScreenWidth * fPixelRatio < fScreenHeight * fSourceInvAR))
  ...
  float fSmallWidth =
      sqrt(fSmallArea * fAR / fPixelRatio); // fAR*height = width, so total area*far = width*width
  ...
  for (int i = 0; i < 4; i++)
  {
    if (ox[i] < fSmallX)
      ox[i] = fSmallX;
    if (ox[i] > fSmallX + fSmallWidth)
      ox[i] = fSmallX + fSmallWidth;
    if (oy[i] < fSmallY)
      oy[i] = fSmallY;
    if (oy[i] > fSmallY + fSmallHeight)
      oy[i] = fSmallY + fSmallHeight;
  }
...
void CSlideShowPic::Rotate(float fRotateAngle, bool immediate /* = false */)
{
  ...
  // reset the timer
  m_transitionEnd.start = m_iCounter + m_transitionStart.length +
                          (int)(CServiceBroker::GetWinSystem()->GetGfxContext().GetFPS() *
                                CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                                    CSettings::SETTING_SLIDESHOW_STAYTIME));
}
...
void CSlideShowPic::Render()
{
  ...
  // now render the image in the top right corner if we're zooming
  if (m_fZoomAmount == 1.0f || m_bIsComic)
    return;
