                                       ...
                                       size_t& result_size)
{
  ...
  if (image == NULL || !image->CreateThumbnailFromSurface(const_cast<unsigned char*>(buffer), width,
...
bool CPicture::CreateThumbnailFromSurface(
    const unsigned char* buffer, int width, int height, int stride, const std::string& thumbFile)
{
  CLog::Log(LOGDEBUG, "cached image '%s' size %dx%d", CURL::GetRedacted(thumbFile).c_str(), width,
            height);
  if (URIUtils::HasExtension(thumbFile, ".jpg"))
  {
#if defined(TARGET_RASPBERRY_PI)
    if (COMXImage::CreateThumbnailFromSurface(const_cast<unsigned char*>(buffer), width, height,
                                              XB_FMT_A8R8G8B8, stride, thumbFile.c_str()))
      return true;
#endif
  }
  ...
  if (pImage == NULL || !pImage->CreateThumbnailFromSurface(const_cast<unsigned char*>(buffer),
                                                            width, height, XB_FMT_A8R8G8B8, stride,
                                                            thumbFile.c_str(), thumb, thumbsize))
  {
    CLog::Log(LOGERROR, "Failed to CreateThumbnailFromSurface for %s",
...
CThumbnailWriter::CThumbnailWriter(
    unsigned char* buffer, int width, int height, int stride, const std::string& thumbFile)
...
bool CThumbnailWriter::DoWork()
{
  ...
  {
    CLog::Log(LOGERROR, "CThumbnailWriter::DoWork unable to write %s",
                                 ...
                                 scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
  ...
  if (dest_width >= width || dest_height >= height)
    return GetThumbnailFromSurface(pixels, dest_width, dest_height, pitch, image, result,
                                   result_size);
  ...
  if (!ScaleImage(pixels, width, height, pitch, buffer, dest_width, dest_height,
                  dest_width * sizeof(uint32_t), scalingAlgorithm))
  {
    delete[] buffer;
    result = NULL;
    result_size = 0;
  ...
  bool success = GetThumbnailFromSurface(buffer, dest_width, dest_height,
                                         dest_width * sizeof(uint32_t), image, result, result_size);
  delete[] buffer;
                                ...
                                scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
  return CacheTexture(texture->GetPixels(), texture->GetWidth(), texture->GetHeight(),
                      texture->GetPitch(), texture->GetOrientation(), dest_width, dest_height, dest,
                      scalingAlgorithm);
}
                              ...
                              scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
  struct SwsContext* context =
      sws_getContext(in_width, in_height, AV_PIX_FMT_BGRA, out_width, out_height, AV_PIX_FMT_BGRA,
