...
bool CGUIInfoManager::GetBool(int condition1, int contextWindow, const CGUIListItem *item)
{
  ...
  {
    if (item)
      return GetItemBool(item, condition);
    
    auto window = GetWindowWithCondition(contextWindow, WINDOW_CONDITION_HAS_LIST_ITEMS); // true for has list items
    if (window)
    {
      auto curItem = window->GetCurrentListItem();
      return GetItemBool(curItem.get(), condition);
    }
  }
...
void CGUIInfoManager::UpdateAVInfo()
{
  if(g_application.m_pPlayer->IsPlaying())
  {
    if (g_dataCacheCore.HasAVInfoChanges())
    {
      ...
      m_videoInfo = video;
      auto mp = dynamic_cast<GUIINFO::CGUIMusicPlayerInfo>(m_infoHandlers[MUSICPLAYER_MASK]);
      if (mp)
        mp.UpdateAVInfo();
    }
  }
...
std::string CGUIInfoManager::GetItemLabel(const CFileItem *item, int info, std::string *fallback)
{
  ...
  return fallback != nullptr ? *fallback : std::string();
}
