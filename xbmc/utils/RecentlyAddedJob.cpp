...
bool CRecentlyAddedJob::UpdateVideo()
{
  ...
  if (videodatabase.GetRecentlyAddedMoviesNav("videodb://recentlyaddedmovies/", items, NUM_ITEMS))
  {
    for (; i < items.Size(); ++i)
    {
      ...
      home->SetProperty("LatestMovie." + value + ".RunningTime",
                        item->GetVideoInfoTag()->GetDuration() / 60);
  ...
  if (videodatabase.GetRecentlyAddedEpisodesNav("videodb://recentlyaddedepisodes/", TVShowItems,
                                                NUM_ITEMS))
  {
  ...
  if (videodatabase.GetRecentlyAddedMusicVideosNav("videodb://recentlyaddedmusicvideos/",
...
bool CRecentlyAddedJob::UpdateTotal()
{
  ...
  musicUrl.AddOption("albumartistsonly",
  ...
  int MusAlbumTotals =
      atoi(musicdatabase.GetSingleValue("songview", "count(distinct strAlbum)").c_str());
  musicdatabase.Close();
  ...
  int MusVidWatched =
      atoi(videodatabase.GetSingleValue("musicvideo_view", "count(playCount)").c_str());
  ...
  int TvShowsWatched =
      atoi(videodatabase.GetSingleValue("tvshow_view", "sum(watchedcount = totalcount)").c_str());
  videodatabase.Close();
