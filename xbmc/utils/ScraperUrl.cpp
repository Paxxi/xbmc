...
unsigned int CScraperUrl::GetMaxSeasonThumb() const
{
  ...
  for (std::vector<SUrlEntry>::const_iterator iter = m_url.begin(); iter != m_url.end(); ++iter)
  {
    if (iter->m_type == URL_TYPE_SEASON && iter->m_season > 0 &&
        (unsigned int)iter->m_season > maxSeason)
                      ...
                      const std::string& cacheContext)
{
  ...
  if (ftype == CMime::FileTypeZip || ftype == CMime::FileTypeGZip)
  {
    ...
    int iSize = file.UnpackFromMemory(
        strBuffer, strHTML, scrURL.m_isgz); // FIXME: use FileTypeGZip instead of scrURL.m_isgz?
    if (iSize > 0)
    {
      strHTML = strBuffer;
      CLog::Log(LOGDEBUG, "%s: Archive \"%s\" was unpacked in memory", __FUNCTION__,
                scrURL.m_url.c_str());
    }
    else
  ...
  if (ftype == CMime::FileTypeHtml)
  {
    ...
    if (!CCharsetDetection::ConvertHtmlToUtf8(strHTML, converted, reportedCharset, realHtmlCharset))
      ...
      CLog::Log(LOGDEBUG, "%s: Using \"%s\" charset for HTML \"%s\"", __FUNCTION__,
                realHtmlCharset.c_str(), scrURL.m_url.c_str());
  ...
  else if (ftype == CMime::FileTypeXml)
  {
    ...
    {
      CLog::Log(LOGDEBUG, "%s: Using \"%s\" charset for XML \"%s\"", __FUNCTION__,
                realXmlCharset.c_str(), scrURL.m_url.c_str());
           ...
           StringUtils::EqualsNoCase(mimeType.substr(0, 5), "text/"))
  {
    ...
    if (reportedCharset != realTextCharset)
      CLog::Log(LOGWARNING,
                ...
                "charset",
                __FUNCTION__, realTextCharset.c_str(), scrURL.m_url.c_str(),
                reportedCharset.c_str());
    else
  ...
  {
    CLog::Log(LOGDEBUG, "%s: Using \"%s\" charset for \"%s\"", __FUNCTION__,
              reportedCharset.c_str(), scrURL.m_url.c_str());
