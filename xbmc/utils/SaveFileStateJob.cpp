...
void CSaveFileState::DoWork(CFileItem& item, CBookmark& bookmark, bool updatePlayCount)
{
  ...
  if (!progressTrackingFile.empty())
  {
    ...
    if (item.IsVideo())
    {
      ...
      {
        CLog::Log(LOGWARNING, "%s - Unable to open video database. Can not save file state!",
                  __FUNCTION__);
      }
      else
      {
    ...
    if (item.IsAudio())
    {
      ...
      if (updatePlayCount)
      {
        ...
        {
          CLog::Log(LOGWARNING, "%s - Unable to open music database. Can not save file state!",
                    __FUNCTION__);
        }
        else
        {
