...
void HttpParser::parseHeader()
{
  ...
  static const struct FSM
  {
    ...
    State nextState;
    unsigned actions;
  } fsm[] = {{p_request_line, CR, p_request_line_cr, NULLIFY},
  ...
  for (unsigned i = _parsedTo; i < _data.length(); ++i)
  {
    ...
    for (const FSM& f : fsm)
    {
      if (f.curState == _state && (c == f.c || f.c == ANY))
      {
...
HttpParser::status_t HttpParser::addBytes(const char* bytes, unsigned len)
{
...
{
  for (IntArray::const_iterator iter = _keys.begin(); iter != _keys.end(); ++iter)
  {
    unsigned index = *iter;
