...
static Translator s_videoDbTranslator[] = {{"videodb://1/1", "videodb://movies/genres"},
...
// ATTENTION: Make sure the longer match strings go first
// because the string match is performed with StringUtils::StartsWith()
static Translator s_musicDbTranslator[] = {{"musicdb://10", "musicdb://singles"},
...
std::string CLegacyPathTranslation::TranslatePath(const std::string& legacyPath,
                                                  ...
                                                  size_t translationMapSize)
{
  ...
  for (size_t index = 0; index < translationMapSize; index++)
  {
    ...
    {
      StringUtils::Replace(newPath, translationMap[index].legacyPath,
                           translationMap[index].newPath);
      break;
    }
  }
