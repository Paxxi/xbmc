...
template<class T>
typename GlobalsSingleton<T>::template Deleter<std::shared_ptr<T>> GlobalsSingleton<T>::instance;
template<class T>
...
{
public:
  explicit InitFlag(bool& flag) { flag = true; }
};
} // namespace xbmcutil
...
#define XBMC_GLOBAL_REF(classname, g_variable) \
  static std::shared_ptr<classname> g_variable##Ref( \
