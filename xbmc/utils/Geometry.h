...
public:
  ...
  template<class U>
  explicit constexpr CPointGen(const CPointGen<U>& rhs)
  ...
  constexpr this_type operator*(T factor) const { return {x * factor, y * factor}; }
...
public:
  ...
  CSizeGen(T width, T height) { CheckSet(width, height); }

  T Width() const { return m_w; }

  T Height() const { return m_h; }

  void SetWidth(T width) { CheckSet(width, m_h); }

  void SetHeight(T height) { CheckSet(m_w, height); }

  void Set(T width, T height) { CheckSet(width, height); }

  bool IsZero() const { return (m_w == static_cast<T>(0) && m_h == static_cast<T>(0)); }

  T Area() const { return m_w * m_h; }

  CPointGen<T> ToPoint() const { return {m_w, m_h}; }

  template<class U>
  explicit CSizeGen<T>(const CSizeGen<U>& rhs)
  {
    CheckSet(static_cast<T>(rhs.m_w), static_cast<T>(rhs.m_h));
  }

  this_type operator+(const this_type& size) const { return {m_w + size.m_w, m_h + size.m_h}; };
  ...
  this_type operator*(T factor) const { return {m_w * factor, m_h * factor}; }
...
public:
  ...
  template<class U>
  explicit constexpr CRectGen(const CRectGen<U>& rhs)
  ...
  constexpr bool IsEmpty() const XBMC_FORCE_INLINE { return (x2 - x1) * (y2 - y1) == 0; };

  constexpr point_type P1() const XBMC_FORCE_INLINE { return {x1, y1}; }

  constexpr point_type P2() const XBMC_FORCE_INLINE { return {x2, y2}; }

  constexpr T Width() const XBMC_FORCE_INLINE { return x2 - x1; };

  constexpr T Height() const XBMC_FORCE_INLINE { return y2 - y1; };

  constexpr T Area() const XBMC_FORCE_INLINE { return Width() * Height(); };

  size_type ToSize() const { return {Width(), Height()}; };
...
{
  return (rect1.x1 == rect2.x1 && rect1.y1 == rect2.y1 && rect1.x2 == rect2.x2 &&
          rect1.y2 == rect2.y2);
}
