...
#define CHECK_HEADER_L1 \
  CHECK_PROT_LINE_200 \
      ...
      "Pragma: no-cache\r\n" \
      "Set-Cookie: user_country=ot; expires=Thu, 09-Jan-2014 18:58:30 GMT; path=/; " \
      "domain=.example.com\r\n" \
      "\r\n"
...
TEST(TestHttpHeader, General)
{
  ...
  EXPECT_FALSE(testHdr.GetValue(CHECK_CNT_TYPE_NAME).empty())
      << "Parsed header has empty \"" CHECK_CNT_TYPE_NAME "\" parameter";
  EXPECT_FALSE(testHdr.GetValues(CHECK_CNT_TYPE_NAME).empty())
      << "Parsed header has no \"" CHECK_CNT_TYPE_NAME "\" parameters";
  ...
  EXPECT_TRUE(testHdr.GetValues(CHECK_CNT_TYPE_NAME).empty())
      << "Cleared object has some parameters";
...
TEST(TestHttpHeader, Parse)
{
  ...
  EXPECT_STREQ(CHECK_CONTENT_TYPE_HTML, testHdr.GetValue(CHECK_CNT_TYPE_NAME).c_str())
      << "Wrong value of parameter \"" CHECK_CNT_TYPE_NAME "\"";

  /* check autoclearing when new header is parsed */
  testHdr.Parse(CHECK_PROT_LINE_200 "\r\n");
  EXPECT_FALSE(testHdr.IsHeaderDone()) << "Not completed header has \"parsing finished\" state";
  EXPECT_TRUE(testHdr.GetValues(CHECK_CNT_TYPE_NAME).empty())
      << "Cleared header has some parameters";
  testHdr.Clear();
  EXPECT_TRUE(testHdr.GetHeader().empty()) << "Cleared object is not empty";
  ...
  testHdr.Parse(CHECK_HEADER_SMPL);
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STRCASEEQ(CHECK_HEADER_SMPL, testHdr.GetHeader().c_str())
      << "Parsed header mismatch the original header";
  testHdr.Parse(CHECK_HEADER_L1);
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STRCASEEQ(CHECK_HEADER_L1, testHdr.GetHeader().c_str())
      << "Parsed header mismatch the original header";
  EXPECT_STREQ("Thu, 09 Jan 2014 17:58:30 GMT",
               testHdr.GetValue("Date").c_str()); // case-sensitive match of value
  testHdr.Parse(CHECK_HEADER_L2);
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STRCASEEQ(CHECK_HEADER_L2, testHdr.GetHeader().c_str())
      << "Parsed header mismatch the original header";
  EXPECT_STREQ("Thu, 09 Jan 2014 20:10:28 GMT",
               testHdr.GetValue("Last-Modified").c_str()); // case-sensitive match of value
  testHdr.Parse(CHECK_HEADER_R);
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STRCASEEQ(CHECK_HEADER_R, testHdr.GetHeader().c_str())
      << "Parsed header mismatch the original header";
  EXPECT_STREQ("http://www.Example.Com",
               testHdr.GetValue("Location").c_str()); // case-sensitive match of value
...
TEST(TestHttpHeader, Parse_Multiline)
{
  ...
  testHdr.Parse("\r\n");
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STREQ("This is multi line value", testHdr.GetValue("X-Comment").c_str())
      << "Wrong multiline value";
  ...
  testHdr.Parse(" :line\r\n valu:e\r\n\n"); // the only parameter
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_STREQ("This is: mul:ti :line valu:e", testHdr.GetValue("X-Comment").c_str())
      << "Wrong multiline value";
  ...
  testHdr.Parse("\r\n");
  EXPECT_TRUE(testHdr.IsHeaderDone()) << "Completed header has \"parsing not finished\" state";
  EXPECT_GE(strlen("Apache/2.4.7 (Unix)     mod_wsgi/3.4 \tPython/2.7.5\t \t \tOpenSSL/1.0.1e"),
            testHdr.GetValue("Server").length())
      << "Length of miltiline value is greater than length of original string";
  EXPECT_LE(strlen("Apache/2.4.7 (Unix) mod_wsgi/3.4 Python/2.7.5 OpenSSL/1.0.1e"),
            testHdr.GetValue("Server").length())
      << "Length of miltiline value is less than length of trimmed original string";
  EXPECT_STREQ("Apache/2.4.7(Unix)mod_wsgi/3.4Python/2.7.5OpenSSL/1.0.1e",
               strReplace(strReplace(testHdr.GetValue("Server"), " ", ""), "\t", "").c_str())
      << "Multiline value with removed whitespaces does not match original string with removed "
         "whitespaces";
...
TEST(TestHttpHeader, GetValue)
{
  ...
  EXPECT_STREQ(CHECK_DATE_VALUE2, testHdr.GetValue(CHECK_DATE_NAME).c_str())
  ...
  EXPECT_STREQ("http://www.Example.Com", testHdr.GetValue("Location").c_str())
      << "Wrong parameter value";
  EXPECT_TRUE(testHdr.GetValue("foo").empty())
      << "Some value is returned for non-existed parameter";
  ...
  EXPECT_STREQ("http://www.Example.Com", testHdr.GetValue("location").c_str())
      << "Wrong parameter value for lowercase name";
  EXPECT_STREQ("http://www.Example.Com", testHdr.GetValue("LOCATION").c_str())
      << "Wrong parameter value for UPPERCASE name";
  EXPECT_STREQ("http://www.Example.Com", testHdr.GetValue("LoCAtIOn").c_str())
      << "Wrong parameter value for MiXEdcASe name";

  /* Check value of last added parameter with the same name is returned */
  testHdr.Parse(CHECK_HEADER_L1);
  EXPECT_STREQ("close", testHdr.GetValue("Connection").c_str()) << "Wrong parameter value";
  EXPECT_STREQ(
      "user_country=ot; expires=Thu, 09-Jan-2014 18:58:30 GMT; path=/; domain=.example.com",
      testHdr.GetValue("Set-Cookie").c_str())
      << "Wrong parameter value";
  EXPECT_STREQ(
      "user_country=ot; expires=Thu, 09-Jan-2014 18:58:30 GMT; path=/; domain=.example.com",
      testHdr.GetValue("set-cookie").c_str())
...
TEST(TestHttpHeader, GetValues)
{
  ...
  testHdr.Parse(CHECK_HEADER_L1);
  EXPECT_EQ(1U, testHdr.GetValues("Server").size())
      << "Wrong number of values for parameter \"Server\"";
  EXPECT_STREQ("nginx/1.4.4", testHdr.GetValues("Server")[0].c_str()) << "Wrong parameter value";
  EXPECT_EQ(2U, testHdr.GetValues("Set-Cookie").size())
      << "Wrong number of values for parameter \"Set-Cookie\"";
  EXPECT_STREQ("PHPSESSID=90857d437518db8f0944ca012761048a; path=/; domain=example.com",
               testHdr.GetValues("Set-Cookie")[0].c_str())
      << "Wrong parameter value";
  EXPECT_STREQ(
      "user_country=ot; expires=Thu, 09-Jan-2014 18:58:30 GMT; path=/; domain=.example.com",
      testHdr.GetValues("Set-Cookie")[1].c_str())
      << "Wrong parameter value";
  EXPECT_TRUE(testHdr.GetValues("foo").empty())
      << "Some values are returned for non-existed parameter";
}
...
TEST(TestHttpHeader, AddParam)
{
  ...
  EXPECT_EQ(3U, testHdr.GetValues("Server").size())
      << "Wrong number of values for parameter \"Server\"";
  ...
  EXPECT_EQ(3U, testHdr.GetValues("X-FOO").size())
      << "Wrong number of values for parameter \"X-foo\"";
  ...
  EXPECT_STREQ("testValue2 and more", testHdr.GetValue("X-TestParam").c_str())
      << "Wrong parameter value";
...
TEST(TestHttpHeader, GetMimeType)
{
  ...
  EXPECT_STREQ(CHECK_CONTENT_TYPE_TEXT, testHdr.GetMimeType().c_str())
      << "MIME-type was not overwritten by \"AddParam\"";

  /* Correct trimming */
  testHdr.AddParam(CHECK_CNT_TYPE_NAME, "  " CHECK_CONTENT_TYPE_TEXT " \t ;foo=bar");
  EXPECT_STREQ(CHECK_CONTENT_TYPE_TEXT, testHdr.GetMimeType().c_str())
      << "MIME-type is not trimmed correctly";
}
...
TEST(TestHttpHeader, GetCharset)
{
  ...
  EXPECT_STREQ("WINDOWS-1252", testHdr.GetCharset().c_str())
      << "Charset was not overwritten by \"AddParam\"";
