...
{
  CLog::Log(LOGINFO, "Start ripping track %s to %s", m_input.c_str(), m_output.c_str());
  ...
  std::string strLine0 = StringUtils::Format(
      "%02i. %s - %s", iTrack, m_tag.GetArtistString().c_str(), m_tag.GetTitle().c_str());
  ...
  if (file.IsRemote() && !cancelled && result == 2)
  {
    ...
    {
      CLog::Log(LOGERROR, "CDDARipper: Error copying file from %s to %s", m_output.c_str(),
                file.GetPath().c_str());
...
CEncoder* CCDDARipJob::SetupEncoder(CFile& reader)
{
  ...
  else
  {
    const BinaryAddonBasePtr addonInfo =
        CServiceBroker::GetBinaryAddonManager().GetInstalledAddonInfo(audioEncoder,
                                                                      ADDON_AUDIOENCODER);
    if (addonInfo)
    {
  ...
  const std::string strTrack = StringUtils::Format(
      "%li", strtol(m_input.substr(13, m_input.size() - 13 - 5).c_str(), nullptr, 10));
