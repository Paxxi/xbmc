...
bool CDVDDemuxFFmpeg::Open(std::shared_ptr<CDVDInputStream> pInput, bool streaminfo, bool fileinfo)
{
  ...
  if (!skipCreateStreams || m_pFormatContext->nb_programs > 0)
  {
    ...
    if (m_pFormatContext->nb_programs > 0)
    {
      ...
      else if (m_pFormatContext->iformat &&
               strcmp(m_pFormatContext->iformat->name, "hls,applehttp") == 0)
      {
...
AVDictionary* CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput()
{
  ...
  if (url.IsProtocol("http") || url.IsProtocol("https"))
  {
         ...
         it != protocolOptions.end(); ++it)
    {
      ...
      {
        CLog::Log(LOGDEBUG,
                  "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() ignoring header option '%s: %s'",
                  it->first.c_str(), value.c_str());
      }
    }
    if (!hasUserAgent)
...
DemuxPacket* CDVDDemuxFFmpeg::Read()
{
  ...
  if (pPacket && pPacket->iStreamId >= 0)
  {
    ...
    {
      if (static_cast<CDemuxStreamAudio*>(stream)->iChannels !=
              m_pFormatContext->streams[pPacket->iStreamId]->codecpar->channels ||
          static_cast<CDemuxStreamAudio*>(stream)->iSampleRate !=
