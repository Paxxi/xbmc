...
void cc708_service_reset(cc708_service_decoder *decoder)
{
  ...
  for (auto & window : decoder->windows)
  {
    window.is_defined=0;
    window.visible=0;
...
void clearTV (cc708_service_decoder *decoder)
{
  for (auto & i : decoder->tv.chars)
  {
    memset (i, ' ', I708_SCREENGRID_COLUMNS);
    i[I708_SCREENGRID_COLUMNS]=0;
  }
};
...
void printTVtoBuf (cc708_service_decoder *decoder)
{
  ...
  for (auto & i : decoder->tv.chars)
  {
    ...
    if (!empty)
    {
      ...
      for (int j=f;j<=l;j++) {
        decoder->text[decoder->textlen++] = i[j];
}
