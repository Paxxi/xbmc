...
void CMMALYUVBuffer::GetPlanes(uint8_t* (&planes)[YuvImage::MAX_PLANES])
{
  ...
  if (VERBOSE)
    CLog::Log(LOGDEBUG, LOGVIDEO, "%s::%s %dx%d %dx%d (%dx%d %dx%d)", CLASSNAME, __FUNCTION__,
              geo.getStrideY(), geo.getHeightY(), geo.getStrideC(), geo.getHeightC(), Width(),
              Height(), AlignedWidth(), AlignedHeight());
...
void CMMALYUVBuffer::SetDimensions(int width,
                                   int height,
...
CDecoder::CDecoder(CProcessInfo& processInfo, CDVDStreamInfo& hints)
  : m_processInfo(processInfo)
  , m_hints(hints)
{
  CLog::Log(LOGDEBUG, LOGVIDEO, "%s::%s - create %p", CLASSNAME, __FUNCTION__,
...
CDecoder::~CDecoder()
{
  ...
  CLog::Log(LOGDEBUG, LOGVIDEO, "%s::%s - destroy %p", CLASSNAME, __FUNCTION__,
            static_cast<void*>(this));
}
...
int CDecoder::FFGetBuffer(AVCodecContext* avctx, AVFrame* frame, int flags)
{
  ...
  CLog::Log(LOGDEBUG, LOGVIDEO, "%s::%s %dx%d format:%x:%x flags:%x", CLASSNAME, __FUNCTION__,
            frame->width, frame->height, frame->format, dec->m_fmt, flags);

  if ((avctx->codec && (avctx->codec->capabilities & AV_CODEC_CAP_DR1) == 0) ||
      frame->format != dec->m_fmt)
  {
    assert(0);
  ...
  AVBufferRef* buf = av_buffer_create((uint8_t*)gmem->m_arm, gmem->m_numbytes,
                                      CDecoder::FFReleaseBuffer, gmem, AV_BUFFER_FLAG_READONLY);
  if (!buf)
  {
...
bool CDecoder::Open(AVCodecContext* avctx, AVCodecContext* mainctx, enum AVPixelFormat fmt)
{
  ...
  CLog::Log(LOGDEBUG, "%s::%s MMAL - source requires %d references", CLASSNAME, __FUNCTION__,
            avctx->refs);
  ...
  m_pool = std::make_shared<CMMALPool>(MMAL_COMPONENT_DEFAULT_VIDEO_DECODER, false,
                                       MMAL_NUM_OUTPUT_BUFFERS, 0, MMAL_ENCODING_UNKNOWN,
                                       MMALStateFFDec);
  if (!m_pool)
  {
...
CDVDVideoCodec::VCReturn CDecoder::Decode(AVCodecContext* avctx, AVFrame* frame)
{
  ...
  if (frame)
  {
    if ((frame->format != AV_PIX_FMT_YUV420P && frame->format != AV_PIX_FMT_YUV420P10 &&
         frame->format != AV_PIX_FMT_YUV420P12 && frame->format != AV_PIX_FMT_YUV420P14 &&
         frame->format != AV_PIX_FMT_YUV420P16 && frame->format != AV_PIX_FMT_BGR0 &&
