...
DEFINE_GUID(
    DXVA_NoEncrypt, 0x1b81beD0, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00, 0xc0, 0x4f, 0x2e, 0x73, 0xc5);
#endif

static const int PROFILES_MPEG2_SIMPLE[] = {FF_PROFILE_MPEG2_SIMPLE, FF_PROFILE_UNKNOWN};
static const int PROFILES_MPEG2_MAIN[] = {FF_PROFILE_MPEG2_SIMPLE, FF_PROFILE_MPEG2_MAIN,
                                          FF_PROFILE_UNKNOWN};
...
static const int PROFILES_HEVC_MAIN[] = {FF_PROFILE_HEVC_MAIN, FF_PROFILE_UNKNOWN};
static const int PROFILES_HEVC_MAIN10[] = {FF_PROFILE_HEVC_MAIN, FF_PROFILE_HEVC_MAIN_10,
                                           FF_PROFILE_UNKNOWN};
static const int PROFILES_VP9_0[] = {FF_PROFILE_VP9_0, FF_PROFILE_UNKNOWN};
static const int PROFILES_VP9_10_2[] = {FF_PROFILE_VP9_2, FF_PROFILE_UNKNOWN};
...
static const std::vector<dxva2_mode_t> dxva2_modes = {
    {"MPEG2 variable-length decoder", &D3D11_DECODER_PROFILE_MPEG2_VLD, AV_CODEC_ID_MPEG2VIDEO,
     PROFILES_MPEG2_MAIN},
    {"MPEG1/2 variable-length decoder", &D3D11_DECODER_PROFILE_MPEG2and1_VLD,
     AV_CODEC_ID_MPEG2VIDEO, PROFILES_MPEG2_MAIN},
    ...
    {"H.264 variable-length decoder, film grain technology", &D3D11_DECODER_PROFILE_H264_VLD_FGT,
     AV_CODEC_ID_H264, PROFILES_H264_HIGH},
    {"H.264 variable-length decoder, no film grain technology (Intel ClearVideo)",
     &DXVADDI_Intel_ModeH264_E, AV_CODEC_ID_H264, PROFILES_H264_HIGH},
    {"H.264 variable-length decoder, no film grain technology",
     &D3D11_DECODER_PROFILE_H264_VLD_NOFGT, AV_CODEC_ID_H264, PROFILES_H264_HIGH},
    {"H.264 variable-length decoder, no film grain technology, FMO/ASO",
     &D3D11_DECODER_PROFILE_H264_VLD_WITHFMOASO_NOFGT, AV_CODEC_ID_H264, PROFILES_H264_HIGH},
    {"H.264 variable-length decoder, no film grain technology, Flash",
     &DXVA_ModeH264_VLD_NoFGT_Flash, AV_CODEC_ID_H264, PROFILES_H264_HIGH},

    {"H.264 inverse discrete cosine transform, film grain technology",
     &D3D11_DECODER_PROFILE_H264_IDCT_FGT, 0, nullptr},
    {"H.264 inverse discrete cosine transform, no film grain technology",
     &D3D11_DECODER_PROFILE_H264_IDCT_NOFGT, 0, nullptr},
    {"H.264 inverse discrete cosine transform, no film grain technology (Intel)",
     &DXVADDI_Intel_ModeH264_C, 0, nullptr},

    {"H.264 motion compensation, film grain technology", &D3D11_DECODER_PROFILE_H264_MOCOMP_FGT, 0,
     nullptr},
    {"H.264 motion compensation, no film grain technology",
     &D3D11_DECODER_PROFILE_H264_MOCOMP_NOFGT, 0, nullptr},
    ...
    {"H.264 stereo high profile, mbs flag set",
     &D3D11_DECODER_PROFILE_H264_VLD_STEREO_PROGRESSIVE_NOFGT, 0, nullptr},
    ...
    {"Windows Media Video 9 inverse discrete cosine transform", &D3D11_DECODER_PROFILE_WMV9_IDCT, 0,
     nullptr},
    ...
    {"VC-1 variable-length decoder 2010", &D3D11_DECODER_PROFILE_VC1_D2010, AV_CODEC_ID_VC1,
     nullptr},
    {"VC-1 variable-length decoder 2010", &D3D11_DECODER_PROFILE_VC1_D2010, AV_CODEC_ID_WMV3,
     nullptr},
    ...
    {"HEVC variable-length decoder, main", &D3D11_DECODER_PROFILE_HEVC_VLD_MAIN, AV_CODEC_ID_HEVC,
     PROFILES_HEVC_MAIN},
    {"HEVC variable-length decoder, main10", &D3D11_DECODER_PROFILE_HEVC_VLD_MAIN10,
     AV_CODEC_ID_HEVC, PROFILES_HEVC_MAIN10},

    {"VP9 variable-length decoder, Profile 0", &D3D11_DECODER_PROFILE_VP9_VLD_PROFILE0,
     AV_CODEC_ID_VP9, PROFILES_VP9_0},
    {"VP9 variable-length decoder, 10bit, profile 2", &D3D11_DECODER_PROFILE_VP9_VLD_10BIT_PROFILE2,
     AV_CODEC_ID_VP9, PROFILES_VP9_10_2},
};
...
bool CContext::CreateContext()
{
  ...
  if (m_sharingAllowed)
  {
    ...
    hr = D3D11CreateDevice(DX::DeviceResources::Get()->GetAdapter(), D3D_DRIVER_TYPE_UNKNOWN,
                           nullptr, D3D11_CREATE_DEVICE_VIDEO_SUPPORT, featureLevels,
                           ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &pD3DDevice, nullptr,
                           &pD3DDeviceContext);
                                  ...
                                  D3D11_VIDEO_DECODER_CONFIG& config) const
{
  ...
  for (const dxva2_mode_t& mode : dxva2_modes)
  {
    ...
    if (supported)
    {
      ...
      if (!supported)
        CLog::LogFunction(LOGDEBUG, "DXVA", "Unsupported profile %d for %s.", avctx->profile,
                          mode.name);
    }
    if (!supported)
      continue;
    ...
    for (unsigned j = 0; render_targets_dxgi[j]; ++j)
    {
      ...
      {
        CLog::LogFunction(LOGNOTICE, "DXVA", "Ouput format %d is not supported by '%s'",
                         ...
                         D3D11_VIDEO_DECODER_CONFIG& config) const
{
  ...
  if (!cfg_count)
  {
    CLog::LogF(LOGNOTICE, "no decoder configuration possible for %dx%d (%d).", format.SampleWidth,
  ...
  for (unsigned i = 0; i < cfg_count; i++)
  {
    ...
    CLog::LogFunction(
        LOGDEBUG, "DXVA", "config %d: bitstream type %d%s.", i, pConfig.ConfigBitstreamRaw,
                              ...
                              HANDLE* pHandle) const
{
  ...
  CLog::LogFunction(LOGDEBUG, "DXVA", "allocating %d surfaces with format %d.", count,
                    format.OutputFormat);
  ...
  : m_processInfo(processInfo)
{
  ...
  m_avD3D11Context->surface = reinterpret_cast<ID3D11VideoDecoderOutputView**>(
      av_mallocz_array(32, sizeof(ID3D11VideoDecoderOutputView*)));
  m_bufferPool.reset();
...
static bool CheckCompatibility(AVCodecContext* avctx)
{
  ...
  if (HasVP3WidthBug(avctx))
  {
    CLog::LogFunction(LOGWARNING, "DXVA",
                      "width %i is not supported with nVidia VP3 hardware. DXVA will not be used.",
                      avctx->coded_width);
    return false;
  }
  ...
  {
    CLog::LogFunction(LOGWARNING, "DXVA",
                      "compatibility check: video exceeds L4.1. DXVA will not be used.");
    return false;
  }
...
bool CDecoder::Open(AVCodecContext* avctx, AVCodecContext* mainctx, enum AVPixelFormat fmt)
{
  ...
  if (m_format.Guid == DXVADDI_Intel_ModeH264_E && avctx->refs > 11)
  {
    ...
    CLog::LogFunction(LOGWARNING, "DXVA", "too many references %d for selected decoder '%s'.",
  ...
  {
    CLog::LogFunction(LOGWARNING, "DXVA",
                      "source requires to much refs which is not supported on Xbox One S/X. dxva "
                      "will not be used.");
    return false;
  }
  ...
  if (m_format.Guid == DXVADDI_Intel_ModeH264_E)
  {
    ...
    CLog::LogFunction(
        LOGWARNING, "DXVA",
        "used Intel ClearVideo decoder, but no support workaround for it in libavcodec.");
#endif
  }
...
#else
    CLog::LogFunction(LOGWARNING, "DXVA",
                      "video card with different scaling list zigzag order detected, but no "
                      "support in libavcodec.");
#endif
  }
...
CDVDVideoCodec::VCReturn CDecoder::Check(AVCodecContext* avctx)
{
  ...
  if (avctx->codec_id == AV_CODEC_ID_H264)
  {
    if (status.h264.bStatus)
      CLog::LogFunction(LOGWARNING, "DXVA", "decoder problem of status %d with %d.",
...
bool CDecoder::OpenDecoder()
{
  ...
  if (m_dxvaContext->IsContextShared())
  {
    ...
    if (AIdentifier.VendorId ==
        PCIV_Intel) //std::reinterpret_pointer_cast<CVideoBufferPool<CVideoBuffer>>(
