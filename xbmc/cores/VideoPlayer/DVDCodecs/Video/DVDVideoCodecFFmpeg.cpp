...
void CDVDVideoCodecFFmpeg::CDropControl::Process(int64_t pts, bool drop)
{
  if (m_state == INIT)
  {
    ...
    if (m_count > 10)
    {
      ...
      if (m_diffPTS > 0)
      {
        CLog::Log(LOGNOTICE, "CDVDVideoCodecFFmpeg::CDropControl: calculated diff time: %" PRId64,
                  m_diffPTS);
                                                   ...
                                                   const AVPixelFormat* fmt)
{
  ...
  while (*cur != AV_PIX_FMT_NONE)
  {
    ...
    {
      IHardwareDecoder* pDecoder(CDVDFactoryCodec::CreateVideoCodecHWAccel(
          hwaccel, ctx->m_hints, ctx->m_processInfo, *cur));
      if (pDecoder)
      {
...
bool CDVDVideoCodecFFmpeg::Open(CDVDStreamInfo& hints, CDVDCodecOptions& options)
{
  ...
  m_formats.push_back(
      AV_PIX_FMT_NONE); /* always add none to get a terminated list in ffmpeg world */
  ...
  CLog::Log(LOGNOTICE, "CDVDVideoCodecFFmpeg::Open() Using codec: %s",
  ...
  if (!(hints.codecOptions & CODEC_FORCE_SOFTWARE))
  {
    ...
    else
    {
      ...
      CLog::Log(LOGDEBUG, "CDVDVideoCodecFFmpeg - open frame threaded with %d threads",
                num_threads);
    }
  }
  else
  ...
  int iSkipLoopFilter =
      CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_iSkipLoopFilter;
...
CDVDVideoCodec::VCReturn CDVDVideoCodecFFmpeg::GetPicture(VideoPicture* pVideoPicture)
{
  ...
  else
  {
    ...
    bool need_scale =
        std::find(m_formats.begin(), m_formats.end(), m_pCodecContext->pix_fmt) == m_formats.end();
...
bool CDVDVideoCodecFFmpeg::GetPictureCommon(VideoPicture* pVideoPicture)
{
  ...
  if (m_pCodecContext->coded_height &&
      m_pCodecContext->coded_height < (int)pVideoPicture->iHeight &&
  ...
  pVideoPicture->color_space = m_pCodecContext->colorspace == AVCOL_SPC_UNSPECIFIED
                                   ? m_hints.colorSpace
  ...
  pVideoPicture->qp_table =
      av_frame_get_qp_table(m_pFrame, &pVideoPicture->qstride, &pVideoPicture->qscale_type);
...
int CDVDVideoCodecFFmpeg::FilterOpen(const std::string& filters, bool scale)
{
  ...
  {
    CLog::Log(LOGWARNING,
              "CDVDVideoCodecFFmpeg::FilterOpen - skipped opening filters on hardware decode");
    return 0;
  }
  ...
  const AVFilter* outFilter =
      avfilter_get_by_name("buffersink"); // should be last filter in the graph for now

  std::string args = StringUtils::Format(
      "%d:%d:%d:%d:%d:%d:%d", m_pCodecContext->width, m_pCodecContext->height,
  ...
  if ((result = avfilter_graph_create_filter(&m_pFilterOut, outFilter, "out", NULL, NULL,
                                             m_pFilterGraph)) < 0)
  {
