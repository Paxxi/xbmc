...
public:
  explicit PredicateVideoFilter(int videoStream)
...
int CSelectionStreams::TypeIndexOf(StreamType type, int source, int64_t demuxerId, int id) const
{
  ...
  auto it = std::find_if(m_Streams.begin(), m_Streams.end(), [&](const SelectionStream& stream) {
    return stream.type == type && stream.source == source && stream.id == id &&
...
bool CVideoPlayer::OpenInputStream()
{
  ...
  {
    CLog::Log(LOGERROR, "CVideoPlayer::OpenInputStream - unable to create input stream for [%s]",
  ...
  {
    CLog::Log(LOGERROR, "CVideoPlayer::OpenInputStream - error opening [%s]",
              CURL::GetRedacted(m_item.GetPath()).c_str());
    return false;
  }
...
bool CVideoPlayer::ReadPacket(DemuxPacket*& packet, CDemuxStream*& stream)
{
  ...
  if (m_pSubtitleDemuxer && m_VideoPlayerSubtitle->AcceptsData())
  {
    ...
    if (packet)
    {
      ...
      if (!stream)
      {
        CLog::Log(LOGERROR, "%s - Error demux packet doesn't belong to a valid stream",
                  __FUNCTION__);
        return false;
      }
...
bool CVideoPlayer::IsBetterStream(CCurrentStream& current, CDemuxStream* stream)
{
                         ...
                         m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY)))
  {
    ...
    if (source_type != STREAM_SOURCE_DEMUX || stream->type != current.type ||
...
void CVideoPlayer::Prepare()
{
  ...
  if (m_playerOptions.starttime > 0 || m_playerOptions.startpercent > 0)
  {
    ...
    {
      int playerStartTime = (int)(((float)m_pDemuxer->GetStreamLength()) *
                                  (m_playerOptions.startpercent / (float)100));
      starttime = m_Edl.RestoreCutTime(playerStartTime);
    }
    else
    {
      starttime =
          m_Edl.RestoreCutTime(static_cast<int>(m_playerOptions.starttime * 1000)); // s to ms
    }
    CLog::Log(LOGDEBUG, "%s - Start position set to last stopped position: %d", __FUNCTION__,
              starttime);
  }
  else if (m_Edl.InCut(starttime, &cut))
  {
    ...
    {
      starttime = cut.end;
      CLog::Log(LOGDEBUG, "%s - Start position set to end of first cut: %d", __FUNCTION__,
                starttime);
    }
    else if (cut.action == EDL::Action::COMM_BREAK)
    {
      ...
      {
        starttime = cut.end;
        CLog::Log(LOGDEBUG, "%s - Start position set to end of first commercial break: %d",
                  __FUNCTION__, starttime);
      }

      std::string strTimeString =
          StringUtils::SecondsToTimeString(cut.end / 1000, TIME_FORMAT_MM_SS);
  ...
  if (starttime > 0)
  {
    ...
    if (m_pSubtitleDemuxer)
    {
      if (m_pSubtitleDemuxer->SeekTime(starttime, true, &startpts))
        ...
        CLog::Log(LOGDEBUG, "%s - failed to start subtitle demuxing from: %d", __FUNCTION__,
                  starttime);
    }
    ...
    if (!pPacket)
    {
              ...
              std::dynamic_pointer_cast<CDVDInputStream::IMenus>(m_pInputStream))
      {
        ...
        if (m_dvd.state == DVDSTATE_STILL)
        {
          ...
          {
            if ((XbmcThreads::SystemClockMillis() - m_dvd.iDVDStillStartTime) >=
                m_dvd.iDVDStillTime)
            {
...
void CVideoPlayer::ProcessAudioData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  if (CheckSceneSkip(m_CurrentAudio))
    drop = true;
  else if (m_Edl.InCut(DVD_TIME_TO_MSEC(m_CurrentAudio.dts + m_offset_pts), &cut) &&
...
bool CVideoPlayer::GetCachingTimes(double& level, double& delay, double& offset)
{
  ...
  double cache_need = std::max(
      0.0, remain - play_left / cache_sbp); /* bytes needed until play_left == cache_left */
    ...
    if (m_playSpeed == DVD_PLAYSPEED_NORMAL && !isInMenu)
    {
          ...
          m_syncTimer.IsTimePast())
      {
        ...
        else
        {
          ...
          if (m_VideoPlayerAudio->GetLevel() <= 50 && m_processInfo->GetLevelVQ() <= 50)
          {
            SetCaching(CACHESTATE_FULL);
          }
      ...
      if (check)
      {
        ...
        if (error > DVD_MSEC_TO_TIME(1000))
        {
              ...
              (m_VideoPlayerVideo->IsRewindStalled() && std::abs(error) > 100))
          {
            ...
            double iTime =
                (m_clock.GetClock() + m_State.time_offset + 1000000.0 * direction) / 1000;
            CDVDMsgPlayerSeek::CMode mode;
            mode.time = iTime;
...
bool CVideoPlayer::CheckPlayerInit(CCurrentStream& current)
{
  ...
  if (current.startpts != DVD_NOPTS_VALUE)
  {
    ...
    {
      CLog::Log(LOGDEBUG, "%s - dropping packet type:%d dts:%f to get to start point at %f",
...
bool CVideoPlayer::CheckSceneSkip(CCurrentStream& current)
{
  ...
  EDL::Cut cut;
  return m_Edl.InCut(DVD_TIME_TO_MSEC(current.dts + m_offset_pts), &cut) &&
...
void CVideoPlayer::CheckAutoSceneSkip()
{
  ...
  else if (cut.action == EDL::Action::COMM_BREAK)
  {
    ...
    {
      std::string strTimeString =
          StringUtils::SecondsToTimeString((cut.end - cut.start) / 1000, TIME_FORMAT_MM_SS);
      CGUIDialogKaiToast::QueueNotification(g_localizeStrings.Get(25011), strTimeString);
      ...
      {
        CLog::Log(LOGDEBUG,
                  "%s - Clock in commercial break [%s - %s]: %s. Automatically skipping to end of "
                  "commercial break",
                  __FUNCTION__, CEdl::MillisecondsToTimeString(cut.start).c_str(),
                  CEdl::MillisecondsToTimeString(cut.end).c_str(),
                  CEdl::MillisecondsToTimeString(clock).c_str());
  ...
  while (m_messenger.Get(&pMsg, 0) == MSGQ_OK)
  {
             ...
             m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK_CHAPTER) == 0)
    {
      ...
      CServiceBroker::GetGUI()
          ...
          .GetInfoProviders()
          .GetPlayerInfoProvider()
          .SetDisplayAfterSeek(2500, offset);
    }
    ...
    else if (pMsg->IsType(CDVDMsg::PLAYER_REPORT_STATE))
    {
      ...
      CLog::Log(LOGDEBUG, "CVideoPlayer::HandleMessages - player %d reported state: %d", msg.player,
                msg.syncState);
    }
...
void CVideoPlayer::GetGeneralInfo(std::string& strGeneralInfo)
{
  if (!m_bStop)
  {
    ...
    strGeneralInfo = StringUtils::Format("Player: a/v:% 6.3f, %s", dDiff, strBuf.c_str());
  }
}
    ...
    CCurrentStream& current, int64_t demuxerId, int iStream, int source, bool reset /*= true*/)
{
  ...
  if (STREAM_SOURCE_MASK(source) == STREAM_SOURCE_DEMUX_SUB)
  {
    ...
    {
      CLog::Log(LOGNOTICE, "No demuxer found for file %s", CURL::GetRedacted(st.filename).c_str());
      return false;
    }
  ...
  else
  {
    if (stream)
    {
      ...
      CLog::Log(LOGWARNING, "%s - Unsupported stream %d. Stream disabled.", __FUNCTION__,
                stream->uniqueId);
...
bool CVideoPlayer::OpenAudioStream(CDVDStreamInfo& hint, bool reset)
{
  ...
  static_cast<IDVDStreamPlayerAudio*>(player)->SendMessage(
      new CDVDMsg(CDVDMsg::PLAYER_REQUEST_STATE), 1);
  ...
  if (m_CurrentVideo.id < 0 || m_CurrentVideo.hint != hint)
  {
    ...
    if (hint.fpsrate > 0 && hint.fpsscale > 0)
    {
      float fFramesPerSecond =
          (float)m_CurrentVideo.hint.fpsrate / (float)m_CurrentVideo.hint.fpsscale;
...
void CVideoPlayer::FlushBuffers(double pts, bool accurate, bool sync)
{
  ...
  if (m_playSpeed == DVD_PLAYSPEED_NORMAL || m_playSpeed == DVD_PLAYSPEED_PAUSE)
  {
    ...
    CDVDMsgGeneralSynchronize* msg =
        new CDVDMsgGeneralSynchronize(1000, SYNCSOURCE_AUDIO | SYNCSOURCE_VIDEO);
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY))
  {
    ...
    case BD_EVENT_STILL_TIME:
    {
      if (m_dvd.state != DVDSTATE_STILL)
      {
        ...
        CLog::Log(LOGDEBUG, "BD_EVENT_STILL_TIME - waiting %i sec, with delay of %d sec",
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    ...
    case DVDNAV_STILL_FRAME:
    {
      ...
      if (m_dvd.state != DVDSTATE_STILL)
      {
        ...
        m_dvd.state = DVDSTATE_STILL;
        CLog::Log(LOGDEBUG, "DVDNAV_STILL_FRAME - waiting %i sec, with delay of %d sec",
    ...
    case DVDNAV_VTS_CHANGE:
    {
      ...
      if (m_VideoPlayerVideo->IsInited())
        m_VideoPlayerVideo->SendMessage(
            new CDVDMsgDouble(CDVDMsg::VIDEO_SET_ASPECT, m_CurrentVideo.hint.aspect));
...
int CVideoPlayer::AddSubtitleFile(const std::string& filename, const std::string& subfilename)
{
  ...
  if (ext == ".idx")
  {
    ...
    ExternalStreamInfo info =
        CUtil::GetExternalStreamDetailsFromFilename(m_item.GetDynPath(), vobsubfile);
...
void CVideoPlayer::UpdateFileItemStreamDetails(CFileItem& item)
{
  ...
  item.GetVideoInfoTag()->m_streamDetails.SetStreams(videoInfo, m_processInfo->GetMaxTime() / 1000,
