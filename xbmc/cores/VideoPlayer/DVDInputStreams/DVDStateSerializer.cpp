...
bool CDVDStateSerializer::DVDToXMLState(std::string& xmlstate, const dvd_state_t* state)
{
...

  {
    ...
    for (int i = 0; i < 16; i++)
    {
      if (state->registers.GPRM[i] || state->registers.GPRM_mode[i] ||
          state->registers.GPRM_time[i].tv_sec || state->registers.GPRM_time[i].tv_usec)
      {
...
bool CDVDStateSerializer::XMLToDVDState(dvd_state_t* state, const std::string& xmlstate)
{
  ...
  if (strcmp(hRoot.Element()->Value(), "navstate") != 0)
    return false;
  ...
  while (element)
  {
    ...
    if (index >= 0 && index < 16)
    {
      ...
      text = TiXmlHandle(element)
                 .FirstChildElement("time")
                 .FirstChildElement("tv_sec")
                 .FirstChild()
                 .Text();
      if (text)
