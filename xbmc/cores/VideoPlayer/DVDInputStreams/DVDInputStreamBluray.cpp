...
BLURAY_TITLE_INFO* CDVDInputStreamBluray::GetTitleFile(const std::string& filename)
{
  ...
  {
    CLog::Log(LOGERROR, "get_playlist_title - unsupported playlist file selected %s",
...
bool CDVDInputStreamBluray::Open()
{
  ...
  if (openStream)
  {
    ...
    {
      CLog::Log(LOGERROR, "CDVDInputStreamBluray::Open - failed to open %s in stream mode",
  ...
  else
  {
    ...
    {
      CLog::Log(LOGERROR, "CDVDInputStreamBluray::Open - failed to open %s",
                CURL::GetRedacted(root).c_str());
      return false;
    }
  }
  ...
  if (disc_info->bluray_detected)
  {
    ...
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - Disc name           : %s",
              disc_info->disc_name ? disc_info->disc_name : "");
#endif
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - First Play supported: %d",
              disc_info->first_play_supported);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - Top menu supported  : %d",
              disc_info->top_menu_supported);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - HDMV titles         : %d",
              disc_info->num_hdmv_titles);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - BD-J titles         : %d",
              disc_info->num_bdj_titles);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - BD-J handled        : %d",
              disc_info->bdj_handled);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - UNSUPPORTED titles  : %d",
              disc_info->num_unsupported_titles);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - AACS detected       : %d",
              disc_info->aacs_detected);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - libaacs detected    : %d",
              disc_info->libaacs_detected);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - AACS handled        : %d",
              disc_info->aacs_handled);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - BD+ detected        : %d",
              disc_info->bdplus_detected);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - libbdplus detected  : %d",
              disc_info->libbdplus_detected);
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - BD+ handled         : %d",
              disc_info->bdplus_handled);
#if (BLURAY_VERSION >= BLURAY_VERSION_CODE(1, 0, 0))
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - no menus (libmmbd, or profile 6 bdj)  : %d",
  ...
  if (m_navmode)
  {
    ...
    if (bd_play(m_bd) <= 0)
    {
      CLog::Log(LOGERROR, "CDVDInputStreamBluray::Open - failed play disk %s",
  ...
  else
  {
    ...
    {
      CLog::Log(LOGERROR, "CDVDInputStreamBluray::Open - failed to select playlist %d",
                m_titleInfo->idx);
      return false;
    }
    m_clip = nullptr;
...
void CDVDInputStreamBluray::ProcessEvent()
{
  ...
  case BD_EVENT_PG_TEXTST_STREAM:
    ...
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray - BD_EVENT_PG_TEXTST_STREAM %d, %d", m_event.param,
              pid);
    m_player->OnDiscNavResult(static_cast<void*>(&pid), BD_EVENT_PG_TEXTST_STREAM);
    break;

  case BD_EVENT_MENU:
    CLog::Log(LOGDEBUG, "CDVDInputStreamBluray - BD_EVENT_MENU %d", m_event.param);
  ...
  case BD_EVENT_SOUND_EFFECT:
  {
    ...
    {
      CLog::Log(LOGDEBUG, "CDVDInputStreamBluray - BD_EVENT_SOUND_EFFECT %d", m_event.param);
    }
  }
  ...
  default:
    CLog::Log(LOGWARNING, "CDVDInputStreamBluray - unhandled libbluray event %d [param %d]",
...
void CDVDInputStreamBluray::OverlayCallback(const BD_OVERLAY* const ov)
{
  ...
  if (ov->img && ov->cmd == BD_OVERLAY_DRAW)
  {
    ...
    uint8_t* img =
        reinterpret_cast<uint8_t*>(malloc(static_cast<size_t>(ov->w) * static_cast<size_t>(ov->h)));
    if (!img)
      return;
...
int64_t CDVDInputStreamBluray::Seek(int64_t offset, int whence)
{
  ...
  if (pos < 0)
  {
    CLog::Log(LOGERROR, "CDVDInputStreamBluray::Seek - seek to %" PRId64 ", failed with %" PRId64,
              offset, pos);
    return -1;
  }

  if (pos != offset)
    CLog::Log(LOGWARNING, "CDVDInputStreamBluray::Seek - seek to %" PRId64 ", ended at %" PRId64,
              offset, pos);
