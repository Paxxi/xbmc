...
CDVDOverlaySpu* CDVDDemuxSPU::AddData(uint8_t* data, int iSize, double pts)
{
  ...
  {
    uint8_t* tmpptr = reinterpret_cast<uint8_t*>(realloc(pSPUData->data, ALIGN(pSPUData->iSize + iSize, 0x4000)));
    if (!tmpptr)
    {
      free(pSPUData->data);
      return nullptr;
...
CDVDOverlaySpu* CDVDDemuxSPU::ParseRLE(CDVDOverlaySpu* pSPU, uint8_t* pUnparsedData)
{
  ...
  for ( i_y = 0 ; i_y < i_height ; i_y++ )
  {
    ...
    for ( i_x = 0 ; i_x < i_width ; i_x += i_code >> 2 )
    {
      ...
      {
        CLog::Log(LOGERROR, "ParseRLE: Overrunning our data range.  Need %li bytes", (reinterpret_cast<uint8_t *>(p_dest) - pSPU->result));
        pSPU->Release();
        return nullptr;
      }
      *p_dest++ = i_code;
