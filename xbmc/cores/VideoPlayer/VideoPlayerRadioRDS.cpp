...
void CDVDRadioRDSData::SetRadioStyle(std::string genre)
{
  ...
  CLog::Log(LOGDEBUG, "Radio UECP (RDS) Processor - %s - Stream genre set to %s", __FUNCTION__,
            genre.c_str());
}
...
unsigned int CDVDRadioRDSData::DecodePI(uint8_t* msgElement)
{
  ...
  if (m_PI_Current != PICode)
  {
    ...
    CLog::Log(
        LOGINFO,
        "Radio UECP (RDS) Processor - PI code changed to Country %X, Type %X and reference no. %i",
...
unsigned int CDVDRadioRDSData::DecodePS(uint8_t* msgElement)
{
  ...
  {
    if (text[i] <= 0xfe)
      m_PS_Text[m_PS_Index][i] =
          (text[i] >= 0x80) ? sRDSAddChar[text[i] - 0x80]
...
unsigned int CDVDRadioRDSData::DecodeDI(uint8_t* msgElement)
{
  ...
  {
    CLog::Log(LOGDEBUG, "Radio UECP (RDS) Processor - %s - Stream changed over to %s", __FUNCTION__,
              value ? "Stereo" : "Mono");
    m_DI_IsStereo = value;
  }
  ...
  {
    CLog::Log(LOGDEBUG,
              "Radio UECP (RDS) Processor - %s - Stream changed over to %sArtificial Head",
  ...
  {
    CLog::Log(LOGDEBUG,
              "Radio UECP (RDS) Processor - %s - Stream changed over to %sCompressed Head",
              __FUNCTION__, value ? "" : "Not ");
    m_DI_ArtificialHead = value;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "Radio UECP (RDS) Processor - %s - Stream changed over to %s PTY",
...
unsigned int CDVDRadioRDSData::DecodeTA_TP(uint8_t* msgElement)
{
      ...
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("pvrplayback.trafficadvisory"))
  {
    ...
    data["on"] = true;
    CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioTA",
                                                       data);
  }
...
unsigned int CDVDRadioRDSData::DecodePTYN(uint8_t* msgElement)
{
  ...
  if (!m_RTPlus_GenrePresent)
  {
    std::string progTypeName = StringUtils::Format(
        "%s: %s", g_localizeStrings.Get(pty_skin_info_table[m_PTY][m_RDS_IsRBDS].name).c_str(),
        m_PTYN);
...
unsigned int CDVDRadioRDSData::DecodeRTC(uint8_t* msgElement)
{
  ...
  CLog::Log(LOGDEBUG,
            "Radio UECP (RDS) - %s - Current RDS Data Time: %02i.%02i.%02i - UTC: "
            "%02i:%02i:%02i,0.%is - Local: %c%i min",
  ...
  CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioRTC",
                                                     data);
...
unsigned int CDVDRadioRDSData::DecodeRTPlus(uint8_t* msgElement, unsigned int len)
{
  ...
  {
    CLog::Log(LOGERROR, "Radio UECP (RDS) - %s - RTp-Error: Length not correct (MEL= %d)",
  ...
  for (int i = 0; i < 2; ++i)
  {
    if (rtp_start[i] + rtp_len[i] + 1 >= RT_MEL) // length-error
    {
      CLog::Log(
          LOGERROR,
          "Radio UECP (RDS) - %s - (tag#%d = Typ/Start/Len): %d/%d/%d (Start+Length > 'RT-MEL' !)",
...
unsigned int CDVDRadioRDSData::DecodeEPPTransmitterInfo(uint8_t* msgElement)
{
  if (!m_RDS_SlowLabelingCodesPresent && m_PI_CountryCode != 0)
  {
    ...
    if (codeLow > 7)
    {
      CLog::Log(LOGERROR, "Radio RDS - %s - invalid country code 0x%02X%02X", __FUNCTION__,
                codeHigh, codeLow);
      return 7;
    }
    ...
    case 0xF0:
      ...
      break;
    default:
      CLog::Log(LOGERROR, "Radio RDS - %s - invalid extended country region code:%02X%02X",
...
unsigned int CDVDRadioRDSData::DecodeSlowLabelingCodes(uint8_t* msgElement)
{
  ...
  case VARCODE_LANGUAGE_CODES: // language codes
    if (slowLabellingCode > 1 && slowLabellingCode < 0x80)
      ...
      CLog::Log(LOGERROR, "Radio RDS - %s - invalid language code %i", __FUNCTION__,
                slowLabellingCode);
    break;
...
void CDVDRadioRDSData::SendTMCSignal(unsigned int flags, uint8_t* data)
{
  ...
  if (m_currentChannel)
  {
    ...
    CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioTMC",
                                                       msg);
  }
}
