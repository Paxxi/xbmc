...
{
public:
  CMMALPool(const char* component_name,
            bool input,
  ...
  virtual int AlignedWidth()
  {
    return m_mmal_format == MMAL_ENCODING_YUVUV128 || m_mmal_format == MMAL_ENCODING_YUVUV64_16 ||
                   m_geo.getBytesPerPixel() == 0
               ? 0
               : m_geo.getStrideY() / m_geo.getBytesPerPixel();
  }
  virtual int AlignedHeight()
  {
    return m_mmal_format == MMAL_ENCODING_YUVUV128 || m_mmal_format == MMAL_ENCODING_YUVUV64_16
               ? 0
               : m_geo.getHeightY();
...
public:
  ...
  virtual void Unref();
  virtual std::shared_ptr<CMMALPool> Pool()
  {
    return std::dynamic_pointer_cast<CMMALPool>(m_pool);
  };
  ...
  const char* GetStateName()
  {
    static const char* names[] = {
        "MMALStateNone", "MMALStateHWDec", "MMALStateFFDec", "MMALStateDeint", "MMALStateBypass",
    };
...
protected:
  ...
  bool CheckConfigurationDeint(uint32_t width,
                               uint32_t height,
  ...
  bool CheckConfigurationVout(uint32_t width,
                              uint32_t height,
                              uint32_t aligned_width,
                              uint32_t aligned_height,
                              uint32_t encoding);
