...
{
  if (codec != right.codec || type != right.type || uniqueId != right.uniqueId ||
      demuxerId != right.demuxerId || codec_tag != right.codec_tag || flags != right.flags)
    return false;
  ...
  if (fpsscale != right.fpsscale || fpsrate != right.fpsrate || height != right.height ||
      ...
      forced_aspect != right.forced_aspect || bitsperpixel != right.bitsperpixel ||
      vfr != right.vfr || colorSpace != right.colorSpace || colorRange != right.colorRange ||
