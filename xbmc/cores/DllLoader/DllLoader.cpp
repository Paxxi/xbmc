...
DllLoader::DllLoader(
    const char* sDll, bool bTrack, bool bSystemDll, bool bLoadSymbols, Export* exps)
  : LibraryLoader(sDll)
{
  ImportDirTable = 0;
  m_pExportHead = NULL;
  ...
  if (fp)
  {
    if (CoffLoader::ParseCoff(fp))
    {
      ...
      else
      {
        ...
        {
          iMinAddr = std::min<uintptr_t>(iMinAddr, (uintptr_t)SectionHeader[i].VirtualAddress);
          iMaxAddr = std::max<uintptr_t>(iMaxAddr, (uintptr_t)(SectionHeader[i].VirtualAddress +
...
void DllLoader::PrintImportTable(ImportDirTable_t* ImportDirTable)
{
  ...
  while (Imp->ImportLookupTable_RVA != 0 || Imp->TimeStamp != 0 || Imp->ForwarderChain != 0 ||
         Imp->Name_RVA != 0 || Imp->ImportAddressTable_RVA != 0)
  {
    char* Name;
    HavePrinted = 1;
...
void DllLoader::PrintExportTable(ExportDirTable_t* ExportDirTable)
{
  ...
  unsigned long* ExportAddressTable =
      (unsigned long*)RVA2Data(ExportDirTable->ExportAddressTable_RVA);
...
{
  DllLoader* pDll =
      static_cast<DllLoader*>(DllLoaderContainer::LoadModule(dll, GetPath(), m_bLoadSymbols));
...
int DllLoader::LoadExports()
{
  if (NumOfDirectories > EXPORT_TABLE && Directory[EXPORT_TABLE].Size > 0)
  {
    ...
    unsigned long* NamePointerTable =
        (unsigned long*)RVA2Data(ExportDirTable->NamePointerTable_RVA);
    unsigned short* OrdinalTable = (unsigned short*)RVA2Data(ExportDirTable->OrdinalTable_RVA);
...
#ifdef LOGALL
  CLog::Log(LOGDEBUG, "Executing EntryPoint with DLL_PROCESS_ATTACH at: 0x%x - Dll: %s",
...
{
#ifdef LOGALL
  CLog::Log(LOGDEBUG, "Executing EntryPoint with DLL_PROCESS_DETACH at: 0x%x - Dll: %s",
...
void DllLoader::LoadSymbols()
{
  ...
  if (pBaseAddress)
  {
    ...
    if (dllxbdm.ParseHeaders(pBaseAddress))
    {
      ...
      {
        CLog::Log(LOGDEBUG,
                  "DllLoader: Unable to load symbols for %s. No offset for xbdm.dll with checksum "
                  "0x%08X found",
      ...
      {
        CLog::Log(LOGDEBUG, "DllLoader: Loading symbols for %s failed with an exception.",
                  GetName());
      }
    }
  }
  else
    CLog::Log(LOGDEBUG, "DllLoader: Can't load symbols for %s. xbdm.dll is needed and not loaded",
              GetName());
...
void DllLoader::UnloadSymbols()
{
  ...
  if (pBaseAddress)
  {
    ...
    if (dllxbdm.ParseHeaders(pBaseAddress))
    {
      ...
      {
        CLog::Log(LOGDEBUG,
                  "DllLoader: Unable to unload symbols for %s. No offset for xbdm.dll with "
                  "checksum 0x%08X found",
                  GetName(), dllxbdm.WindowsHeader->CheckSum);
        return;
      }
      ...
      {
        CLog::Log(LOGDEBUG, "DllLoader: Unloading symbols for %s failed with an exception.",
                  GetName());
      }
    }
  }
  else
    CLog::Log(LOGDEBUG, "DllLoader: Can't unload symbols for %s. xbdm.dll is needed and not loaded",
              GetName());
#endif
}
