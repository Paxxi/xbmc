...
#define __IS_STDIN_STREAM(stream) \
  (stream == stdin || fileno(stream) == fileno(stdin) || fileno(stream) == 0)
#define __IS_STDOUT_STREAM(stream) \
  (stream == stdout || fileno(stream) == fileno(stdout) || fileno(stream) == 1)
#define __IS_STDERR_STREAM(stream) \
  (stream == stderr || fileno(stream) == fileno(stderr) || fileno(stream) == 2)
...
#define IS_STD_DESCRIPTOR(fd) \
  (IS_STDIN_DESCRIPTOR(fd) || IS_STDOUT_DESCRIPTOR(fd) || IS_STDERR_DESCRIPTOR(fd))
