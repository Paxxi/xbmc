...
bool COMXImage::CreateThumbnailFromSurface(unsigned char* buffer,
                                           unsigned int width,
                                           unsigned int height,
                                           unsigned int format,
                                           unsigned int pitch,
                                           const std::string& destFile)
{
  ...
  if (!ret)
    CLog::Log(LOGNOTICE, "%s: unable to create thumbnail %s %dx%d", __func__, destFile.c_str(),
              width, height);
  return ret;
}
...
bool COMXImage::DecodeJpeg(
    COMXImageFile* file, unsigned int width, unsigned int height, unsigned int stride, void* pixels)
{
  bool ret = false;
  ...
  if (omx_image.Decode(file->GetImageBuffer(), file->GetImageSize(), width, height, stride, pixels))
  {
    ...
    CLog::Log(LOGNOTICE, "%s: unable to decode %s %dx%d", __func__, file->GetFilename(), width,
              height);
  omx_image.Close();
  return ret;
}

bool COMXImage::ClampLimits(unsigned int& width,
                            unsigned int& height,
                            unsigned int m_width,
                            unsigned int m_height,
                            bool transposed)
{
  RESOLUTION_INFO& res_info = CDisplaySettings::GetInstance().GetResolutionInfo(
  ...
  if (max_width == 0 || max_height == 0)
  {
    ...
    { // 16x9 images larger than the fanart res use that rather than the image res
      if (fabsf(aspect / (16.0f / 9.0f) - 1.0f) <= 0.01f &&
          m_height >= advancedSettings->m_fanartRes)
      {
...
bool COMXImage::CreateThumb(const std::string& srcFile,
                            ...
                            const std::string& destFile)
{
  ...
  if (URIUtils::HasExtension(srcFile, ".jpg|.tbn") && file.ReadFile(srcFile, orientation) &&
      reenc.ReEncode(file, maxWidth, maxHeight, pDestBuffer, nDestSize))
  {
    XFILE::CFile outfile;
...
bool COMXImage::SendMessage(
    bool (*callback)(EGLDisplay egl_display, EGLContext egl_context, void* cookie), void* cookie)
{
  // we can only call gl functions from the application thread or texture thread
  if (g_application.IsCurrentThread())
  {
    CWinSystemRpiGLESContext* winsystem =
        static_cast<CWinSystemRpiGLESContext*>(CServiceBroker::GetWinSystem());
...
bool COMXImage::AllocTextureInternal(EGLDisplay egl_display,
                                     EGLContext egl_context,
                                     struct textureinfo* tex)
{
  ...
  GLenum type =
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("videoscreen.textures32")
          ? GL_UNSIGNED_BYTE
          : GL_UNSIGNED_SHORT_5_6_5;
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, tex->width, tex->height, 0, GL_RGB, type, 0);
  tex->egl_image = eglCreateImageKHR(egl_display, egl_context, EGL_GL_TEXTURE_2D_KHR,
                                     (EGLClientBuffer)tex->texture, NULL);
  if (!tex->egl_image)
...
bool COMXImage::DestroyTextureInternal(EGLDisplay egl_display,
                                       EGLContext egl_context,
                                       struct textureinfo* tex)
{
  bool s = true;
...
bool COMXImage::DecodeJpegToTexture(COMXImageFile* file,
                                    unsigned int width,
                                    unsigned int height,
                                    void** userdata)
{
  ...
  if (tex->egl_image && tex->texture &&
      omx_image.Decode(file->GetImageBuffer(), file->GetImageSize(), width, height, tex->egl_image))
  {
    ret = true;
    *userdata = tex;
  ...
  {
    CLog::Log(LOGNOTICE, "%s: unable to decode to texture %s %dx%d", __func__, file->GetFilename(),
              width, height);
    DestroyTexture(tex);
  }
  return ret;
...
void COMXImage::CreateContext()
{
  ...
  static const EGLint configAttrs[] = {EGL_RED_SIZE,
                                       8,
                                       EGL_GREEN_SIZE,
                                       8,
                                       EGL_BLUE_SIZE,
  ...
  m_egl_context =
      eglCreateContext(egl_display, egl_config, winsystem->GetEGLContext(), contextAttrs);
  CheckError();
  if (m_egl_context == EGL_NO_CONTEXT)
  {
...
bool COMXImageFile::ReadFile(const std::string& inputFile, int orientation)
{
  ...
  if (eCompressionFormat != OMX_IMAGE_CodingJPEG || m_width < 1 || m_height < 1)
  {
    CLog::Log(LOGDEBUG, "%s::%s %s GetCodingType=0x%x (%dx%d)\n", CLASSNAME, __func__,
              GetFilename(), eCompressionFormat, m_width, m_height);
    return false;
  }
                                           ...
                                           unsigned int resize_stride)
{
  ...
  if (!m_decoded_buffer)
  {
    ...
    m_omx_tunnel_decode.Initialize(&m_omx_decoder, m_omx_decoder.GetOutputPort(), &m_omx_resize,
                                   m_omx_resize.GetInputPort());
...
bool COMXImageDec::Decode(const uint8_t* demuxer_content,
                          unsigned demuxer_bytes,
                          unsigned width,
                          unsigned height,
                          unsigned stride,
                          void* pixels)
{
  ...
  portParam.nBufferSize =
      std::max(portParam.nBufferSize, ALIGN_UP(demuxer_bytes, portParam.nBufferAlignment));
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s m_omx_decoder.AllocInputBuffers result(0x%x)", CLASSNAME, __func__,
              omx_err);
    return false;
  }
  ...
  while (demuxer_bytes > 0 || !m_decoded_buffer)
  {
    ...
    if (demuxer_bytes)
    {
      ...
      omx_buffer->nFilledLen =
          (demuxer_bytes > omx_buffer->nAllocLen) ? omx_buffer->nAllocLen : demuxer_bytes;
...
bool COMXImageEnc::Encode(
    unsigned char* buffer, int size, unsigned width, unsigned height, unsigned int pitch)
{
  CSingleLock lock(m_OMXSection);
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s m_omx_encoder.AllocInputBuffers result(0x%x)", CLASSNAME, __func__,
              omx_err);
    return false;
  }
                                             ...
                                             bool port_settings_changed)
{
  ...
  if (!port_settings_changed)
  {
    ...
    if (resize_width != port_def.format.image.nFrameWidth ||
        resize_height != port_def.format.image.nFrameHeight || (orientation & 4))
    {
      if (!m_omx_resize.Initialize("OMX.broadcom.resize", OMX_IndexParamImageInit))
      {
        CLog::Log(LOGERROR, "%s::%s error m_omx_resize.Initialize\n", CLASSNAME, __func__);
    ...
    if (m_omx_resize.IsInitialized())
    {
      ...
      m_omx_tunnel_resize.Initialize(&m_omx_resize, m_omx_resize.GetOutputPort(), &m_omx_encoder,
                                     m_omx_encoder.GetInputPort());
    ...
    {
      m_omx_tunnel_decode.Initialize(&m_omx_decoder, m_omx_decoder.GetOutputPort(), &m_omx_encoder,
                                     m_omx_encoder.GetInputPort());
                              ...
                              unsigned int& nDestSize)
{
  ...
  COMXImage::ClampLimits(maxWidth, maxHeight, srcFile.GetWidth(), srcFile.GetHeight(),
                         srcFile.GetOrientation() & 4);
  ...
  {
    CLog::Log(LOGERROR,
              "%s::%s %s error GetParameter:OMX_IndexParamPortDefinition omx_err(0x%08x)\n",
  ...
  {
    CLog::Log(LOGERROR,
              "%s::%s %s error SetParameter:OMX_IndexParamPortDefinition omx_err(0x%08x)\n",
              CLASSNAME, __func__, srcFile.GetFilename(), omx_err);
    return false;
  }
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s %s m_omx_decoder.AllocInputBuffers result(0x%x)", CLASSNAME,
  ...
  while (demuxer_bytes > 0 || !port_settings_changed || !eos)
  {
    ...
    if (omx_err == OMX_ErrorNone)
    {
      if (!HandlePortSettingChange(maxWidth, maxHeight, srcFile.GetOrientation(),
    ...
    {
      CLog::Log(LOGERROR, "%s::%s %s - image not supported", CLASSNAME, __func__,
                srcFile.GetFilename());
      return false;
    }
    else if (timeout || omx_err != OMX_ErrorTimeout)
    ...
    if (m_encoded_buffer)
    {
      ...
      memcpy((char*)m_pDestBuffer + nDestSize, m_encoded_buffer->pBuffer,
             m_encoded_buffer->nFilledLen);
                                          ...
                                          bool port_settings_changed)
{
  ...
  if (resize_width != port_def.format.image.nFrameWidth ||
      resize_height != port_def.format.image.nFrameHeight)
  {
    if (!m_omx_resize.Initialize("OMX.broadcom.resize", OMX_IndexParamImageInit))
    {
  ...
  omx_err = m_omx_egl_render.UseEGLImage(&m_egl_buffer, m_omx_egl_render.GetOutputPort(), NULL,
                                         egl_image);
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s error m_omx_egl_render.UseEGLImage (%x)", CLASSNAME, __func__,
              omx_err);
    return false;
  }
  if (m_omx_resize.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "%s::%s m_omx_tunnel_decode.Establish (%x)", CLASSNAME, __func__,
                omx_err);
      return false;
    }

    m_omx_tunnel_egl.Initialize(&m_omx_resize, m_omx_resize.GetOutputPort(), &m_omx_egl_render,
                                m_omx_egl_render.GetInputPort());
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "%s::%s error m_omx_egl_render.GetParameter (%x)", CLASSNAME, __func__,
                omx_err);
      return false;
    }
  }
  ...
  {
    m_omx_tunnel_decode.Initialize(&m_omx_decoder, m_omx_decoder.GetOutputPort(), &m_omx_egl_render,
                                   m_omx_egl_render.GetInputPort());
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s error m_omx_egl_render.SetStateForComponent (%x)", CLASSNAME,
              __func__, omx_err);
    return false;
  }
...
bool COMXTexture::Decode(const uint8_t* demuxer_content,
                         unsigned demuxer_bytes,
                         unsigned int width,
                         unsigned int height,
                         void* egl_image)
{
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s error m_omx_sched.SetStateForComponent (%x)", CLASSNAME, __func__,
              omx_err);
    return false;
  }
  ...
  while (demuxer_bytes > 0 || !port_settings_changed || !eos)
  {
    ...
    if (demuxer_bytes)
    {
      ...
      if (omx_buffer)
      {
        ...
        if (omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR, "%s::%s - m_omx_decoder.OMX_EmptyThisBuffer (%x)", CLASSNAME,
                    __func__, omx_err);
          m_omx_decoder.DecoderEmptyBufferDone(m_omx_decoder.GetComponent(), omx_buffer);
          return false;
        }
    ...
    if (omx_err == OMX_ErrorNone)
    {
      ...
      {
        CLog::Log(LOGERROR, "%s::%s - HandlePortSettingChange failed (%x)", CLASSNAME, __func__,
                  omx_err);
        return false;
      }
      port_settings_changed = true;
    ...
    if (port_settings_changed && m_egl_buffer && demuxer_bytes == 0 && !eos)
    {
      ...
      if (omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "%s::%s error m_omx_egl_render.FillThisBuffer (%x)", CLASSNAME,
                  __func__, omx_err);
