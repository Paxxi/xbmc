...
void CEngineStats::UpdateStream(CActiveAEStream* stream)
{
  ...
  for (auto& str : m_streamStats)
  {
    if (str.m_streamId == stream->m_id)
    {
      ...
      for (itBuf = stream->m_processingSamples.begin(); itBuf != stream->m_processingSamples.end();
           ++itBuf)
      {
        if (m_pcmOutput)
    ...
    case AE_TOP: // TOP
      if (port == &m_controlPort)
      {
        ...
        case CActiveAEControlProtocol::VOLUME:
          ...
          if (m_sinkHasVolume)
            m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::VOLUME, &m_volume,
                                                sizeof(float));
          return;
        ...
        case CActiveAEControlProtocol::APPFOCUSED:
          m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::APPFOCUSED, msg->data,
                                              sizeof(bool));
          return;
      ...
      }
      {
        ...
        CLog::Log(LOGWARNING, "CActiveAE::%s - signal: %d from port: %s not handled for state: %d",
    ...
    case AE_TOP_CONFIGURED:
      if (port == &m_controlPort)
      {
        ...
        case CActiveAEControlProtocol::RECONFIGURE:
        {
          ...
          LoadSettings();
          m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::SETNOISETYPE,
                                              &m_settings.streamNoise, sizeof(bool));
          m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::SETSILENCETIMEOUT,
                                              &m_settings.silenceTimeout, sizeof(int));
          ChangeResamplers();
        ...
        case CActiveAEControlProtocol::DISPLAYLOST:
          if (m_sink.GetDeviceType(m_mode == MODE_PCM
                                       ? m_settings.device
                                       : m_settings.passthroughdevice) == AE_DEVTYPE_HDMI)
          {
            UnconfigureSink();
            m_stats.SetSuspended(true);
            m_state = AE_TOP_CONFIGURED_SUSPEND;
        ...
        case CActiveAEControlProtocol::DEVICECHANGE:
          ...
          {
            CLog::Log(LOGWARNING, "CActiveAE - received %ld device change events within one second",
...
void CActiveAE::Configure(AEAudioFormat* desiredFmt)
{
  ...
  std::string device = (m_sinkRequestFormat.m_dataFormat == AE_FMT_RAW)
  ...
  if ((!CompareFormat(m_sinkRequestFormat, m_sinkFormat) &&
      ...
      m_currDevice.compare(device) != 0 || m_settings.driver.compare(driver) != 0)
  {
    FlushEngine();
    if (!InitSink())
      return;
  ...
  else
  {
    ...
    else
    {
      ...
      outputFormat.m_dataFormat =
          AE_IS_PLANAR(outputFormat.m_dataFormat) ? AE_FMT_FLOATP : AE_FMT_FLOAT;
    ...
    for (it = m_streams.begin(); it != m_streams.end(); ++it)
    {
      if (!(*it)->m_inputBuffers)
      {
        ...
        (*it)->m_format.m_frames =
            ...
            ((float)(*it)->m_format.m_sampleRate / m_internalFormat.m_sampleRate);
      ...
      {
        (*it)->m_processingBuffers = new CActiveAEStreamBuffers(
  ...
  {
    m_sinkBuffers =
        new CActiveAEBufferPoolResample(sinkInputFormat, m_sinkFormat, m_settings.resampleQuality);
  ...
  if (!CompareFormat(oldInternalFormat, m_internalFormat))
  {
    if (m_settings.guisoundmode == AE_SOUND_ALWAYS ||
        (m_settings.guisoundmode == AE_SOUND_IDLE && m_streams.empty()) || m_aeGUISoundForce)
    {
...
CActiveAEStream* CActiveAE::CreateStream(MsgStreamNew* streamMsg)
{
  ...
  stream->m_streamPort = new CActiveAEDataProtocol("stream", &stream->m_inMsgEvent, &m_outMsgEvent);
...
void CActiveAE::FlushEngine()
{
  ...
  Message* reply;
  if (m_sink.m_controlPort.SendOutMessageSync(CSinkControlProtocol::FLUSH, &reply, 2000))
  {
...
void CActiveAE::ApplySettingsToFormat(AEAudioFormat& format, AudioSettings& settings, int* mode)
{
  ...
  // transcode
           ...
           settings.passthrough && settings.ac3passthrough && settings.ac3transcode &&
           !m_streams.empty() && (format.m_channelLayout.Count() > 2 || settings.stereoupmix))
  {
    format.m_dataFormat = AE_FMT_RAW;
    format.m_sampleRate = 48000;
    format.m_channelLayout = AE_CH_LAYOUT_2_0;
  ...
  else
  {
        ...
        (settings.config == AE_CONFIG_FIXED))
    {
      ...
      case 1:
        ...
        break;
      case 2:
        stdLayout = AE_CH_LAYOUT_2_1;
        break;
      case 3:
        stdLayout = AE_CH_LAYOUT_3_0;
        break;
      case 4:
        stdLayout = AE_CH_LAYOUT_3_1;
        break;
      case 5:
        stdLayout = AE_CH_LAYOUT_4_0;
        break;
      case 6:
        stdLayout = AE_CH_LAYOUT_4_1;
        break;
      case 7:
        stdLayout = AE_CH_LAYOUT_5_0;
        break;
      case 8:
        stdLayout = AE_CH_LAYOUT_5_1;
        break;
      case 9:
        stdLayout = AE_CH_LAYOUT_7_0;
        break;
      case 10:
        stdLayout = AE_CH_LAYOUT_7_1;
        break;
      }
    ...
    else if ((settings.config == AE_CONFIG_AUTO) && m_stats.GetWaterLevel() > 0 &&
             m_internalFormat.m_channelLayout.Count() > 2)
    {
      format.m_channelLayout = m_internalFormat.m_channelLayout;
    }
    ...
    if (m_sink.GetDeviceType(m_settings.device) == AE_DEVTYPE_IEC958)
    {
      if (format.m_sampleRate > m_settings.samplerate)
      {
        ...
        CLog::Log(LOGINFO, "CActiveAE::ApplySettings - limit samplerate for SPDIF to %d",
                  format.m_sampleRate);
      }
      format.m_channelLayout = AE_CH_LAYOUT_2_0;
    }
    ...
    if (m_settings.config == AE_CONFIG_FIXED)
    {
      ...
      CLog::Log(LOGINFO, "CActiveAE::ApplySettings - Forcing samplerate to %d",
                format.m_sampleRate);
    }
...
bool CActiveAE::NeedReconfigureSink()
{
  ...
  return !CompareFormat(newFormat, m_sinkFormat) || m_currDevice.compare(device) != 0 ||
...
bool CActiveAE::InitSink()
{
  ...
  // send message to sink
  m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::SETNOISETYPE, &m_settings.streamNoise,
                                      sizeof(bool));
  m_sink.m_controlPort.SendOutMessage(CSinkControlProtocol::SETSILENCETIMEOUT,
                                      &m_settings.silenceTimeout, sizeof(int));

  Message* reply;
  if (m_sink.m_controlPort.SendOutMessageSync(CSinkControlProtocol::CONFIGURE, &reply, 5000,
...
void CActiveAE::DrainSink()
{
  // send message to sink
  Message* reply;
  if (m_sink.m_dataPort.SendOutMessageSync(CSinkDataProtocol::DRAIN, &reply, 2000))
  {
...
void CActiveAE::UnconfigureSink()
{
  // send message to sink
  Message* reply;
  if (m_sink.m_controlPort.SendOutMessageSync(CSinkControlProtocol::UNCONFIGURE, &reply, 2000))
  {
    bool success = reply->signal == CSinkControlProtocol::ACC;
    if (!success)
    {
...
bool CActiveAE::RunStages()
{
  ...
  for (it = m_streams.begin(); it != m_streams.end(); ++it)
  {
    ...
    if ((*it)->m_streamIsBuffering && (*it)->m_processingBuffers &&
    ...
    if (!(*it)->m_drain)
    {
      float buftime = (float)(*it)->m_inputBuffers->m_format.m_frames /
             ...
             !(*it)->m_inputBuffers->m_freeSamples.empty())
      {
        ...
        (*it)->m_streamPort->SendInMessage(CActiveAEDataProtocol::STREAMBUFFER, &buffer,
                                           sizeof(CSampleBuffer*));
        (*it)->IncFreeBuffers();
        time += buftime;
      }
    }
    ...
    {
      if ((*it)->m_processingBuffers->IsDrained() && (*it)->m_processingSamples.empty())
      {
  ...
  while (!m_sinkBuffers->m_outputSamples.empty())
  {
    ...
    m_sink.m_dataPort.SendOutMessage(CSinkDataProtocol::SAMPLE, &out, sizeof(CSampleBuffer*));
    busy = true;
  }
...
CSampleBuffer* CActiveAE::SyncStream(CActiveAEStream* stream)
{
  ...
  int timeout = (stream->m_syncState != CAESyncInfo::AESyncState::SYNC_INSYNC)
                    ? 100
                    : stream->GetErrorInterval();
  bool newerror = stream->m_syncError.Get(error, timeout);

  if (newerror && fabs(error) > threshold &&
  ...
  else if (stream->m_syncState == CAESyncInfo::AESyncState::SYNC_ADJUST)
  {
    ...
    else
    {
      ...
      {
        int bytesToSkip = framesToSkip * buf->pkt->bytes_per_sample * buf->pkt->config.channels /
                          buf->pkt->planes;
        for (int i = 0; i < buf->pkt->planes; i++)
        {
          memmove(buf->pkt->data[i], buf->pkt->data[i] + bytesToSkip,
                  buf->pkt->linesize - bytesToSkip);
        }
    ...
    if (fabs(error) < 30)
    {
      ...
      else
      {
        ...
        stream->m_resampleIntegral = 0;
        stream->m_processingBuffers->SetRR(1.0, m_settings.atempoThreshold);
        CLog::Log(LOGDEBUG, "ActiveAE::SyncStream - average error %f below threshold of %f", error,
                  30.0);
      }
    }
...
void CActiveAE::MixSounds(CSoundPacket& dstSample)
{
  ...
  for (it = m_sounds_playing.begin(); it != m_sounds_playing.end();)
  {
    ...
    int start = it->samples_played *
                ...
                it->sound->GetSound(false)->config.channels / it->sound->GetSound(false)->planes;
...
void CActiveAE::LoadSettings()
{
  ...
  m_settings.channels = (m_sink.GetDeviceType(m_settings.device) == AE_DEVTYPE_IEC958)
                            ? AE_CH_LAYOUT_2_0
  ...
  m_settings.stereoupmix = IsSettingVisible(CSettings::SETTING_AUDIOOUTPUT_STEREOUPMIX)
                               ? settings->GetBool(CSettings::SETTING_AUDIOOUTPUT_STEREOUPMIX)
                               : false;
  m_settings.normalizelevels =
  ...
  m_settings.passthrough = m_settings.config == AE_CONFIG_FIXED
                               ? false
  ...
  m_settings.resampleQuality =
      static_cast<AEQuality>(settings->GetInt(CSettings::SETTING_AUDIOOUTPUT_PROCESSQUALITY));
  m_settings.atempoThreshold =
      settings->GetInt(CSettings::SETTING_AUDIOOUTPUT_ATEMPOTHRESHOLD) / 100.0;
  m_settings.streamNoise = settings->GetBool(CSettings::SETTING_AUDIOOUTPUT_STREAMNOISE);
  m_settings.silenceTimeout =
      settings->GetInt(CSettings::SETTING_AUDIOOUTPUT_STREAMSILENCE) * 60000;
}
...
void CActiveAE::Start()
{
  Create();
  Message* reply;
  if (m_controlPort.SendOutMessageSync(CActiveAEControlProtocol::INIT, &reply, 10000))
  {
...
bool CActiveAE::SupportsRaw(AEAudioFormat& format)
{
  ...
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_AC3 && !m_settings.ac3passthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTS_512 &&
      !m_settings.dtspassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTS_1024 &&
      !m_settings.dtspassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTS_2048 &&
      !m_settings.dtspassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTSHD_CORE &&
      !m_settings.dtspassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_EAC3 && !m_settings.eac3passthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_TRUEHD &&
      !m_settings.truehdpassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTSHD &&
      !m_settings.dtshdpassthrough)
    return false;
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTSHD_MA &&
      !m_settings.dtshdpassthrough)
    return false;

  if (!m_sink.SupportsFormat(CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
...
bool CActiveAE::HasStereoAudioChannelCount()
{
  ...
  int numChannels = (m_sink.GetDeviceType(device) == AE_DEVTYPE_IEC958)
                        ? AE_CH_LAYOUT_2_0
                        : settings->GetInt(CSettings::SETTING_AUDIOOUTPUT_CHANNELS);
  bool passthrough = settings->GetInt(CSettings::SETTING_AUDIOOUTPUT_CONFIG) == AE_CONFIG_FIXED
                         ? false
                         : settings->GetBool(CSettings::SETTING_AUDIOOUTPUT_PASSTHROUGH);
...
bool CActiveAE::IsSettingVisible(const std::string& settingId)
{
  if (settingId == CSettings::SETTING_AUDIOOUTPUT_SAMPLERATE)
  {
    ...
    if (m_sink.GetDeviceType(settings->GetString(CSettings::SETTING_AUDIOOUTPUT_AUDIODEVICE)) ==
        AE_DEVTYPE_IEC958)
      return true;
  ...
  {
    if (m_sink.GetDeviceType(CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
            CSettings::SETTING_AUDIOOUTPUT_AUDIODEVICE)) != AE_DEVTYPE_IEC958)
      return true;
  }
  ...
  else if (settingId == CSettings::SETTING_AUDIOOUTPUT_DTSPASSTHROUGH)
  {
    ...
    const std::shared_ptr<CSettings> settings =
        CServiceBroker::GetSettingsComponent()->GetSettings();
    if (m_sink.SupportsFormat(settings->GetString(CSettings::SETTING_AUDIOOUTPUT_PASSTHROUGHDEVICE),
                              format) &&
  ...
  else if (settingId == CSettings::SETTING_AUDIOOUTPUT_STEREOUPMIX)
  {
    if (m_sink.HasPassthroughDevice() ||
        CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
            CSettings::SETTING_AUDIOOUTPUT_CHANNELS) > AE_CH_LAYOUT_2_0)
      return true;
  }
...
{
  Message* reply;
  if (m_controlPort.SendOutMessageSync(CActiveAEControlProtocol::INIT, &reply, 5000))
  {
    bool success = reply->signal == CActiveAEControlProtocol::ACC;
    reply->Release();
    if (!success)
...
{
  Message* reply;
  if (m_controlPort.SendOutMessageSync(CActiveAEControlProtocol::DISPLAYLOST, &reply, 5000))
  {
    bool success = reply->signal == CActiveAEControlProtocol::ACC;
    reply->Release();
    if (!success)
...
uint8_t** CActiveAE::AllocSoundSample(
    SampleConfig& config, int& samples, int& bytes_per_sample, int& planes, int& linesize)
{
  uint8_t** buffer;
  ...
  av_samples_alloc(buffer, &linesize, config.channels, samples, config.fmt, 16);
...
{
  if (lhs.m_channelLayout != rhs.m_channelLayout || lhs.m_dataFormat != rhs.m_dataFormat ||
...
IAESound* CActiveAE::MakeSound(const std::string& file)
{
  ...
  io_ctx = avio_alloc_context(buffer, bufferSize, 0, sound, CActiveAESound::Read, NULL,
                              CActiveAESound::Seek);
...
{
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STOPSOUND, &sound,
                               sizeof(CActiveAESound*));
}
...
bool CActiveAE::ResampleSound(CActiveAESound* sound)
{
  ...
  if (sound->GetSound(true)->config.channels == 1 && testChannel != AE_CH_NULL)
  {
    ...
    {
      if (m_internalFormat.m_channelLayout[out] == AE_CH_FC &&
          testChannel != AE_CH_FC) /// To become center clear on position test ??????
  ...
  dst_samples = resampler->CalcDstSampleCount(
      sound->GetSound(true)->nb_samples, m_internalFormat.m_sampleRate, orig_config.sample_rate);
  ...
  int samples = resampler->Resample(dst_buffer, dst_samples, sound->GetSound(true)->data,
                                    sound->GetSound(true)->nb_samples, 1.0);
...
IAEStream* CActiveAE::MakeStream(AEAudioFormat& audioFormat,
                                 ...
                                 IAEClockCallback* clock)
{
  ...
  Message* reply;
  if (m_dataPort.SendOutMessageSync(CActiveAEDataProtocol::NEWSTREAM, &reply, 10000, &msg,
                                    sizeof(MsgStreamNew)))
  {
    bool success = reply->signal == CActiveAEControlProtocol::ACC;
    if (success)
...
bool CActiveAE::FreeStream(IAEStream* stream, bool finish)
{
  ...
  Message* reply;
  if (m_dataPort.SendOutMessageSync(CActiveAEDataProtocol::FREESTREAM, &reply, 1000, &msg,
...
{
  Message* reply;
  if (m_controlPort.SendOutMessageSync(CActiveAEControlProtocol::FLUSHSTREAM, &reply, 1000, &stream,
...
void CActiveAE::PauseStream(CActiveAEStream* stream, bool pause)
{
  ...
  if (pause)
    m_controlPort.SendOutMessage(CActiveAEControlProtocol::PAUSESTREAM, &stream,
                                 sizeof(CActiveAEStream*));
  else
    m_controlPort.SendOutMessage(CActiveAEControlProtocol::RESUMESTREAM, &stream,
                                 sizeof(CActiveAEStream*));
}
...
void CActiveAE::SetStreamAmplification(CActiveAEStream* stream, float amplify)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMAMP, &msg,
                               sizeof(MsgStreamParameter));
}
...
void CActiveAE::SetStreamReplaygain(CActiveAEStream* stream, float rgain)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMRGAIN, &msg,
                               sizeof(MsgStreamParameter));
}
...
void CActiveAE::SetStreamVolume(CActiveAEStream* stream, float volume)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMVOLUME, &msg,
                               sizeof(MsgStreamParameter));
}
...
void CActiveAE::SetStreamResampleRatio(CActiveAEStream* stream, double ratio)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMRESAMPLERATIO, &msg,
                               sizeof(MsgStreamParameter));
}
...
void CActiveAE::SetStreamResampleMode(CActiveAEStream* stream, int mode)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMRESAMPLEMODE, &msg,
                               sizeof(MsgStreamParameter));
}
                                    ...
                                    enum AVAudioServiceType audio_service_type)
{
  ...
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMFFMPEGINFO, &msg,
                               sizeof(MsgStreamFFmpegInfo));
}
                              ...
                              unsigned int millis)
{
  ...
  msg.target = target;
  msg.millis = millis;
  m_controlPort.SendOutMessage(CActiveAEControlProtocol::STREAMFADE, &msg, sizeof(MsgStreamFade));
}
