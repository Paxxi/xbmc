...
#define LOGTIME(n) \
  ...
  CLog::Log(LOGNOTICE, "ActiveAE::%s %d - resample %s took %.0fms", __FUNCTION__, n, _filename, \
            (End - Start) * 1e-6); \
  Start = End;
#else
#define LOGTIMEINIT(f)
                               ...
                               bool force_resample)
{
  ...
  CLog::Log(LOGINFO,
            ...
            "norm:%d upmix:%d",
            CLASSNAME, __func__, static_cast<void*>(remapLayout), srcConfig.channels,
            dstConfig.channels, srcConfig.sample_rate, dstConfig.sample_rate, srcConfig.fmt,
            dstConfig.fmt, srcConfig.bits_per_sample, dstConfig.bits_per_sample,
            srcConfig.dither_bits, dstConfig.dither_bits, normalize, upmix);
  ...
  if ((m_dst_fmt == AV_SAMPLE_FMT_S32 || m_dst_fmt == AV_SAMPLE_FMT_S32P) && m_dst_bits == 24 &&
      m_dst_dither_bits == -8)
    m_dst_pitch = 24;
  ...
  // this code is just uses ffmpeg to produce the 8x8 mixing matrix
  {
    ...
    SwrContext* m_pContext =
        swr_alloc_set_opts(NULL, m_dst_chan_layout, AV_SAMPLE_FMT_FLT, 48000, m_src_chan_layout,
                           AV_SAMPLE_FMT_FLT, 48000, 0, NULL);
    if (!m_pContext)
    {
  ...
  if (!m_omx_mixer.Initialize("OMX.broadcom.audio_mixer", OMX_IndexParamAudioInit))
    CLog::Log(LOGERROR, "%s::%s - m_omx_mixer.Initialize omx_err(0x%08x)", CLASSNAME, __func__,
              omx_err);
  ...
  if (m_force_resample)
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(
          LOGERROR,
          "%s::%s - error m_omx_mixer Failed to set OMX_IndexParamBrcmTimeScale omx_err(0x%08x)",
  ...
  if (omx_err != OMX_ErrorNone)
    CLog::Log(LOGERROR, "%s::%s - error m_omx_mixer in SetParameter omx_err(0x%08x)", CLASSNAME,
              __func__, omx_err);
  ...
  if (omx_err != OMX_ErrorNone)
    CLog::Log(LOGERROR, "%s::%s - error m_omx_mixer out SetParameter omx_err(0x%08x)", CLASSNAME,
              __func__, omx_err);
  ...
  port_param.nBufferCountActual =
      std::max((unsigned int)port_param.nBufferCountMin, (unsigned int)1);
  ...
  if (omx_err != OMX_ErrorNone)
    CLog::Log(LOGERROR, "%s:%s - m_omx_mixer OMX_StateExecuting omx_err(0x%08x)", CLASSNAME,
              __func__, omx_err);
...
static void copy_planes(uint8_t** dst_buffer,
                        int d_pitch,
                        int d_planes,
                        int d_samplesize,
                        int offset,
                        ...
                        int planesize)
{
  for (int i = 0; i < d_planes; i++)
    memcpy(dst_buffer[i] + offset * d_pitch, src_buffer + i * planesize,
           src_samples * d_samplesize / d_planes);
}

int CActiveAEResamplePi::Resample(
    uint8_t** dst_buffer, int dst_samples, uint8_t** src_buffer, int src_samples, double ratio)
{
#ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "%s::%s samples:%d->%d (%.2f)", CLASSNAME, __func__, src_samples, dst_samples,
            ratio);
#endif
  if (!m_Initialized)
    return 0;
  ...
  const int max_dst_samples =
      (long long)(BUFFERSIZE / d_samplesize) * m_src_rate / (m_dst_rate + m_src_rate - 1);
  ...
  while (1)
  {
    if (m_encoded_buffer && m_encoded_buffer->nFilledLen)
    {
      ...
      copy_planes(dst_buffer, d_pitch, d_planes, d_samplesize, received,
                  (uint8_t*)m_encoded_buffer->pBuffer + m_offset * d_pitch, samples,
                  m_encoded_buffer->nFilledLen / d_planes);
      received += samples;
      m_offset += samples;
      ...
      else if (m_offset > m_encoded_buffer->nFilledLen / d_samplesize)
        assert(0);
      else
        assert(sent == src_samples);
    }
    ...
    if (omx_buffer == NULL)
    {
      CLog::Log(LOGERROR, "%s::%s m_omx_mixer.GetInputBuffer failed to get buffer", CLASSNAME,
                __func__);
      return false;
    }
    ...
    if (omx_buffer->nFilledLen)
    {
      ...
      for (int i = 0; i < s_planes; i++)
        memcpy((uint8_t*)omx_buffer->pBuffer + i * planesize, src_buffer[i] + sent * s_pitch,
               planesize);
      sent += send;
    }
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "%s::%s m_omx_mixer.FillThisBuffer result(0x%x)", CLASSNAME, __func__,
                omx_err);
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "%s::%s m_omx_mixer.WaitForOutputDone result(0x%x)", CLASSNAME, __func__,
                omx_err);
      return false;
    }
    assert(m_encoded_buffer->nFilledLen > 0 &&
           m_encoded_buffer->nFilledLen <= m_encoded_buffer->nAllocLen);
...
#ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "%s::%s format:%d->%d rate:%d->%d chan:%d->%d samples %d->%d (%f) %d",
            CLASSNAME, __func__, (int)m_src_fmt, (int)m_dst_fmt, m_src_rate, m_dst_rate,
            m_src_channels, m_dst_channels, src_samples, dst_samples, ratio, received);
#endif
