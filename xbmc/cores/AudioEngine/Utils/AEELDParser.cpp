...
void CAEELDParser::Parse(const uint8_t* data, size_t length, CAEDeviceInfo& info)
{
  ...
  {
  case ELD_CONN_TYPE_HDMI:
    info.m_deviceType = AE_DEVTYPE_HDMI;
    break;
  case ELD_CONN_TYPE_DP:
    info.m_deviceType = AE_DEVTYPE_DP;
    break;
  }
  ...
  for (uint8_t i = 0; i < header.sad_count; ++i)
  {
    ...
    {
    //! @todo implement
    case CEA_861_FORMAT_AC3:
      fmt = AE_FMT_RAW;
      break;
    case CEA_861_FORMAT_DTS:
      fmt = AE_FMT_RAW;
      break;
    case CEA_861_FORMAT_DTSHD:
      fmt = AE_FMT_RAW;
      break;
    case CEA_861_FORMAT_EAC3:
      fmt = AE_FMT_RAW;
      break;
    case CEA_861_FORMAT_LPCM:
      fmt = AE_FMT_RAW;
      break;
    case CEA_861_FORMAT_MLP:
      fmt = AE_FMT_RAW;
      break;
    }
    ...
    if (std::find(info.m_dataFormats.begin(), info.m_dataFormats.end(), fmt) ==
