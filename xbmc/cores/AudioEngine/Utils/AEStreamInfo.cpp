                                 ...
                                 bool wantEAC3dependent)
{
  ...
  if (bsid <= 10)
  {
    ...
    CLog::Log(LOGINFO, "CAEStreamParser::TrySyncAC3 - AC3 stream detected (%d channels, %dHz)",
  ...
  else
  {
    ...
    {
      CLog::Log(LOGDEBUG,
                "CAEStreamParser::TrySyncAC3 - Unexpected stream type: %d (wantEAC3dependent: %d)",
    ...
    CLog::Log(LOGINFO, "CAEStreamParser::TrySyncAC3 - E-AC3 stream detected (%d channels, %dHz)",
              m_info.m_channels, m_info.m_sampleRate);
    return true;
  }
}
...
unsigned int CAEStreamParser::SyncDTS(uint8_t* data, unsigned int size)
{
  ...
  for (; size - skip > 13; ++skip, ++data)
  {
        ...
        dtsBlocks != m_dtsBlocks)
    {
      ...
      CLog::Log(LOGINFO,
                "CAEStreamParser::SyncDTS - %s stream detected (%d channels, %dHz, %dbit %s, "
                "period: %u, syncword: 0x%x, target rate: 0x%x, framesize %u))",
...
unsigned int CAEStreamParser::SyncTrueHD(uint8_t* data, unsigned int size)
{
  ...
  for (; left; ++skip, ++data, --left)
  {
    ...
    if (syncword == 0xf8726fba)
    {
      ...
      if (!m_hasSync)
        CLog::Log(LOGINFO,
                  "CAEStreamParser::SyncTrueHD - TrueHD stream detected (%d channels, %dHz)",
                  m_info.m_channels, m_info.m_sampleRate);
