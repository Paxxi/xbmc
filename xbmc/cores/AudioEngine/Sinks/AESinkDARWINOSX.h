...
private:
  static OSStatus renderCallback(AudioDeviceID inDevice,
  ...
  bool
      m_outputBitstream; ///< true if we're bistreaming into a LinearPCM stream rather than AC3 stream.
  ...
  volatile bool
      m_started; // set once we get a callback from CoreAudio, which can take a little while.
