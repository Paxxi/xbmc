...
snd_pcm_chmap_t* CAESinkALSA::AEChannelMapToALSAchmap(const CAEChannelInfo& info)
{
  ...
  snd_pcm_chmap_t* alsaMap = reinterpret_cast<snd_pcm_chmap_t*>(malloc(sizeof(snd_pcm_chmap_t) + AECount * sizeof(int)));
...
{
  snd_pcm_chmap_t* copyMap = reinterpret_cast<snd_pcm_chmap_t*>(malloc(sizeof(snd_pcm_chmap_t) + alsaMap->channels * sizeof(int)));
...
AEDeviceType CAESinkALSA::AEDeviceTypeFromName(const std::string &name)
{
  ...
  if (name.substr(0, 6) == "iec958" || name.substr(0, 5) == "spdif")
