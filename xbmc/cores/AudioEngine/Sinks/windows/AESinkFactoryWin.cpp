                                                  ...
                                                  WAVEFORMATEXTENSIBLE& wfxex)
{
  ...
  if (format.m_dataFormat != AE_FMT_RAW) // PCM data
  {
    ...
    wfxex.SubFormat = format.m_dataFormat < AE_FMT_FLOAT ? KSDATAFORMAT_SUBTYPE_PCM
  ...
  else //Raw bitstream
  {
         ...
         (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_DTS_512)))
    {
      ...
      wfxex.dwChannelMask = bool(format.m_channelLayout.Count() == 2) ? KSAUDIO_SPEAKER_STEREO
