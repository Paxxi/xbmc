...
static const unsigned int PassthroughSampleRates[] = {8000,  11025, 16000, 22050, 24000,  32000,
...
void CAESinkPi::SetAudioDest()
{
  ...
  if (m_omx_render.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - m_omx_render.SetConfig omx_err(0x%08x)", CLASSNAME, __func__,
                omx_err);
  }
  if (m_omx_render_slave.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - m_omx_render_slave.SetConfig omx_err(0x%08x)", CLASSNAME,
                __func__, omx_err);
  }
}
...
static void SetAudioProps(bool stream_channels, uint32_t channel_map)
{
  ...
  CLog::Log(LOGDEBUG, "%s:%s hdmi_stream_channels %d hdmi_channel_map %08x", CLASSNAME, __func__,
            stream_channels, channel_map);
}
...
static uint32_t GetChannelMap(const CAEChannelInfo& channelLayout, bool passthrough)
{
  ...
  if (cea == 0xff)
    CLog::Log(LOGERROR, "%s::%s - Unexpected CEA mapping %d,%d", CLASSNAME, __func__,
              channelLayout.HasChannel(AE_CH_LFE), channels);
...
bool CAESinkPi::Initialize(AEAudioFormat& format, std::string& device)
{
  ...
  const std::string audioDevice = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
  ...
  if (format.m_dataFormat != AE_FMT_FLOATP && format.m_dataFormat != AE_FMT_FLOAT &&
      format.m_dataFormat != AE_FMT_S32NE && format.m_dataFormat != AE_FMT_S32NEP &&
      format.m_dataFormat != AE_FMT_S32LE && format.m_dataFormat != AE_FMT_S16NE &&
      format.m_dataFormat != AE_FMT_S16NEP && format.m_dataFormat != AE_FMT_S16LE)
  ...
  CLog::Log(
      LOGDEBUG,
      "%s:%s Format:%d Channels:%d Samplerate:%d framesize:%d bufsize:%d bytes/s=%.2f dest=%s",
      CLASSNAME, __func__, m_format.m_dataFormat, channels, m_format.m_sampleRate,
      m_format.m_frameSize, m_format.m_frameSize * m_format.m_frames,
      1.0 / m_sinkbuffer_sec_per_byte, audioDevice.c_str());
  ...
  if (!m_omx_render.Initialize("OMX.broadcom.audio_render", OMX_IndexParamAudioInit))
    CLog::Log(LOGERROR, "%s::%s - m_omx_render.Initialize omx_err(0x%08x)", CLASSNAME, __func__,
              omx_err);

  if (m_output == AESINKPI_BOTH)
  {
    if (!m_omx_splitter.Initialize("OMX.broadcom.audio_splitter", OMX_IndexParamAudioInit))
      CLog::Log(LOGERROR, "%s::%s - m_omx_splitter.Initialize omx_err(0x%08x)", CLASSNAME, __func__,
                omx_err);
  ...
  if (m_omx_splitter.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - error m_omx_splitter SetParameter in omx_err(0x%08x)",
                CLASSNAME, __func__, omx_err);

    m_pcm_input.nPortIndex = m_omx_splitter.GetOutputPort();
    omx_err = m_omx_splitter.SetParameter(OMX_IndexParamAudioPcm, &m_pcm_input);
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - error m_omx_splitter SetParameter omx_err(0x%08x)", CLASSNAME,
                __func__, omx_err);
  ...
  if (m_omx_render_slave.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - error m_omx_render_slave SetParameter in omx_err(0x%08x)",
                CLASSNAME, __func__, omx_err);
  }
  ...
  if (m_omx_render.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s::%s - error m_omx_render SetParameter in omx_err(0x%08x)", CLASSNAME,
                __func__, omx_err);
  }
  ...
  if (m_omx_output->IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s:%s - error get OMX_IndexParamPortDefinition (input) omx_err(0x%08x)",
                CLASSNAME, __func__, omx_err);

    port_param.nBufferCountActual =
        std::max((unsigned int)port_param.nBufferCountMin, (unsigned int)NUM_OMX_BUFFERS);
    port_param.nBufferSize =
        ALIGN_UP(m_format.m_frameSize * m_format.m_frames, port_param.nBufferAlignment);

    omx_err = m_omx_output->SetParameter(OMX_IndexParamPortDefinition, &port_param);
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s:%s - error set OMX_IndexParamPortDefinition (input) omx_err(0x%08x)",
                CLASSNAME, __func__, omx_err);
  ...
  if (m_omx_splitter.IsInitialized())
  {
    m_omx_tunnel_splitter.Initialize(&m_omx_splitter, m_omx_splitter.GetOutputPort(), &m_omx_render,
                                     m_omx_render.GetInputPort());
    omx_err = m_omx_tunnel_splitter.Establish();
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "COMXAudio::Initialize - Error m_omx_tunnel_splitter.Establish 0x%08x",
                omx_err);
      return false;
    }

    m_omx_tunnel_splitter_slave.Initialize(&m_omx_splitter, m_omx_splitter.GetOutputPort() + 1,
                                           &m_omx_render_slave, m_omx_render_slave.GetInputPort());
    omx_err = m_omx_tunnel_splitter_slave.Establish();
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR,
                "COMXAudio::Initialize - Error m_omx_tunnel_splitter_slave.Establish 0x%08x",
                omx_err);
      return false;
    }
  }
  ...
  if (m_omx_splitter.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s:%s - m_omx_splitter OMX_StateExecuting omx_err(0x%08x)", CLASSNAME,
                __func__, omx_err);
  }
  if (m_omx_render.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s:%s - m_omx_render OMX_StateExecuting omx_err(0x%08x)", CLASSNAME,
                __func__, omx_err);
  }
  if (m_omx_render_slave.IsInitialized())
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "%s:%s - m_omx_render_slave OMX_StateExecuting omx_err(0x%08x)",
                CLASSNAME, __func__, omx_err);
  }
...
bool CAESinkPi::IsCompatible(const AEAudioFormat& format, const std::string& device)
{
  bool compatible =
      ...
      (m_initFormat.m_sampleRate == format.m_sampleRate ||
       m_format.m_sampleRate == format.m_sampleRate) &&
      (m_initFormat.m_dataFormat == format.m_dataFormat ||
       m_format.m_dataFormat == format.m_dataFormat) &&
      (m_initFormat.m_channelLayout == format.m_channelLayout ||
       m_format.m_channelLayout == format.m_channelLayout) &&
      (m_initDevice == device);
  CLog::Log(LOGDEBUG, "%s:%s Format:%d Channels:%d Samplerate:%d = %d", CLASSNAME, __func__,
            format.m_dataFormat, format.m_channelLayout.Count(), format.m_sampleRate, compatible);
  return compatible;
}
...
void CAESinkPi::GetDelay(AEDelayStatus& status)
{
  ...
  double sinkbuffer_seconds_to_empty =
      m_sinkbuffer_sec_per_byte * param.nU32 * m_format.m_frameSize;
...
unsigned int CAESinkPi::AddPackets(uint8_t** data, unsigned int frames, unsigned int offset)
{
  ...
  if (delay <= 0.0 && m_submitted)
    CLog::Log(LOGNOTICE, "%s:%s Underrun (delay:%.2f frames:%d)", CLASSNAME, __func__, delay,
              frames);
...
void CAESinkPi::Drain()
{
  ...
  if (delay)
    Sleep(delay);
  CLog::Log(LOGDEBUG, "%s:%s delay:%dms now:%dms", CLASSNAME, __func__, delay,
            (int)(status.GetDelay() * 1000.0));
}
