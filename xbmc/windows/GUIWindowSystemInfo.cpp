  ...
  case GUI_MSG_FOCUSED:
  {
    ...
    else if (m_section == CONTROL_BT_POLICY)
    {
      SET_CONTROL_LABEL(CONTROL_TB_POLICY,
                        CServiceBroker::GetGUI()->GetInfoManager().GetLabel(SYSTEM_PRIVACY_POLICY));
...
void CGUIWindowSystemInfo::FrameMove()
{
  ...
  else if (m_section == CONTROL_BT_VIDEO)
  {
    ...
    SET_CONTROL_LABEL(
        i++, CServiceBroker::GetGUI()->GetInfoManager().GetLabel(SYSTEM_VIDEO_ENCODER_INFO));
    SetControlLabel(i++, "%s %s", 13287, SYSTEM_SCREEN_RESOLUTION);
#ifndef HAS_DX
...
{
  std::string tmpStr =
      StringUtils::Format(format, g_localizeStrings.Get(label).c_str(),
