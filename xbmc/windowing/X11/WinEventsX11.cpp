...
static uint32_t SymMappingsX11[][2] = {{XK_BackSpace, XBMCK_BACKSPACE},
                                       {XK_Tab, XBMCK_TAB},
...
bool CWinEventsX11::Init(Display* dpy, Window win)
{
  ...
  if (m_xim)
  {
    m_xic = XCreateIC(m_xim, XNClientWindow, m_window, XNFocusWindow, m_window, XNInputStyle,
                      XIMPreeditNothing | XIMStatusNothing, XNResourceName, res_name,
                      XNResourceClass, res_name, nullptr);
  }
  ...
  {
    XRRSelectInput(m_display, RootWindow(m_display, i),
...
bool CWinEventsX11::MessagePump()
{
  ...
  while (m_display && XPending(m_display))
  {
    ...
    else if (m_display && (xevent.type == m_RREventBase + RRNotify))
    {
      ...
      {
        XRROutputChangeNotifyEvent* changeEvent =
    ...
    case KeyPress:
    {
      ...
      Status status;
      int len;
      len = Xutf8LookupString(m_xic, &xevent.xkey, m_keybuf, m_keybuf_len, &xkeysym, &status);
    ...
    case KeyRelease:
    {
      ...
      if (XEventsQueued(m_display, QueuedAfterReading))
      {
        ...
        if (next_event.type == KeyPress && next_event.xkey.window == xevent.xkey.window &&
