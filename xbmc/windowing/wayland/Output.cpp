  ...
  , m_doneHandler{doneHandler}
{
  ...
  m_output.on_mode() = [this](wayland::output_mode flags, std::int32_t width, std::int32_t height,
                              std::int32_t refresh) {
  ...
  m_output.on_scale() = [this](std::int32_t scale) { m_scale = scale; };

  m_output.on_done() = [this]() { m_doneHandler(); };
}
...
float COutput::GetPixelRatioForMode(const Mode& mode) const
{
  ...
  {
    return ((static_cast<float>(m_physicalSize.Width()) / static_cast<float>(mode.size.Width())) /
...
float COutput::GetDpiForMode(const Mode& mode) const
{
  ...
  else
  {
    ...
    float diagonalPixels =
        std::sqrt(mode.size.Width() * mode.size.Width() + mode.size.Height() * mode.size.Height());
    // physicalWidth/physicalHeight is in millimeters
    float diagonalInches = std::sqrt(m_physicalSize.Width() * m_physicalSize.Width() +
