...
bool CDRMLegacy::SetVideoMode(const RESOLUTION_INFO& res, struct gbm_bo* bo)
{
  ...
  auto ret = drmModeSetCrtc(m_fd, m_crtc->crtc->crtc_id, drm_fb->fb_id, 0, 0,
                            &m_connector->connector->connector_id, 1, m_mode);

  if (ret < 0)
  {
    CLog::Log(LOGERROR, "CDRMLegacy::%s - failed to set crtc mode: %dx%d%s @ %d Hz", __FUNCTION__,
  ...
  CLog::Log(LOGDEBUG, "CDRMLegacy::%s - set crtc mode: %dx%d%s @ %d Hz", __FUNCTION__,
...
bool CDRMLegacy::QueueFlip(struct gbm_bo* bo)
{
  ...
  auto ret = drmModePageFlip(m_fd, m_crtc->crtc->crtc_id, drm_fb->fb_id, DRM_MODE_PAGE_FLIP_EVENT,
                             &flip_happening);
