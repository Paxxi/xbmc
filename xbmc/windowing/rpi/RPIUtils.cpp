...
#define IS_WIDESCREEN(m) \
  (m == 3 || m == 7 || m == 9 || m == 11 || m == 13 || m == 15 || m == 18 || m == 22 || m == 24 || \
   m == 26 || m == 28 || m == 30 || m == 36 || m == 38 || m == 43 || m == 45 || m == 49 || \
   m == 51 || m == 53 || m == 55 || m == 57 || m == 59)
...
int CRPIUtils::FindMatchingResolution(const RESOLUTION_INFO& res,
                                      const std::vector<RESOLUTION_INFO>& resolutions,
                                      bool desktop)
{
  uint32_t mask =
      desktop ? D3DPRESENTFLAG_MODEMASK : D3DPRESENTFLAG_MODE3DSBS | D3DPRESENTFLAG_MODE3DTB;
...
int CRPIUtils::AddUniqueResolution(RESOLUTION_INFO& res,
...
bool CRPIUtils::SetNativeResolution(const RESOLUTION_INFO res, EGLSurface m_nativeWindow)
{
  ...
  if (GETFLAGS_GROUP(res.dwFlags) && GETFLAGS_MODE(res.dwFlags))
  {
    ...
    if (fabsf(res.fRefreshRate * (1001.0f / 1000.0f) - iFrameRate) <
        fabsf(res.fRefreshRate - iFrameRate))
    ...
    int success = m_DllBcmHost->vc_tv_hdmi_power_on_explicit_new(
        HDMI_MODE_HDMI, GETFLAGS_GROUP(res.dwFlags), GETFLAGS_MODE(res.dwFlags));

    if (success == 0)
    {
      CLog::Log(LOGDEBUG, "EGL set HDMI mode (%d,%d)=%d %s%s\n", GETFLAGS_GROUP(res.dwFlags),
  ...
  else if (!GETFLAGS_GROUP(res.dwFlags) && GETFLAGS_MODE(res.dwFlags))
  {
    ...
    options.aspect =
        get_sdtv_aspect_from_display_aspect((float)res.iScreenWidth / (float)res.iScreenHeight);

    int success =
        m_DllBcmHost->vc_tv_sdtv_power_on((SDTV_MODE_T)GETFLAGS_MODE(res.dwFlags), &options);
  ...
  m_dispman_element = m_DllBcmHost->vc_dispmanx_element_add(dispman_update, m_dispman_display,
                                                            1, // layer
                                                            &dst_rect,
  ...
  m_DllBcmHost->vc_dispmanx_display_set_background(dispman_update, m_dispman_display, 0x00, 0x00,
                                                   0x00);
...
static void SetResolutionString(RESOLUTION_INFO& res)
{
  ...
  res.strMode = StringUtils::Format("%dx%d (%dx%d) @ %.2f%s - Full Screen", res.iScreenWidth,
                                    res.iScreenHeight, res.iWidth, res.iHeight, res.fRefreshRate,
...
bool CRPIUtils::ProbeResolutions(std::vector<RESOLUTION_INFO>& resolutions)
{
  ...
  if (m_initDesktopRes)
  {
    ...
    if ((tv_state.state & (VC_HDMI_HDMI | VC_HDMI_DVI)) != 0) // hdtv
    {
      ...
      m_desktopRes.dwFlags = MAKEFLAGS(tv_state.display.hdmi.group, tv_state.display.hdmi.mode,
                                       tv_state.display.hdmi.scan_mode);
      m_desktopRes.fPixelRatio =
          tv_state.display.hdmi.display_options.aspect == 0
              ? 1.0f
              : get_display_aspect_ratio(
                    (HDMI_ASPECT_T)tv_state.display.hdmi.display_options.aspect) /
                    ((float)m_desktopRes.iScreenWidth / (float)m_desktopRes.iScreenHeight);
      HDMI_PROPERTY_PARAM_T property;
      property.property = HDMI_PROPERTY_PIXEL_CLOCK_TYPE;
      vc_tv_hdmi_get_property(&property);
      m_desktopRes.fRefreshRate = property.param1 == HDMI_PIXEL_CLOCK_TYPE_NTSC
                                      ? tv_state.display.hdmi.frame_rate * (1000.0f / 1001.0f)
    ...
    else if ((tv_state.state & (VC_SDTV_NTSC | VC_SDTV_PAL)) != 0) // sdtv
    {
      ...
      m_desktopRes.fPixelRatio =
          tv_state.display.hdmi.display_options.aspect == 0
              ? 1.0f
              : get_display_aspect_ratio(
                    (SDTV_ASPECT_T)tv_state.display.sdtv.display_options.aspect) /
                    ((float)m_desktopRes.iScreenWidth / (float)m_desktopRes.iScreenHeight);
    }
...
void CRPIUtils::SetVisible(bool enable)
{
  ...
  if (m_dispman_element != DISPMANX_NO_HANDLE)
  {
    ...
    m_shown = enable;
    m_DllBcmHost->vc_dispmanx_element_change_attributes(dispman_update, m_dispman_element, (1 << 2),
                                                        0, 0, &dst_rect, nullptr, 0,
                                                        DISPMANX_NO_ROTATE);
  }
...
void CRPIUtils::GetSupportedModes(HDMI_RES_GROUP_T group, std::vector<RESOLUTION_INFO>& resolutions)
{
  ...
  int max_supported_modes =
      m_DllBcmHost->vc_tv_hdmi_get_supported_modes_new(group, NULL, 0, &prefer_group, &prefer_mode);
  ...
  if (supported_modes)
  {
    num_modes = m_DllBcmHost->vc_tv_hdmi_get_supported_modes_new(
  ...
  if (num_modes > 0 && prefer_group != HDMI_RES_GROUP_INVALID)
  {
    ...
    for (i = 0; i < num_modes; i++, tv++)
    {
      ...
      res.fPixelRatio = get_display_aspect_ratio((HDMI_ASPECT_T)tv->aspect_ratio) /
                        ((float)res.iScreenWidth / (float)res.iScreenHeight);
      ...
      if (tv->frame_rate == 24 || tv->frame_rate == 30 || tv->frame_rate == 48 ||
          tv->frame_rate == 60 || tv->frame_rate == 72)
      {
...
void CRPIUtils::CallbackTvServiceCallback(void* userdata,
                                          uint32_t reason,
                                          uint32_t param1,
                                          uint32_t param2)
{
