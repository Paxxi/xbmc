...
static const translateField fields[] = {
    ...
    {"tracknumber", FieldTrackNumber, CDatabaseQueryRule::NUMERIC_FIELD,
     StringValidation::IsPositiveInteger, false, 554},
    ...
    {"year", FieldYear, CDatabaseQueryRule::NUMERIC_FIELD, StringValidation::IsPositiveInteger,
     true, 562},
    {"time", FieldTime, CDatabaseQueryRule::SECONDS_FIELD, StringValidation::IsTime, false, 180},
    {"playcount", FieldPlaycount, CDatabaseQueryRule::NUMERIC_FIELD,
     StringValidation::IsPositiveInteger, false, 567},
    ...
    {"rating", FieldRating, CDatabaseQueryRule::REAL_FIELD, CSmartPlaylistRule::ValidateRating,
     false, 563},
    {"userrating", FieldUserRating, CDatabaseQueryRule::REAL_FIELD,
     CSmartPlaylistRule::ValidateMyRating, false, 38018},
    {"votes", FieldVotes, CDatabaseQueryRule::REAL_FIELD, StringValidation::IsPositiveInteger,
     false, 205},
    ...
    {"season", FieldSeason, CDatabaseQueryRule::NUMERIC_FIELD, StringValidation::IsPositiveInteger,
     false, 20373},
    {"episode", FieldEpisodeNumber, CDatabaseQueryRule::NUMERIC_FIELD,
     StringValidation::IsPositiveInteger, false, 20359},
    {"numepisodes", FieldNumberOfEpisodes, CDatabaseQueryRule::REAL_FIELD,
     StringValidation::IsPositiveInteger, false, 20360},
    {"numwatched", FieldNumberOfWatchedEpisodes, CDatabaseQueryRule::REAL_FIELD,
     StringValidation::IsPositiveInteger, false, 21457},
    ...
    {"audiocount", FieldAudioCount, CDatabaseQueryRule::REAL_FIELD,
     StringValidation::IsPositiveInteger, false, 21481},
    {"subtitlecount", FieldSubtitleCount, CDatabaseQueryRule::REAL_FIELD,
     StringValidation::IsPositiveInteger, false, 21482},
    {"subtitlelanguage", FieldSubtitleLanguage, CDatabaseQueryRule::TEXTIN_FIELD, NULL, false,
     21448},
...
std::string CSmartPlaylistRule::GetLocalizedField(int field)
{
  for (const translateField& f : fields)
    if (field == f.field)
      return g_localizeStrings.Get(f.localizedString);
...
{
  return StringUtils::Format("%s %s %s", GetLocalizedField(m_field).c_str(),
...
CDatabaseQueryRule::SEARCH_OPERATOR CSmartPlaylistRule::GetOperator(
    const std::string& strType) const
{
                                                  ...
                                                  const std::string& strType) const
{
  ...
  else if (strType == "albums")
  {
    ...
    if (m_field == FieldGenre)
      ...
      query = negate +
              ...
              "song.idAlbum = " +
              GetField(FieldId, strType) + " AND path.strPath" + parameter + ")";
    else if (m_field == FieldLastPlayed &&
             (m_operator == OPERATOR_LESS_THAN || m_operator == OPERATOR_BEFORE ||
              m_operator == OPERATOR_NOT_IN_THE_LAST))
      query = GetField(m_field, strType) + " is NULL or " + GetField(m_field, strType) + parameter;
  ...
  else if (strType == "artists")
  {
    ...
    {
      query =
          negate + " (EXISTS (SELECT DISTINCT song_artist.idArtist FROM song_artist JOIN song ON "
                   "song.idSong = song_artist.idSong JOIN path ON song.idpath = path.idpath ";
      query += "WHERE song_artist.idArtist = " + GetField(FieldId, strType) + " AND path.strPath" +
               parameter + "))";
    }
    else if (m_field == FieldSource)
    {
  ...
  else if (strType == "movies")
  {
    ...
    if (m_field == FieldGenre)
      query = negate + FormatLinkQuery("genre", "genre", MediaTypeMovie, GetField(FieldId, strType),
                                       parameter);
    else if (m_field == FieldDirector)
      query = negate + FormatLinkQuery("director", "actor", MediaTypeMovie,
                                       GetField(FieldId, strType), parameter);
    else if (m_field == FieldActor)
      query = negate + FormatLinkQuery("actor", "actor", MediaTypeMovie, GetField(FieldId, strType),
                                       parameter);
      ...
      query = negate +
              FormatLinkQuery("tag", "tag", MediaTypeMovie, GetField(FieldId, strType), parameter);
  }
  ...
  else if (strType == "tvshows")
  {
    ...
    if (m_field == FieldGenre)
      ...
      query = "CASE WHEN COALESCE(" + GetField(FieldNumberOfEpisodes, strType) + " - " +
              GetField(FieldNumberOfWatchedEpisodes, strType) + ", 0) > 0 THEN 0 ELSE 1 END " +
              parameter;
    else if (m_field == FieldTag)
      query = negate +
              FormatLinkQuery("tag", "tag", MediaTypeTvShow, GetField(FieldId, strType), parameter);
  }
  else if (strType == "episodes")
  {
    table = "episode_view";
...
std::string CSmartPlaylistRule::GetField(int field, const std::string& type) const
{
  if (field >= FieldUnknown && field < FieldMax)
    return DatabaseUtils::GetField((Field)field, CMediaTypes::FromString(type),
  ...
  for (CDatabaseQueryRules::const_iterator it = m_rules.begin(); it != m_rules.end(); ++it)
  {
    ...
    if ((*it)->m_field == FieldPlaylist)
    {
      std::string playlistFile =
          CSmartPlaylistDirectory::GetPlaylistByName((*it)->m_parameter.at(0), strType);
      if (!playlistFile.empty() &&
          referencedPlaylists.find(playlistFile) == referencedPlaylists.end())
      {
    ...
    const std::string& strType, std::vector<std::string>& virtualFolders) const
{
  ...
  for (CDatabaseQueryRules::const_iterator it = m_rules.begin(); it != m_rules.end(); ++it)
  {
    if (((*it)->m_field != FieldVirtualFolder && (*it)->m_field != FieldPlaylist) ||
        (*it)->m_operator != CDatabaseQueryRule::OPERATOR_EQUALS)
      continue;
...
const TiXmlNode* CSmartPlaylist::readNameFromPath(const CURL& url)
{
  ...
  if (!file.Open(url))
  {
    CLog::Log(LOGERROR, "Error loading Smart playlist %s (failed to read file)",
...
const TiXmlNode* CSmartPlaylist::readNameFromXml(const std::string& xml)
{
  ...
  {
    CLog::Log(LOGERROR, "Error loading Smart playlist (failed to parse xml: %s)",
              m_xmlDoc.ErrorDesc());
    return NULL;
  }
...
bool CSmartPlaylist::LoadFromXML(const TiXmlNode* root, const std::string& encoding)
{
  ...
  if (order && order->FirstChild())
  {
    ...
    if (direction)
      m_orderDirection = StringUtils::EqualsNoCase(direction, "ascending") ? SortOrderAscending
                                                                           : SortOrderDescending;
...
bool CSmartPlaylist::Save(const std::string& path) const
{
  ...
  XMLUtils::SetString(
      pRoot, "match",
      m_ruleCombination.GetType() == CSmartPlaylistRuleCombination::CombinationAnd ? "all" : "one");
                                        ...
                                        std::vector<std::string>& fieldList)
{
  ...
  for (std::vector<Field>::const_iterator field = typeFields.begin(); field != typeFields.end();
       ++field)
  {
