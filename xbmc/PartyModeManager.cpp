...
bool CPartyModeManager::AddRandomSongs()
{
  ...
  if (iMissingSongs > 0)
  {
    ...
    iMissingSongs =
        std::min(iMissingSongs, static_cast<int>(m_songIDCache.size()) - m_iMatchingSongsPicked);
    ...
    if (bSongs)
    {
      ...
      if (database.Open())
      {
        database.GetSongsFullByWhere("musicdb://songs/", CDatabase::Filter(sqlWhereMusic), items,
