...
bool InfoScanner::HasNoMedia(const std::string &strDirectory) const
{
  std::string noMediaFile = URIUtils::AddFileToFolder(strDirectory, ".nomedia");
  return XFILE::CFile::Exists(noMediaFile);
}

bool InfoScanner::IsExcluded(const std::string& strDirectory, const std::vector<std::string> &regexps)
{
