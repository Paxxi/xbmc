...
#define XBMCCOMMONS_STANDARD_EXCEPTION(E) \
  ...
  { \
  public: \
    inline E(const char* message, ...) XBMCCOMMONS_ATTRIB_EXCEPTION_FORMAT : Exception(#E) \
    { \
      XBMCCOMMONS_COPYVARARGS(message); \
    } \
\
    inline E(const E& other) \
      : Exception(other) \
    { \
    } \
  }
...
protected:
  inline explicit Exception(const char* classname_)
