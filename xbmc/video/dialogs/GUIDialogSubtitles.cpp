...
public:
  ...
  ~CSubtitlesJob() override { delete m_items; }
...
bool CGUIDialogSubtitles::OnMessage(CGUIMessage& message)
{
  if (message.GetMessage() == GUI_MSG_CLICKED)
  {
    ...
    {
      //manual search
      if (CGUIKeyboardFactory::ShowAndGetInput(m_strManualSearch,
                                               CVariant{g_localizeStrings.Get(24121)}, true))
      {
...
void CGUIDialogSubtitles::FillServices()
{
  ...
  if (item.GetVideoContentType() == VIDEODB_CONTENT_TVSHOWS ||
    ...
    defaultService = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(
                                             ...
                                             const std::string& language)
{
  ...
  SUBTITLE_STORAGEMODE storageMode =
      (SUBTITLE_STORAGEMODE)CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
