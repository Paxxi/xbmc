...
void CGUIDialogCMSSettings::InitializeSettings()
{
  ...
  dependencyCmsEnabled.Or()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(
  ...
  dependencyCms3dlut.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(
  ...
  dependencyCmsIcc.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(
  ...
  dependencyCmsGamma.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(
  ...
  AddToggle(groupColorManagement, SETTING_VIDEO_CMSENABLE, 36560, SettingLevel::Basic,
            settings->GetBool(SETTING_VIDEO_CMSENABLE));
...
#endif
  std::shared_ptr<CSettingInt> settingCmsMode =
      AddSpinner(groupColorManagement, SETTING_VIDEO_CMSMODE, 36562, SettingLevel::Basic,
                 currentMode, entries);
  settingCmsMode->SetDependencies(depsCmsEnabled);

  std::string current3dLUT = settings->GetString(SETTING_VIDEO_CMS3DLUT);
  std::shared_ptr<CSettingString> settingCms3dlut =
      AddList(groupColorManagement, SETTING_VIDEO_CMS3DLUT, 36564, SettingLevel::Basic,
              current3dLUT, Cms3dLutsFiller, 36564);
  ...
  std::shared_ptr<CSettingInt> settingCmsWhitepoint =
      AddSpinner(groupColorManagement, SETTING_VIDEO_CMSWHITEPOINT, 36568, SettingLevel::Basic,
                 currentWhitepoint, entries);
  ...
  std::shared_ptr<CSettingInt> settingCmsPrimaries =
      AddSpinner(groupColorManagement, SETTING_VIDEO_CMSPRIMARIES, 36570, SettingLevel::Basic,
                 currentPrimaries, entries);
  ...
  std::shared_ptr<CSettingInt> settingCmsGammaMode =
      AddSpinner(groupColorManagement, SETTING_VIDEO_CMSGAMMAMODE, 36572, SettingLevel::Basic,
                 currentGammaMode, entries);
  settingCmsGammaMode->SetDependencies(depsCmsIcc);

  float currentGamma = settings->GetInt(SETTING_VIDEO_CMSGAMMA) / 100.0f;
  if (currentGamma == 0.0)
    currentGamma = 2.20;
  std::shared_ptr<CSettingNumber> settingCmsGamma =
      AddSlider(groupColorManagement, SETTING_VIDEO_CMSGAMMA, 36574, SettingLevel::Basic,
                currentGamma, 36597, 1.6, 0.05, 2.8, 36574, usePopup);
  ...
  std::shared_ptr<CSettingInt> settingCmsLutSize =
      AddSpinner(groupColorManagement, SETTING_VIDEO_CMSLUTSIZE, 36576, SettingLevel::Basic,
                 currentLutSize, entries);
...
void CGUIDialogCMSSettings::OnSettingChanged(std::shared_ptr<const CSetting> setting)
{
  ...
  if (settingId == SETTING_VIDEO_CMSENABLE)
    ...
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetInt(
        SETTING_VIDEO_CMSMODE, std::static_pointer_cast<const CSettingInt>(setting)->GetValue());
    ...
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetInt(
        SETTING_VIDEO_CMSLUTSIZE, std::static_pointer_cast<const CSettingInt>(setting)->GetValue());
}
...
void CGUIDialogCMSSettings::Cms3dLutsFiller(SettingConstPtr setting,
