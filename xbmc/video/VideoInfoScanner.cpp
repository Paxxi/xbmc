...
void CVideoInfoScanner::Process()
{
  ...
  try
  {
    ...
    CLog::Log(LOGNOTICE, "VideoInfoScanner: Finished scan. Scanning for video info took %s",
...
bool CVideoInfoScanner::DoScan(const std::string& strDirectory)
{
  ...
  const std::vector<std::string>& regexps = content == CONTENT_TVSHOWS
  ...
  if (URIUtils::IsPlugin(strDirectory) &&
      !CPluginDirectory::IsMediaLibraryScanningAllowed(TranslateContent(content), strDirectory))
  {
    CLog::Log(
        LOGNOTICE,
  ...
  if (content == CONTENT_MOVIES || content == CONTENT_MUSICVIDEOS)
  {
    if (m_handle)
    {
      ...
      m_handle->SetTitle(
          StringUtils::Format(g_localizeStrings.Get(str).c_str(), info->Name().c_str()));
    }
    ...
    if (m_database.GetPathHash(strDirectory, dbHash) && !fastHash.empty() &&
        StringUtils::EqualsNoCase(fastHash, dbHash))
  ...
  else if (content == CONTENT_TVSHOWS)
  {
    if (m_handle)
      m_handle->SetTitle(
          StringUtils::Format(g_localizeStrings.Get(20319).c_str(), info->Name().c_str()));
  ...
  if (!bSkip)
  {
    ...
    else
    {
      ...
      CLog::Log(LOGDEBUG, "VideoInfoScanner: No (new) information was found in dir %s",
           ...
           (content == CONTENT_MOVIES || content == CONTENT_MUSICVIDEOS))
                                          ...
                                          CGUIDialogProgress* pDlgProgress)
{
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (info2->Content() == CONTENT_TVSHOWS)
      ret = RetrieveInfoForTvShow(pItem.get(), bDirNames, info2, useLocal, pURL, fetchEpisodes,
                                  pDlgProgress);
    ...
    {
      CLog::Log(LOGWARNING, "No information found for item '%s', it won't be added to the library.",
                CURL::GetRedacted(pItem->GetPath()).c_str());
      ...
      CServiceBroker::GetEventLog().Add(EventPtr(new CMediaLibraryEvent(
          ...
          StringUtils::Format(g_localizeStrings.Get(24147).c_str(), mediaType.c_str(),
...
bool CVideoInfoScanner::EnumerateEpisodeItem(const CFileItem* item, EPISODELIST& episodeList)
{
  ...
  for (unsigned int i = 0; i < expression.size(); ++i)
  {
                                    ...
                                    ->m_tvshowMultiPartEnumRegExp))
    {
      ...
      while (((regexp2pos = reg2.RegFind(remainder.c_str() + offset)) > -1) |
             ((regexppos = reg.RegFind(remainder.c_str() + offset)) > -1))
      {
                                 ...
                                 bool libraryImport /* = false */)
{
  ...
  {
    strTitle =
        StringUtils::Format("%s - %ix%i - %s", showInfo->m_strTitle.c_str(), movieDetails.m_iSeason,
                            movieDetails.m_iEpisode, strTitle.c_str());
  }
  ...
  else if (content == CONTENT_TVSHOWS)
  {
    ...
    else
    {
      ...
      lResult =
          m_database.SetDetailsForEpisode(pItem->GetPath(), movieDetails, art, idShow, idEpisode);
      movieDetails.m_iDbId = lResult;
                                   ...
                                   const std::string& actorArtPath)
{
  ...
  std::vector<std::string> artTypes =
      CVideoThumbLoader::GetArtTypes(ContentToMediaType(content, pItem->m_bIsFolder));
  ...
  if (pItem->HasVideoInfoTag() && !pItem->GetVideoInfoTag()->m_coverArt.empty())
  {
    ...
    {
      if (std::find(artTypes.begin(), artTypes.end(), it.m_type) != artTypes.end() &&
  ...
  if (find(artTypes.begin(), artTypes.end(), "fanart") != artTypes.end() &&
  ...
  for (const auto& url : pItem->GetVideoInfoTag()->m_strPictureURL.m_url)
  {
    ...
    if (aspect.empty())
      ...
      aspect =
          ContentToMediaType(content, pItem->m_bIsFolder) == MediaTypeEpisode ? "thumb" : "poster";
    if (find(artTypes.begin(), artTypes.end(), aspect) == artTypes.end() ||
    ...
    CGUIDialogProgress* pDlgProgress /* = NULL */)
{
  ...
  for (EPISODELIST::iterator file = files.begin(); file != files.end(); ++file)
  {
    ...
    for (; guide != episodes.end(); ++guide)
    {
      ...
      if (!guide->cScraperUrl.strTitle.empty() &&
          StringUtils::EqualsNoCase(guide->cScraperUrl.strTitle, file->strTitle))
      {
        bFound = true;
        break;
      }
    ...
    if (!bFound)
    {
      ...
      else if (!file->strTitle.empty())
      {
        ...
        if (index >= 0 && matchscore >= minscore)
        {
          ...
          CLog::Log(
              LOGDEBUG,
              "%s fuzzy title match for show: '%s', title: '%s', match: '%s', score: %f >= %f",
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (pItem->IsPlugin())
    {
      ...
      if (pItem->m_dateTime.IsValid())
        digest.Update(StringUtils::Format("%02i.%02i.%04i", pItem->m_dateTime.GetDay(),
    ...
    bool useLocal)
{
  if (useLocal)
  {
    ...
    int maxSeasons = 0;
    CFileItemList items;
    CDirectory::GetDirectory(show.m_strPath, items, ".png|.jpg|.tbn",
