  ...
  if (g_application.GetAppPlayer().IsPlayingVideo())
  {
    ...
    case ACTION_AUDIO_NEXT_LANGUAGE:
    {
      ...
      g_application.GetAppPlayer().SetAudioStream(
          currentAudio); // Set the audio stream to the one selected
      std::string aud;
      std::string lan;
    ...
    case ACTION_ZOOM_IN:
    {
      ...
      g_application.GetAppPlayer().SetRenderViewMode(
          ViewModeCustom, vs.m_CustomZoomAmount, vs.m_CustomPixelRatio, vs.m_CustomVerticalShift,
    ...
    case ACTION_SUBTITLE_VSHIFT_UP:
    {
      ...
      if ((subalign == SUBTITLE_ALIGN_BOTTOM_OUTSIDE) || (subalign == SUBTITLE_ALIGN_TOP_INSIDE))
      {
        ...
        ShowSlider(action.GetID(), 274, (float)res_info.iHeight - res_info.iSubtitles, 0.0f, 1.0f,
                   (float)res_info.iHeight);
      }
      else
      {
        res_info.iSubtitles--;
        ...
        if (subalign == SUBTITLE_ALIGN_MANUAL)
          ShowSlider(action.GetID(), 274, (float)res_info.iSubtitles, 0.0f, 1.0f,
                     (float)res_info.iHeight);
        else
          ShowSlider(action.GetID(), 274, (float)res_info.iSubtitles - res_info.iHeight,
                     (float)-res_info.iHeight, -1.0f, 0.0f);
      }
      CServiceBroker::GetWinSystem()->GetGfxContext().SetResInfo(
          CServiceBroker::GetWinSystem()->GetGfxContext().GetVideoResolution(), res_info);
      return true;
    }
    ...
    case ACTION_SUBTITLE_ALIGN:
    {
      ...
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetInt(
          CSettings::SETTING_SUBTITLES_ALIGN, subalign);
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, g_localizeStrings.Get(21460),
    ...
    case ACTION_VOLAMP_DOWN:
    {
      ...
      vs.m_VolumeAmplification = std::max(std::min(vs.m_VolumeAmplification, sliderMax), sliderMin);

      g_application.GetAppPlayer().SetDynamicRangeCompression(
          (long)(vs.m_VolumeAmplification * 100));
    ...
    case ACTION_VOLAMP:
    {
      ...
      ShowSlider(action.GetID(), 660,
                 g_application.GetAppPlayer().GetVideoSettings().m_VolumeAmplification, sliderMin,
                 1.0f, sliderMax, true);
      return true;
    }
