...
static void AnnounceRemove(const std::string& content, int id)
{
  ...
  CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::AudioLibrary, "xbmc", "OnRemove",
                                                     data);
}
...
static void AnnounceUpdate(const std::string& content, int id, bool added = false)
{
  ...
  if (added)
    data["added"] = true;
  CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::AudioLibrary, "xbmc", "OnUpdate",
                                                     data);
}
...
void CMusicDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE versiontagscan (idVersion INTEGER, iNeedsScan INTEGER, lastscanned "
              "VARCHAR(20))");
...
void CMusicDatabase::CreateViews()
{
  ...
  m_pDS->exec("CREATE VIEW albumview AS SELECT "
              ...
              "        bScrapedMBID,"
              "        lastScraped,"
              "        (SELECT ROUND(AVG(song.iTimesPlayed)) FROM song WHERE song.idAlbum = "
              "album.idAlbum) AS iTimesPlayed, "
              "        strReleaseType, "
              "        (SELECT MAX(song.dateAdded) FROM song WHERE song.idAlbum = album.idAlbum) "
              "AS dateAdded, "
              "        (SELECT MAX(song.lastplayed) FROM song WHERE song.idAlbum = album.idAlbum) "
              "AS lastplayed "
              "FROM album");
  ...
  m_pDS->exec("CREATE VIEW artistview AS SELECT"
              ...
              "  (SELECT MAX(song.dateAdded) FROM song_artist INNER JOIN song ON song.idSong = "
              "song_artist.idSong "
...
bool CMusicDatabase::AddAlbum(CAlbum& album, int idSource)
{
  ...
  album.idAlbum =
      AddAlbum(album.strAlbum, album.strMusicBrainzAlbumID, album.strReleaseGroupMBID,
               album.GetAlbumArtistString(), album.GetAlbumArtistSort(), album.GetGenreString(),
               album.iYear, album.strLabel, album.strType, album.bCompilation, album.releaseType);

  // Add the album artists
  if (album.artistCredits.empty())
    AddAlbumArtist(BLANKARTIST_ID, album.idAlbum, BLANKARTIST_NAME,
                   0); // Album must have at least one artist so set artist to [Missing]
  ...
  for (auto song = album.songs.begin(); song != album.songs.end(); ++song)
  {
    ...
    song->idSong =
        AddSong(song->idAlbum, song->strTitle, song->strMusicBrainzTrackID, song->strFileName,
                song->strComment, song->strMood, song->strThumb, song->GetArtistString(),
                song->GetArtistSort(), song->genre, song->iTrack, song->iDuration, song->iYear,
                song->iTimesPlayed, song->iStartOffset, song->iEndOffset, song->lastPlayed,
                song->rating, song->userrating, song->votes, song->replayGain);

    if (song->artistCredits.empty())
      AddSongArtist(BLANKARTIST_ID, song->idSong, ROLE_ARTIST, BLANKARTIST_NAME,
                    0); // Song must have at least one artist so set artist to [Missing]
...
bool CMusicDatabase::UpdateAlbum(CAlbum& album)
{
  ...
  UpdateAlbum(
      ...
      StringUtils::Join(album.themes, itemSeparator), album.strReview, album.thumbURL.m_xml.c_str(),
      album.strLabel, album.strType, album.fRating, album.iUserrating, album.iVotes, album.iYear,
                            ...
                            const ReplayGain& replayGain)
{
  ...
  try
  {
    ...
    if (!strMusicBrainzTrackID.empty())
      strSQL = PrepareSQL("SELECT idSong FROM song WHERE idAlbum = %i AND iTrack=%i AND "
                          "strMusicBrainzTrackID = '%s'",
                          idAlbum, iTrack, strMusicBrainzTrackID.c_str());
    else
      strSQL = PrepareSQL("SELECT idSong FROM song WHERE idAlbum=%i AND strFileName='%s' AND "
                          "strTitle='%s' AND iTrack=%i AND strMusicBrainzTrackID IS NULL",
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      if (dtLastPlayed.IsValid())
        strSQL +=
            PrepareSQL(",%i,%i,%i,'%s', %.1f, %i, %i, '%s','%s', '%s')", iTimesPlayed, iStartOffset,
                       iEndOffset, dtLastPlayed.GetAsDBDateTime().c_str(), rating, userrating,
                       votes, strComment.c_str(), strMood.c_str(), replayGain.Get().c_str());
      else
    ...
    else
    {
      ...
      UpdateSong(idSong, strTitle, strMusicBrainzTrackID, strPathAndFileName, strComment, strMood,
                 strThumb, artistDisp, artistSort, genres, iTrack, iDuration, iYear, iTimesPlayed,
...
bool CMusicDatabase::GetSong(int idSong, CSong& song)
{
  try
  {
    ...
    if (!m_pDS->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
    {
      m_pDS->close();
      return false;
...
{
  int result = UpdateSong(song.idSong, song.strTitle, song.strMusicBrainzTrackID, song.strFileName,
                          song.strComment, song.strMood, song.strThumb, song.GetArtistString(),
                          song.GetArtistSort(), song.genre, song.iTrack, song.iDuration, song.iYear,
                          song.iTimesPlayed, song.iStartOffset, song.iEndOffset, song.lastPlayed,
                          song.rating, song.userrating, song.votes, song.replayGain);
                               ...
                               const ReplayGain& replayGain)
{
  ...
  strSQL = PrepareSQL(
      ...
      StringUtils::Join(
          ...
          CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_musicItemSeparator)
          .c_str(),
      strTitle.c_str(), iTrack, iDuration, iYear, strFileName.c_str());
  if (strMusicBrainzTrackID.empty())
  ...
  if (dtLastPlayed.IsValid())
    strSQL += PrepareSQL(
        ", iTimesPlayed = %i, iStartOffset = %i, iEndOffset = %i, lastplayed = '%s', rating = "
        "%.1f, userrating = %i, votes = %i, comment = '%s', mood = '%s', strReplayGain = '%s'",
        iTimesPlayed, iStartOffset, iEndOffset, dtLastPlayed.GetAsDBDateTime().c_str(), rating,
        userrating, votes, strComment.c_str(), strMood.c_str(), replayGain.Get().c_str());
  else
    strSQL += PrepareSQL(
        ", iTimesPlayed = %i, iStartOffset = %i, iEndOffset = %i, lastplayed = NULL, rating = "
        "%.1f, userrating = %i, votes = %i, comment = '%s', mood = '%s', strReplayGain = '%s'",
        iTimesPlayed, iStartOffset, iEndOffset, rating, userrating, votes, strComment.c_str(),
                             ...
                             CAlbum::ReleaseType releaseType)
{
  ...
  try
  {
    ...
    if (!strMusicBrainzAlbumID.empty())
      ...
      strSQL = PrepareSQL("SELECT * FROM album WHERE strArtistDisp LIKE '%s' AND strAlbum LIKE "
                          "'%s' AND strMusicBrainzAlbumID IS NULL",
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      strSQL = PrepareSQL("INSERT INTO album (idAlbum, strAlbum, strArtistDisp, strGenres, iYear, "
                          ...
                          strAlbum.c_str(), strArtist.c_str(), strGenre.c_str(), year,
    ...
    else
    {
      ...
      if (!strMusicBrainzAlbumID.empty())
        strSQL += PrepareSQL("strAlbum = '%s', strArtistDisp = '%s', ", strAlbum.c_str(),
                             strArtist.c_str());
      if (strReleaseGroupMBID.empty())
      ...
      strSQL += PrepareSQL(", strGenres = '%s', iYear=%i, strLabel = '%s', strType = '%s', "
                           ...
                           "WHERE idAlbum=%i",
                           strGenre.c_str(), year, strRecordLabel.c_str(), strType.c_str(),
                                ...
                                bool bScrapedMBID)
{
  ...
  strSQL = PrepareSQL("UPDATE album SET "
                      ...
                      strAlbum.c_str(), strArtist.c_str(), strGenre.c_str(), strMoods.c_str(),
                      strStyles.c_str(), strThemes.c_str(), strReview.c_str(), strImage.c_str(),
                      strLabel.c_str(), strType.c_str(), fRating, iUserrating, iVotes, iYear,
                      bCompilation, CAlbum::ReleaseTypeToString(releaseType).c_str(),
                      CDateTime::GetCurrentDateTime().GetAsDBDateTime().c_str(), bScrapedMBID);
  if (strMusicBrainzAlbumID.empty())
...
bool CMusicDatabase::UpdateArtist(const CArtist& artist)
{
  ...
  UpdateArtist(artist.idArtist, artist.strArtist, artist.strSortName, artist.strMusicBrainzArtistID,
               ...
               StringUtils::Join(artist.instruments, itemSeparator), artist.strBiography,
               artist.strDied, artist.strDisbanded,
               StringUtils::Join(artist.yearsActive, itemSeparator).c_str(),
               artist.thumbURL.m_xml.c_str(), artist.fanart.m_xml.c_str());
  ...
  try
  {
    ...
    if (!strMusicBrainzArtistID.empty())
    {
      ...
      if (m_pDS->num_rows() > 0)
      {
        ...
        if (update)
        {
          strSQL = PrepareSQL("UPDATE artist SET strArtist = '%s' WHERE idArtist = %i",
      ...
      strSQL = PrepareSQL("SELECT idArtist FROM artist WHERE strArtist LIKE '%s' AND "
                          "strMusicBrainzArtistID IS NULL",
                          strArtist.c_str());
      m_pDS->query(strSQL);
      if (m_pDS->num_rows() > 0)
      {
    ...
    if (strMusicBrainzArtistID.empty())
      ...
      strSQL = PrepareSQL("INSERT INTO artist (idArtist, strArtist, strMusicBrainzArtistID, "
                          "bScrapedMBID) VALUES( NULL, '%s', '%s', %i )",
                                 ...
                                 const std::string& strFanart)
{
  ...
  strSQL =
      PrepareSQL("UPDATE artist SET "
                 ...
                 /* strSortName.c_str(),*/
                 /* strMusicBrainzArtistID.c_str(), */
                 strType.c_str(), strGender.c_str(), strDisambiguation.c_str(), strBorn.c_str(),
                 strFormed.c_str(), strGenres.c_str(), strMoods.c_str(), strStyles.c_str(),
                 strInstruments.c_str(), strBiography.c_str(), strDied.c_str(),
                 strDisbanded.c_str(), strYearsActive.c_str(), strImage.c_str(), strFanart.c_str(),
                 CDateTime::GetCurrentDateTime().GetAsDBDateTime().c_str(), bScrapedMBID);
  if (strMusicBrainzArtistID.empty())
...
bool CMusicDatabase::GetArtist(int idArtist, CArtist& artist, bool fetchAll /* = false */)
{
  try
  {
    ...
    std::string strSQL;
    if (fetchAll)
      strSQL = PrepareSQL("SELECT * FROM artistview LEFT JOIN discography ON artistview.idArtist = "
                          "discography.idArtist WHERE artistview.idArtist = %i",
                          idArtist);
    else
    ...
    if (fetchAll)
    {
      while (!m_pDS->eof())
      {
        ...
        artist.discography.emplace_back(record->at(discographyOffset + 1).get_asString(),
                                        record->at(discographyOffset + 2).get_asString());
        m_pDS->next();
      }
    }
    m_pDS->close(); // cleanup recordset data
...
{
  std::string strSQL =
      PrepareSQL("INSERT INTO discography (idArtist, strAlbum, strYear) values(%i, '%s', '%s')",
                 idArtist, strAlbum.c_str(), strYear.c_str());
  return ExecuteQuery(strSQL);
}
    ...
    int idArtist, int idSong, int idRole, const std::string& strArtist, size_t iOrder)
{
  ...
  strSQL = PrepareSQL("replace into song_artist (idArtist, idSong, idRole, strArtist, iOrder) "
                      "values(%i,%i,%i,'%s',%i)",
                      idArtist, idSong, idRole, strArtist.c_str(), static_cast<int>(iOrder));
  return ExecuteQuery(strSQL);
}
...
bool CMusicDatabase::GetRolesByArtist(int idArtist, CFileItem* item)
{
  ...
  {
    std::string strSQL = PrepareSQL(
        "SELECT DISTINCT song_artist.idRole, Role.strRole FROM song_artist JOIN role ON "
        " song_artist.idRole = role.idRole WHERE idArtist = %i ORDER BY song_artist.idRole ASC",
        idArtist);
    if (!m_pDS->query(strSQL))
      return false;
    if (m_pDS->num_rows() == 0)
...
bool CMusicDatabase::AddAlbumArtist(int idArtist, int idAlbum, std::string strArtist, size_t iOrder)
{
  std::string strSQL;
  strSQL = PrepareSQL(
      "replace into album_artist (idArtist, idAlbum, strArtist, iOrder) values(%i,%i,'%s',%i)",
      idArtist, idAlbum, strArtist.c_str(), static_cast<int>(iOrder));
  return ExecuteQuery(strSQL);
}
...
bool CMusicDatabase::AddSongGenres(int idSong, const std::vector<std::string>& genres)
{
  ...
  try
  {
    ...
    std::string strGenres = StringUtils::Join(
        ...
        CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_musicItemSeparator);
    strSQL =
        PrepareSQL("UPDATE song SET strGenres = '%s' WHERE idSong = %i", strGenres.c_str(), idSong);
    if (!ExecuteQuery(strSQL))
      return false;
...
bool CMusicDatabase::GetSongsByArtist(int idArtist, std::vector<int>& songs)
{
  try
  {
    ...
    strSQL =
        PrepareSQL("SELECT idSong FROM song_artist WHERE idArtist = %i AND idRole = 1", idArtist);
    if (!m_pDS->query(strSQL))
      return false;
    if (m_pDS->num_rows() == 0)
    {
...
bool CMusicDatabase::GetArtistsBySong(int idSong, std::vector<int>& artists)
{
  try
  {
    ...
    //Restrict to Artists only, no other roles
    strSQL =
        PrepareSQL("SELECT idArtist FROM song_artist WHERE idSong = %i AND idRole = 1", idSong);
    if (!m_pDS->query(strSQL))
      return false;
    if (m_pDS->num_rows() == 0)
    {
...
bool CMusicDatabase::GetGenresBySong(int idSong, std::vector<int>& genres)
{
  ...
  {
    std::string strSQL =
        PrepareSQL("select idGenre from song_genre where idSong = %i ORDER BY iOrder ASC", idSong);
    if (!m_pDS->query(strSQL))
      return false;
    if (m_pDS->num_rows() == 0)
    {
...
int CMusicDatabase::AddPath(const std::string& strPath1)
{
  ...
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      // doesnt exists, add it
      strSQL =
          PrepareSQL("insert into path (idPath, strPath) values( NULL, '%s' )", strPath.c_str());
      m_pDS->exec(strSQL);
...
void CMusicDatabase::GetFileItemFromArtistCredits(VECARTISTCREDITS& artistCredits, CFileItem* item)
{
  ...
  item->GetMusicInfoTag()->SetArtist(
      songartists); // Also sets ArtistDesc if empty from song.strArtist field
...
int CMusicDatabase::GetAlbumIdByPath(const std::string& strPath)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL("SELECT DISTINCT idAlbum FROM song JOIN path ON song.idPath = "
                                    "path.idPath WHERE path.strPath='%s'",
                                    strPath.c_str());
    // run query
    if (!m_pDS->query(strSQL))
      return false;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      label = StringUtils::Format(
          "A %s", m_pDS->fv(1).get_asString().c_str()); // sort label is stored in the title tag
...
bool CMusicDatabase::GetTop100AlbumSongs(const std::string& strBaseDir, CFileItemList& items)
{
  try
  {
    ...
    std::string strSQL = StringUtils::Format(
        "SELECT songview.*, albumview.* FROM songview JOIN albumview ON (songview.idAlbum = "
        "albumview.idAlbum) JOIN (SELECT song.idAlbum, SUM(song.iTimesPlayed) AS iTimesPlayedSum "
        "FROM song WHERE song.iTimesPlayed > 0 GROUP BY idAlbum ORDER BY iTimesPlayedSum DESC "
        "LIMIT 100) AS _albumlimit ON (songview.idAlbum = _albumlimit.idAlbum) ORDER BY "
        "_albumlimit.iTimesPlayedSum DESC");
...
bool CMusicDatabase::GetRecentlyPlayedAlbums(VECALBUMS& albums)
{
  try
  {
    ...
    // Get data from album and album_artist tables to fully populate albums
    std::string strSQL =
        PrepareSQL("SELECT albumview.*, albumartistview.* FROM "
                                                 ...
                                                 CFileItemList& items)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL(
        "SELECT songview.*, songartistview.* FROM "
        ...
        "JOIN songartistview ON songview.idSong = songartistview.idSong "
        "ORDER BY playedalbums.lastplayed DESC,songartistview.idsong, songartistview.idRole, "
        "songartistview.iOrder",
...
bool CMusicDatabase::GetRecentlyAddedAlbums(VECALBUMS& albums, unsigned int limit)
{
  try
  {
    ...
    std::string strSQL =
        PrepareSQL("SELECT albumview.*, albumartistview.* FROM "
                   "(SELECT idAlbum FROM album WHERE strAlbum != '' ORDER BY idAlbum DESC LIMIT "
                   "%u) AS recentalbums "
                                                ...
                                                unsigned int limit)
{
  try
  {
    ...
    std::string strSQL;
    strSQL =
        PrepareSQL("SELECT songview.*, songartistview.* FROM "
                   ...
                   "JOIN songartistview ON songview.idSong = songartistview.idSong "
                   "ORDER BY songview.idAlbum DESC, songview.idSong, songartistview.idRole, "
                   "songartistview.iOrder ",
                   limit ? limit
                         : CServiceBroker::GetSettingsComponent()
                               ->GetAdvancedSettings()
...
void CMusicDatabase::IncrementPlayCount(const CFileItem& item)
{
  try
  {
    ...
    std::string sql = PrepareSQL(
        "UPDATE song SET iTimesPlayed=iTimesPlayed+1, lastplayed=CURRENT_TIMESTAMP where idSong=%i",
        idSong);
    m_pDS->exec(sql);
  }
  catch (...)
...
bool CMusicDatabase::SearchSongs(const std::string& search, CFileItemList& items)
{
  try
  {
    ...
    if (search.size() >= MIN_FULL_SEARCH_LENGTH)
      strSQL = PrepareSQL(
          "select * from songview where strTitle like '%s%%' or strTitle like '%% %s%%' limit 1000",
          search.c_str(), search.c_str());
    else
      strSQL = PrepareSQL("select * from songview where strTitle like '%s%%' limit 1000",
                          search.c_str());
...
bool CMusicDatabase::SearchAlbums(const std::string& search, CFileItemList& albums)
{
  try
  {
    ...
    if (search.size() >= MIN_FULL_SEARCH_LENGTH)
      strSQL = PrepareSQL(
          "select * from albumview where strAlbum like '%s%%' or strAlbum like '%% %s%%'",
          search.c_str(), search.c_str());
    else
    ...
    while (!m_pDS->eof())
    {
      ...
      pItem->SetLabel(label);
      label = StringUtils::Format("B %s",
                                  album.strAlbum.c_str()); // sort label is stored in the title tag
      pItem->GetMusicInfoTag()->SetTitle(label);
      albums.Add(pItem);
      m_pDS->next();
    }
...
bool CMusicDatabase::CleanupSongsByIds(const std::string& strSongIds)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL(
        "select * from song join path on song.idPath = path.idPath where song.idSong in %s",
        strSongIds.c_str());
    if (!m_pDS->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS->num_rows();
...
bool CMusicDatabase::CleanupSongs(CGUIDialogProgress* progressDialog /*= nullptr*/)
{
  try
  {
    ...
    {
      std::string strSQL = PrepareSQL(
          "select song.idSong from song order by song.idSong limit %i offset %i", iLIMIT, i);
      if (!m_pDS->query(strSQL))
        return false;
      size_t iRowsFound = m_pDS->num_rows();
...
bool CMusicDatabase::CleanupAlbums()
{
  try
  {
    ...
    std::string strSQL =
        "select * from album where album.idAlbum not in (select idAlbum from song)";
    if (!m_pDS->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
...
bool CMusicDatabase::CleanupPaths()
{
  try
  {
    ...
    m_pDS->exec("INSERT INTO songpaths select idPath,strPath from path where idPath in (select "
                "idPath from song)\n");
...
bool CMusicDatabase::CleanupInfoSettings()
{
  try
  {
    ...
    std::string strSQL =
        "DELETE FROM infosetting WHERE idSetting NOT IN (SELECT idInfoSetting FROM artist) "
...
bool CMusicDatabase::CleanupRoles()
{
  try
  {
    ...
    std::string strSQL =
        "DELETE FROM role WHERE idRole > 1 AND idRole NOT IN (SELECT idRole FROM song_artist)";
    m_pDS->exec(strSQL);
    return true;
  }
  catch (...)
...
int CMusicDatabase::Cleanup(CGUIDialogProgress* progressDialog /*= nullptr*/)
{
  ...
  CLog::Log(LOGNOTICE, "%s: Cleaning musicdatabase done. Operation took %s", __FUNCTION__,
    ...
    bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    if (!extFilter.where.empty())
    {
      ...
      {
        extended = true;
        extFilter.AppendJoin("JOIN song_artist ON song_artist.idArtist = artistview.idArtist JOIN "
                             "songview ON songview.idSong = song_artist.idSong");
      }
      else if (extFilter.where.find("albumview") != std::string::npos)
      {
        extended = true;
...
bool CMusicDatabase::GetAlbumFromSong(int idSong, CAlbum& album)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL("select albumview.* from song join albumview on song.idAlbum = "
                                    "albumview.idAlbum where song.idSong='%i'",
                                    idSong);
    if (!m_pDS->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS->num_rows();
    ...
    bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    if (limitedInSQL)
    {
      total = strtol(GetSingleValue(PrepareSQL(strSQL, "COUNT(1)") + strSQLExtra, m_pDS).c_str(),
                     NULL, 10);
      if (sortDescription.sortBy == SortByRandom)
        strSQLExtra += PrepareSQL(" ORDER BY RANDOM()");
      strSQLExtra +=
    ...
    bool artistData /* = false*/)
{
  ...
  try
  {
    ...
    total = (int)strtol(
        GetSingleValue("SELECT COUNT(1) FROM songview " + strSQLExtra, m_pDS).c_str(), NULL, 10);
    ...
    { // Get data from song and song_artist tables to fully populate songs with artists
      ...
      if (limitedInSQL)
        ...
        strSQL = "SELECT sv.*, songartistview.* "
                 "FROM (SELECT songview.* FROM songview " +
                 strSQLExtra +
                 ") AS sv "
    ...
    CLog::Log(LOGDEBUG, "%s(%s) - took %d ms", __FUNCTION__, filter.where.c_str(),
              XbmcThreads::SystemClockMillis() - time);
    return true;
  }
  catch (...)
    ...
    const SortDescription& sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    if (extFilter.limit.empty() && sortDescription.sortBy == SortByNone &&
        (sortDescription.limitStart > 0 || sortDescription.limitEnd > 0))
    {
      total = (int)strtol(
          GetSingleValue(PrepareSQL(strSQL, "COUNT(1)") + strSQLExtra, m_pDS).c_str(), NULL, 10);
...
static const translateJSONField JSONtoDBArtist[] = {
    ...
    {"musicbrainzartistid", "array", true, "strMusicBrainzArtistId",
     ""}, // Array in schema, but only ever one element

    // Scalar subquery fields
    {"dateadded", "string", true, "dateAdded",
     "(SELECT MAX(song.dateAdded) FROM song_artist JOIN song ON song.idSong = song_artist.idSong "
     "WHERE song_artist.idArtist = artist.idArtist) AS dateAdded"},
    {"", "string", true, "artistsortname",
     "(CASE WHEN strSortName IS NOT NULL THEN strSortname ELSE strArtist END) AS artistsortname"},
    ...
    const SortDescription& sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    total = static_cast<int>(strtol(
        GetSingleValue("SELECT COUNT(1) FROM artist " + strSQLExtra, m_pDS).c_str(), NULL, 10));
    ...
    for (unsigned int i = 1; i < NUM_ARTIST_FIELDS; i++)
    {
      ...
      if (!foundJSON)
        foundOrderby = std::find(orderfields.begin(), orderfields.end(),
                                 JSONtoDBArtist[i].fieldDB) != orderfields.end();
...
static const translateJSONField JSONtoDBAlbum[] = {
    ...
    // Scalar subquery fields
    {"sourceid", "string", true, "sourceid",
     "(SELECT GROUP_CONCAT(album_source.idSource SEPARATOR '; ')  FROM album_source WHERE "
     "album_source.idAlbum = albumview.idAlbum) AS sources"},
    // Single value JOIN fields
    {"thumbnail", "image", true, "thumbnail",
     "art.url AS thumbnail"}, // or (SELECT art.url FROM art WHERE art.media_id = album.idAlbum AND art.media_type = "album" AND art.type = "thumb") as url
    ...
    {"musicbrainzalbumartistid", "array", false, "strArtistMBID",
     "artist.strMusicBrainzArtistID AS strArtistMBID"},
    {"songgenres", "array", false, "idSongGenre", "song_genre.idGenre AS idSongGenre"},
    {"", "", false, "strSongGenre", "genre.strGenre AS strSongGenre"},
    {"", "", true, "artistsortname",
     "CASE WHEN strArtistSort IS NOT NULL THEN strArtistSort ELSE strArtists END AS artistsortname"}
    /*
    ...
    const SortDescription& sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    total = static_cast<int>(
        strtol(GetSingleValue("SELECT COUNT(1) FROM albumview " + strSQLExtra, m_pDS).c_str(),
               nullptr, 10));
    ...
    else if (sortDescription.sortBy == SortByAlbum || sortDescription.sortBy == SortByLabel ||
    ...
    for (auto& name : orderfields)
    {
      ...
      else if (name.compare("strAlbum") == 0 || name.compare("strType") == 0 ||
    ...
    for (unsigned int i = 1; i < NUM_ALBUM_FIELDS; i++)
    {
      ...
      if (!foundJSON)
        foundOrderby = std::find(orderfields.begin(), orderfields.end(),
                                 JSONtoDBAlbum[i].fieldDB) != orderfields.end();
      if (foundOrderby || foundJSON)
      {
    ...
    { // All albums have songs, but left join genre as songs may not have genre
      ...
      joinLayout.SetField(joinToAlbum_strSongGenre,
                          JSONtoDBAlbum[index_idArtist + joinToAlbum_strSongGenre].SQL);
    }
    ...
    while (!m_pDS->eof() || !albumObj.empty())
    {
          ...
          !record->at(joinLayout.GetRecNo(joinToAlbum_idSongGenre)).get_isNull())
      {
        ...
        genreObj["title"] =
            record->at(joinLayout.GetRecNo(joinToAlbum_strSongGenre)).get_asString();
...
static const translateJSONField JSONtoDBSong[] = {
    ...
    {"albumreleasetype", "string", true, "strAlbumReleaseType",
     "album.strReleaseType AS strAlbumReleaseType"},
    {"musicbrainzalbumid", "string", true, "strMusicBrainzAlbumID",
    ...
    {"musicbrainzalbumartistid", "array", false, "strAlbumArtistMBID",
     "albumartist.strMusicBrainzArtistID AS strAlbumArtistMBID"},
    ...
    {"musicbrainzartistid", "array", false, "strArtistMBID",
     "songartist.strMusicBrainzArtistID AS strArtistMBID"},
    {"", "", false, "iOrderArtist", "song_artist.iOrder AS iOrderArtist"},
    {"", "", false, "idRole", "song_artist.idRole"},
    {"", "", false, "strRole", "role.strRole"},
    {"", "", false, "iOrderRole", "song_artist.iOrder AS iOrderRole"},
    {"genreid", "array", false, "idGenre",
     "song_genre.idGenre AS idGenre"}, // Not GROUP_CONCAT as can't control order
    ...
    {"sourceid", "string", true, "sourceid",
     "(SELECT GROUP_CONCAT(album_source.idSource SEPARATOR '; ') FROM album_source WHERE "
     "album_source.idAlbum = song.idAlbum) AS sources"},
    {"", "", true, "artistsortname",
     "CASE WHEN song.strArtistSort IS NOT NULL THEN song.strArtistSort ELSE song.strArtistDisp END "
     "AS artistsortname"}
    /* 
    ...
    const SortDescription& sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    total = static_cast<int>(
        strtol(GetSingleValue("SELECT COUNT(1) FROM songview " + strSQLExtra, m_pDS).c_str(),
               nullptr, 10));
    ...
    for (auto& name : orderfields)
    {
      ...
      else if (name.compare("strTitle") == 0 || name.compare("strAlbum") == 0 ||
    ...
    for (unsigned int i = 1; i < NUM_SONG_FIELDS; i++)
    {
      ...
      if (!foundJSON)
        foundOrderby = std::find(orderfields.begin(), orderfields.end(), JSONtoDBSong[i].fieldDB) !=
                       orderfields.end();
      if (foundOrderby || foundJSON)
      {
...
void CMusicDatabase::UpdateTables(int version)
{
  ...
  if (version < 67)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "Migrating specific artist scraper settings has failed, settings not transfered");
    }
    try
    {
      m_pDS->exec("INSERT INTO content_temp (idItem, strContent, strScraperPath, strSettings ) "
    ...
    {
      CLog::Log(LOGERROR,
                "Migrating specific album scraper settings has failed, settings not transfered");
    }
    try
    {
    ...
    {
      CLog::Log(LOGERROR,
                "Migrating album and artist scraper settings has failed, settings not transfered");
    }
...
size_t CMusicDatabase::GetRandomSongIDs(const Filter& filter,
...
bool CMusicDatabase::GetOldArtistPath(int idArtist, std::string& basePath)
{
  ...
  try
  {
    ...
    if (!m_pDS2->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS2->num_rows();
    if (iRowsFound == 0)
    {
                              ...
                              int id /*= -1*/)
{
  ...
  try
  {
    ...
    if (idSource < 0)
    {
      ...
      if (id > 0)
        strSQL = PrepareSQL(
            "INSERT INTO source (idSource, strName, strMultipath) VALUES(%i, '%s', '%s')", id,
            strName.c_str(), strMultipath.c_str());
      else
        strSQL = PrepareSQL(
            "INSERT INTO source (idSource, strName, strMultipath) VALUES(NULL, '%s', '%s')",
            strName.c_str(), strMultipath.c_str());
      m_pDS->exec(strSQL);
      ...
      {
        strSQL =
            PrepareSQL("INSERT INTO source_path (idSource, idPath, strPath) values(%i,%i,'%s')",
                                 ...
                                 const std::vector<std::string>& vecPaths)
{
  ...
  try
  {
    ...
    if (!pathschanged)
    {
      ...
      {
        strSQL = PrepareSQL("UPDATE source SET strName = '%s' WHERE idSource = %i", strName.c_str(),
                            idSource);
        m_pDS->exec(strSQL);
      }
      return idSource;
...
bool CMusicDatabase::AddAlbumSource(int idAlbum, int idSource)
{
  std::string strSQL;
  strSQL =
      PrepareSQL("INSERT INTO album_source (idAlbum, idSource) values(%i, %i)", idAlbum, idSource);
  return ExecuteQuery(strSQL);
}
...
bool CMusicDatabase::CheckSources(VECSOURCES& sources)
{
  ...
  size_t total =
      static_cast<size_t>(strtol(GetSingleValue("SELECT COUNT(1) FROM source").c_str(), NULL, 10));
...
int CMusicDatabase::GetAlbumByName(const std::string& strAlbum, const std::string& strArtist)
{
  try
  {
    ...
    if (strArtist.empty())
      ...
      strSQL = PrepareSQL("SELECT idAlbum FROM album WHERE album.strAlbum LIKE '%s' AND "
...
int CMusicDatabase::GetAlbumByMatch(const CAlbum& album)
{
  ...
  try
  {
    ...
    if (!album.strMusicBrainzAlbumID.empty())
      ...
      strSQL = PrepareSQL("SELECT idAlbum FROM album WHERE strArtistDisp LIKE '%s' AND strAlbum "
                          "LIKE '%s' AND strMusicBrainzAlbumID IS NULL",
...
int CMusicDatabase::GetGenreByName(const std::string& strGenre)
{
  try
  {
    ...
    std::string strSQL;
    strSQL =
        PrepareSQL("select idGenre from genre where genre.strGenre like '%s'", strGenre.c_str());
    // run query
    if (!m_pDS->query(strSQL))
      return false;
    size_t iRowsFound = m_pDS->num_rows();
...
{
  std::string strSQL = PrepareSQL("SELECT COUNT(DISTINCT idartist) FROM song_artist JOIN role ON "
                                  "song_artist.idRole = role.idRole WHERE role.strRole LIKE '%s'",
                                  strRole.c_str());
  return strtol(GetSingleValue(strSQL).c_str(), NULL, 10);
}
                                ...
                                const ADDON::ScraperPtr scraper)
{
  ...
  try
  {
    ...
    { // Update info setting
      strSQL =
          "UPDATE infosetting SET strScraperPath = '%s', strSettings = '%s' WHERE idSetting = %i";
      strSQL =
...
bool CMusicDatabase::GetScraper(int id, const CONTENT_TYPE& content, ADDON::ScraperPtr& scraper)
{
  ...
  try
  {
    ...
    if (content == CONTENT_ARTISTS)
      strSQL = strSQL +
               "artist ON artist.idInfoSetting = infosetting.idSetting WHERE artist.idArtist = %i";
    else
      strSQL =
          strSQL + "album ON album.idInfoSetting = infosetting.idSetting WHERE album.idAlbum = %i";
    strSQL = PrepareSQL(strSQL, id);
    m_pDS->query(strSQL);
    if (!m_pDS->eof())
    { // try and ascertain scraper
      ...
      if (!scraperUUID.empty() && CServiceBroker::GetAddonMgr().GetAddon(scraperUUID, addon) &&
          addon)
      {
                                 ...
                                 CGUIDialogProgress* progressDialog /*= nullptr*/)
{
  ...
  try
  {
    ...
    if (settings.IsItemExported(ELIBEXPORT_ALBUMS) && !artistfoldersonly)
    {
      ...
      int total = static_cast<int>(m_pDS->num_rows());
      int current = 0;
    ...
    if ((settings.IsArtists() || artistfoldersonly) && !strFolder.empty())
    {
      ...
      if (settings.IsItemExported(ELIBEXPORT_SONGARTISTS))
      {
        if (settings.IsItemExported(ELIBEXPORT_OTHERARTISTS))
          filter.AppendWhere(
              "EXISTS (SELECT 1 FROM song_artist WHERE song_artist.idArtist = artist.idArtist )",
              false);
        else
          filter.AppendWhere("EXISTS (SELECT 1 FROM song_artist WHERE song_artist.idArtist = "
                             "artist.idArtist AND song_artist.idRole = 1)",
                             false);
      }
      ...
      for (const auto& artistId : artistIds)
      {
        ...
        GetArtist(artistId, artist,
                  !artistfoldersonly); // include discography when not folders only
        std::string strPath;
        std::map<std::string, std::string> artwork;
        if (settings.IsSingleFile())
        {
        ...
        if ((current % 50) == 0 && progressDialog)
        {
          ...
          progressDialog->SetPercentage(current * 100 / static_cast<int>(total));
          if (progressDialog->IsCanceled())
            return;
        }
        current++;
...
void CMusicDatabase::ImportFromXML(const std::string& xmlFile, CGUIDialogProgress* progressDialog)
{
  try
  {
    ...
    while (entry)
    {
      if (strnicmp(entry->Value(), "artist", 6) == 0 || strnicmp(entry->Value(), "album", 5) == 0)
        total++;
                                       ...
                                       CGUIDialogProgress* progressDialog)
{
  ...
  try
  {
    ...
    */
    ...
    strSQL = "UPDATE HistSong "
             "SET idAlbum = (SELECT album.idAlbum FROM album "
             "WHERE HistSong.strAlbumArtistDisp = album.strArtistDisp AND HistSong.strAlbum = "
             "album.strAlbum) "
             "WHERE EXISTS(SELECT 1 FROM album "
             "WHERE HistSong.strAlbumArtistDisp = album.strArtistDisp AND HistSong.strAlbum = "
             "album.strAlbum)"
    ...
    int unmatched = static_cast<int>(
        strtol(GetSingleValue("SELECT COUNT(1) FROM HistSong WHERE idSong < 0", m_pDS).c_str(),
               nullptr, 10));
                                   ...
                                   const std::string& url)
{
  try
  {
    ...
    std::string sql =
        PrepareSQL("SELECT art_id FROM art WHERE media_id=%i AND media_type='%s' AND type='%s'",
...
bool CMusicDatabase::GetFilter(CDbUrl& musicUrl, Filter& filter, SortDescription& sorting)
{
  ...
  else if (type == "songs" || type == "singles")
  {
    ...
    if (option != options.end())
      filter.AppendWhere(
          PrepareSQL("songview.iYear = %i", static_cast<int>(option->second.asInteger())));
    ...
    { // Artist name is not unique, so could get songs from more than one.
      ...
      albumArtistClause = PrepareSQL(
          "EXISTS (SELECT 1 FROM album_artist JOIN artist ON artist.idArtist = "
...
void CMusicDatabase::UpdateFileDateAdded(int songId, const std::string& strFileNameAndPath)
{
  ...
  try
  {
    ...
    if (CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_iMusicLibraryDateAdded ==
        1)
    ...
    m_pDS->exec(PrepareSQL("UPDATE song SET dateAdded='%s' WHERE idSong=%d",
                           dateAdded.GetAsDBDateTime().c_str(), songId));
  }
  catch (...)
  {
    CLog::Log(LOGERROR, "%s (%s, %s) failed", __FUNCTION__,
              CURL::GetRedacted(strFileNameAndPath).c_str(), dateAdded.GetAsDBDateTime().c_str());
  }
}
...
bool CMusicDatabase::SetResumeBookmarkForAudioBook(const CFileItem& item, int bookmark)
{
  ...
  strSQL = PrepareSQL("UPDATE audiobook SET bookmark=%i WHERE file='%s'", bookmark,
