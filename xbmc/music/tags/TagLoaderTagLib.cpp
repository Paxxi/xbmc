...
bool CTagLoaderTagLib::ParseTag(ASF::Tag *asf, EmbeddedArt *art, CMusicInfoTag& tag)
{
  ...
  for (const auto & it : attributeListMap)
  {
    if (it.first == "Author")
...
bool CTagLoaderTagLib::ParseTag(ID3v2::Tag *id3v2, MUSIC_INFO::EmbeddedArt *art, MUSIC_INFO::CMusicInfoTag& tag)
{
  ...
  for (auto & picture : pictures)
    if (picture)
    {
      ...
      if (art)
        art->set((const uint8_t*)picture->picture().data(), size, mime);
...
bool CTagLoaderTagLib::ParseTag(Ogg::XiphComment *xiph, EmbeddedArt *art, CMusicInfoTag& tag)
{
  ...
  for (const auto & it : fieldListMap)
  {
    if (it.first == "ARTIST")
  ...
  for (auto & picture : pictures)
    ...
    {
      std::string mime = picture.mimeType().toCString();
      if (mime.compare(0, 6, "image/") != 0)
        continue;
      TagLib::uint size =            picture.data().size();
      tag.SetCoverArtInfo(size, mime);
      if (art)
        art->set((const uint8_t*)picture.data().data(), size, mime);
...
void CTagLoaderTagLib::SetFlacArt(FLAC::File *flacFile, EmbeddedArt *art, CMusicInfoTag &tag)
{
  ...
  for (auto & i : cover)
  {
    if (i)
    {
      tag.SetCoverArtInfo(i->data().size(), i->mimeType().to8Bit(true));
      if (art)
        art->set((const uint8_t*)i->data().data(), i->data().size(), i->mimeType().to8Bit(true));
...
void CTagLoaderTagLib::SetGenre(CMusicInfoTag &tag, const std::vector<std::string> &values)
{
  ...
  {
    std::string genre = value;
    if (StringUtils::IsNaturalNumber(genre))
    {
      int number = strtol(value.c_str(), nullptr, 10);
...
void CTagLoaderTagLib::AddArtistInstrument(CMusicInfoTag &tag, const std::vector<std::string> &values)
{
  ...
  for (const auto & value : values)
  {
    ...
    unsigned firstLim = value.find_first_of("(");
    unsigned lastLim = value.find_last_of(")");
    if (lastLim != std::string::npos && firstLim != std::string::npos && firstLim < lastLim - 1)
    {
      //Pair of brackets with something between them
      strRole = value.substr(firstLim + 1, lastLim - firstLim - 1);
