...
bool CEventClient::AddPacket(CEventPacket *packet)
{
  ...
  if ( packet->Size() > 1 )
  {
    ...
    if (m_seqPackets.size() == packet->Size())
    {
      ...
      if (newPayload)
      {
        unsigned char *payloadPtr = reinterpret_cast<unsigned char *>(newPayload);
...
bool CEventClient::OnPacketHELO(CEventPacket *packet)
{
  ...
  unsigned char *payload = reinterpret_cast<unsigned char *>(packet->Payload());
...
bool CEventClient::OnPacketBUTTON(CEventPacket *packet)
{
  ...
  float famount = 0;
  bool active = static_cast<bool>((flags & PTB_DOWN));
  ...
  if(flags & PTB_QUEUE)
  {
    ...
    CEventButtonState state( keycode,
                             ...
                             button,
                             famount,
                             static_cast<bool>((flags & (PTB_AXIS|PTB_AXISSINGLE))),
                             !static_cast<bool>((flags & PTB_NO_REPEAT)),
                             static_cast<bool>((flags & PTB_USE_AMOUNT)) );
  ...
  else
  {
    ...
    if ( flags & PTB_DOWN )
    {
      ...
      m_currentButton.m_bRepeat    = !static_cast<bool>((flags & PTB_NO_REPEAT));
      m_currentButton.m_bAxis      = static_cast<bool>((flags & PTB_AXIS));
...
bool CEventClient::ParseString(unsigned char* &payload, int &psize, std::string& parsedVal)
{
  ...
  unsigned char *pos = reinterpret_cast<unsigned char *>(memchr((void*)payload, static_cast<int>('\0'), psize));
  if (!pos) {
    return false;
}
...
{
  // 60 seconds timeout
  return (time(nullptr) - m_lastPing) <= 60;
}
