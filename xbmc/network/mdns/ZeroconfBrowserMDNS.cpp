                                                     ...
                                                     void* context)
{
  ...
  if (errorCode == kDNSServiceErr_NoError)
  {
    ...
    if (flags & kDNSServiceFlagsAdd)
    {
      CLog::Log(
          LOGDEBUG,
          "ZeroconfBrowserMDNS::BrowserCallback found service named: %s, type: %s, domain: %s",
    ...
    {
      CLog::Log(LOGDEBUG,
                "ZeroconfBrowserMDNS::BrowserCallback service named: %s, type: %s, domain: %s "
                "disappeared",
                s.GetName().c_str(), s.GetType().c_str(), s.GetDomain().c_str());
    ...
    if (!(flags & kDNSServiceFlagsMoreComing))
    {
      ...
      CLog::Log(LOGDEBUG,
                "ZeroconfBrowserMDNS::BrowserCallback sent gui update for path zeroconf://");
    }
  }
  else
  {
...
void CZeroconfBrowserMDNS::addDiscoveredService(
    DNSServiceRef browser, CZeroconfBrowser::ZeroconfService const& fcr_service)
{
  CSingleLock lock(m_data_guard);
...
void CZeroconfBrowserMDNS::removeDiscoveredService(
    DNSServiceRef browser, CZeroconfBrowser::ZeroconfService const& fcr_service)
{
  CSingleLock lock(m_data_guard);
  tDiscoveredServicesMap::iterator browserIt = m_discovered_services.find(browser);
...
bool CZeroconfBrowserMDNS::doAddServiceType(const std::string& fcr_service_type)
{
...
#if defined(TARGET_WINDOWS_DESKTOP)
    err = WSAAsyncSelect((SOCKET)DNSServiceRefSockFD(m_browser), g_hWnd, BONJOUR_BROWSER_EVENT,
                         FD_READ | FD_CLOSE);
    if (err != kDNSServiceErr_NoError)
...

  {
    CSingleLock lock(m_data_guard);
    browser = m_browser;
    err = DNSServiceBrowse(&browser, kDNSServiceFlagsShareConnection, kDNSServiceInterfaceIndexAny,
                           fcr_service_type.c_str(), NULL, BrowserCallback, this);
  }
...
bool CZeroconfBrowserMDNS::doResolveService(CZeroconfBrowser::ZeroconfService& fr_service,
                                            double f_timeout)
{
  DNSServiceErrorType err;
  ...
  err = DNSServiceResolve(&sdRef, 0, kDNSServiceInterfaceIndexAny, fr_service.GetName().c_str(),
                          fr_service.GetType().c_str(), fr_service.GetDomain().c_str(),
  ...
  if (!fr_service.GetHostname().empty())
  {
    ...
    err = DNSServiceGetAddrInfo(&sdRef, 0, kDNSServiceInterfaceIndexAny, kDNSServiceProtocol_IPv4,
                                fr_service.GetHostname().c_str(), GetAddrInfoCallback, this);
    ...
    {
      CLog::Log(LOGWARNING,
                "ZeroconfBrowserMDNS: Could not resolve hostname %s falling back to CDNSNameCache",
                fr_service.GetHostname().c_str());
      if (CDNSNameCache::Lookup(fr_service.GetHostname(), strIP))
        fr_service.SetIP(strIP);
      else
        CLog::Log(LOGERROR, "ZeroconfBrowserMDNS: Could not resolve hostname %s",
                  fr_service.GetHostname().c_str());
    }
  }
