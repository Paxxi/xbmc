...
public:
  ...
  const std::string& Name() const { return m_deviceName; }
  ...
  SOCKETS::CAddress& Address() { return m_remoteAddr; }

  virtual ~CEventClient() { FreePacketQueues(); }
...
protected:
  ...
  void ResetTimeout() { m_lastPing = time(NULL); }
