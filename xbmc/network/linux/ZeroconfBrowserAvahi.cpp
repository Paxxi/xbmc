...
CZeroconfBrowserAvahi::CZeroconfBrowserAvahi() : mp_client ( nullptr ), mp_poll ( nullptr ), m_shutdown(false), m_thread_id(0)
{
...
std::vector<CZeroconfBrowser::ZeroconfService> CZeroconfBrowserAvahi::doGetFoundServices()
{
  ...
  for (auto & m_discovered_service : m_discovered_services)
    ret.push_back ( m_discovered_service.first );
  return ret;
}
...
void CZeroconfBrowserAvahi::clientCallback ( AvahiClient* fp_client, AvahiClientState f_state, void* fp_data )
{
  ...
  switch ( f_state )
  {
    ...
    case AVAHI_CLIENT_FAILURE:
    {
      ...
      for (auto & m_browser : p_instance->m_browsers)
        m_browser.second = ( AvahiServiceBrowser* ) nullptr;
