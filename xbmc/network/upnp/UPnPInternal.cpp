                                 ...
                                 UPnPService service /* = UPnPServiceNone */)
{
  ...
  {
    object.m_ReferenceID = NPT_String::Format("musicdb://songs/%i%s", tag.GetDatabaseId(),
                                              URIUtils::GetExtension(tag.GetURL()).c_str());
  }
  ...
  if (upnp_server != NULL && item.IsVideo() &&
      (upnp_service == UPnPPlayer || upnp_service == UPnPRenderer ||
       CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
           CSettings::SETTING_SERVICES_UPNPLOOKFOREXTERNALSUBTITLES)))
  {
...
CFileItemPtr BuildObject(PLT_MediaObject* entry, UPnPService upnp_service /* = UPnPServiceNone */)
{
  ...
  {
    SYSTEMTIME time = {};
    sscanf(entry->m_Description.date, "%hu-%hu-%huT%hu:%hu:%hu", &time.wYear, &time.wMonth,
...
bool GetResource(const PLT_MediaObject* entry, CFileItem& item)
{
  ...
  for (unsigned r = 0; r < entry->m_Resources.GetItemCount(); r++)
  {
    ...
    static const char* allowed[] = {"text/srt", "text/ssa", "text/sub", "text/idx"};
