...
|   CUPnPServer::Build
+---------------------------------------------------------------------*/
PLT_MediaObject* CUPnPServer::Build(CFileItemPtr item,
                                    bool with_count,
  ...
  else
  {
    ...
    if (flag == VideoLibrary)
    {
      if (item_type == MediaTypeEpisode)
      {
        ...
        UpdateContainer(StringUtils::Format("videodb://tvshows/titles/%d/%d/?tvshowid=%d", show_id,
                                            season_id, show_id));
                                      ...
                                      const char* parent_id /* = NULL */)
{
  ...
  CLog::Log(LOGDEBUG, "Returning UPnP response with %d items out of %d total matches", count,
            total);
...
NPT_Result CUPnPServer::OnSearchContainer(PLT_ActionReference& action,
                                          ...
                                          const PLT_HttpRequestContext& context)
{
  CLog::Log(LOGDEBUG, "Received Search request for object '%s' with search '%s'", object_id,
            search_criteria);
  ...
  else if (NPT_String(search_criteria).Find("object.item.audioItem") >= 0)
  {
    ...
    artist =
        artist.GetLength() ? artist : FindSubCriteria(search_criteria, "microsoft:artistPerformer");
    artist = artist.GetLength() ? artist
                                : FindSubCriteria(search_criteria, "microsoft:artistAlbumArtist");
    artist =
        artist.GetLength() ? artist : FindSubCriteria(search_criteria, "microsoft:authorComposer");
    ...
    if (genre.GetLength() > 0)
    {
      ...
      std::string strPath = StringUtils::Format(
          "musicdb://genres/%i/%i/%i/", database.GetGenreByName((const char*)genre),
    ...
    else if (artist.GetLength() > 0)
    {
      ...
      std::string strPath = StringUtils::Format(
          "musicdb://artists/%i/%i/", database.GetArtistByName((const char*)artist),
          database.GetAlbumByName((const char*)album)); // will return -1 if no album
    ...
    else if (album.GetLength() > 0)
    {
      ...
      std::string strPath =
          StringUtils::Format("musicdb://albums/%i/", database.GetAlbumByName((const char*)album));
  ...
  else if (NPT_String(search_criteria).Find("object.container.album.musicAlbum") >= 0)
  {
    ...
    {
      std::string strPath = StringUtils::Format(
          "musicdb://genres/%i/%i/", database.GetGenreByName((const char*)genre),
  ...
  else if (NPT_String(search_criteria).Find("object.container.person.musicArtist") >= 0)
  {
    ...
    if (genre.GetLength() > 0)
    {
      ...
      database.Open();
      std::string strPath =
          StringUtils::Format("musicdb://genres/%i/", database.GetGenreByName((const char*)genre));
      return OnBrowseDirectChildren(action, strPath.c_str(), filter, starting_index,
                                    requested_count, sort_criteria, context);
    }
  ...
  else if (NPT_String(search_criteria).Find("object.item.videoItem") >= 0)
  {
    ...
    if (!database.GetMusicVideosByWhere("videodb://musicvideos/titles/", CDatabase::Filter(),
                                        items))
    {
      action->SetError(800, "Internal Error");
      return NPT_SUCCESS;
    ...
    return BuildResponse(action, itemsall, filter, starting_index, requested_count, sort_criteria,
                         context, NULL);
  }
                                  ...
                                  const NPT_String& md5)
{
  ...
  NPT_String file_path(md5), *file_path2;
  {
    ...
    if (NPT_SUCCEEDED(m_FileMap.Get(md5, file_path2)))
    {
      ...
      CLog::Log(LOGDEBUG, "Received request to serve '%s' = '%s'", (const char*)md5,
                (const char*)file_path);
    }
    else
    {
...
bool CUPnPServer::SortItems(CFileItemList& items, const char* sort_criteria)
{
  ...
  for (std::vector<std::string>::reverse_iterator itr = tokens.rbegin(); itr != tokens.rend();
       ++itr)
  {
    SortDescription sorting;
    /* Platinum guarantees 1st char is - or + */
    sorting.sortOrder =
        StringUtils::StartsWith(*itr, "+") ? SortOrderAscending : SortOrderDescending;
    ...
    CLog::Log(LOGINFO, "UPnP: Sorting by method %d, order %d, attributes %d", sorting.sortBy,
              sorting.sortOrder, sorting.sortAttributes);
    items.Sort(sorting);
    sorted = true;
  }
...
NPT_Result CUPnPServer::AddSubtitleUriForSecResponse(NPT_String movie_md5, NPT_String subtitle_uri)
{
