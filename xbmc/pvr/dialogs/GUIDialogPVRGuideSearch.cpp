...
void CGUIDialogPVRGuideSearch::UpdateGroupsSpin(void)
{
  ...
  for (std::vector<CPVRChannelGroupPtr>::const_iterator it = groups.begin(); it != groups.end();
       ++it)
...
void CGUIDialogPVRGuideSearch::UpdateGenreSpin(void)
{
  ...
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19516), EPG_EVENT_CONTENTMASK_NEWSCURRENTAFFAIRS));
  ...
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19564), EPG_EVENT_CONTENTMASK_CHILDRENYOUTH));
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19580), EPG_EVENT_CONTENTMASK_MUSICBALLETDANCE));
  labels.push_back(std::make_pair(g_localizeStrings.Get(19596), EPG_EVENT_CONTENTMASK_ARTSCULTURE));
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19612), EPG_EVENT_CONTENTMASK_SOCIALPOLITICALECONOMICS));
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19628), EPG_EVENT_CONTENTMASK_EDUCATIONALSCIENCE));
  labels.push_back(
      std::make_pair(g_localizeStrings.Get(19644), EPG_EVENT_CONTENTMASK_LEISUREHOBBIES));
...
void CGUIDialogPVRGuideSearch::UpdateDurationSpin(void)
{
  ...
  for (int i = 1; i < 12 * 60 / 5; ++i)
    labels.push_back(
        std::make_pair(StringUtils::Format(g_localizeStrings.Get(14044).c_str(), i * 5), i * 5));
...
void CGUIDialogPVRGuideSearch::OnSearch()
{
  ...
  m_searchFilter->SetChannelNumber(it == m_channelNumbersMap.end() ? CPVRChannelNumber()
                                                                   : (*it).second);
...
void CGUIDialogPVRGuideSearch::Update()
{
  ...
  /* Set time fields */
  SET_CONTROL_LABEL2(CONTROL_EDIT_START_TIME,
  ...
  SET_CONTROL_LABEL2(CONTROL_EDIT_STOP_TIME,
