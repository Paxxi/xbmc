      ...
      if (strGroupName != "")
      {
        ...
        if (groups->AddGroup(strGroupName))
        {
          CServiceBroker::GetPVRManager()
              ...
              ->SetGroupType(PVR_GROUP_TYPE_USER_DEFINED);
  ...
  if (m_selectedGroup)
  {
    ...
    if (m_selectedGroup->IsInternalGroup())
    {
      std::string strNewLabel = StringUtils::Format(
          "%s %s", g_localizeStrings.Get(19022).c_str(),
      ...
      strNewLabel = StringUtils::Format("%s %s", g_localizeStrings.Get(19218).c_str(),
    ...
    else
    {
      ...
      strNewLabel = StringUtils::Format("%s %s", g_localizeStrings.Get(19220).c_str(),
    ...
    else
    {
      ...
      const CPVRChannelGroupPtr allGroup =
          CServiceBroker::GetPVRManager().ChannelGroups()->GetGroupAll(m_bIsRadio);
      const std::vector<PVRChannelGroupMember> allGroupMembers =
