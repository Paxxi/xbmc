...
int CPVREpgDatabase::Persist(const CPVREpgInfoTag& tag, bool bSingleUpdate /* = true */)
{
  ...
  if (iBroadcastId < 0)
  {
    strQuery = PrepareSQL(
        ...
        strGenre.c_str(), static_cast<unsigned int>(iFirstAired), tag.ParentalRating(),
        tag.StarRating(), false /* unused */, tag.SeriesNumber(), tag.EpisodeNumber(),
        tag.EpisodePart(), tag.EpisodeName().c_str(), tag.Flags(), tag.SeriesLink().c_str(),
