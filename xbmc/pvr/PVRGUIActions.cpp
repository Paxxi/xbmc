...
{
public:
  explicit AsyncRenameRecording(const std::string& strNewName)
...
private:
  bool DoRun(const std::shared_ptr<CFileItem>& item) override
  {
    ...
    {
      CLog::LogF(LOGERROR, "Cannot rename item '%s': no valid recording tag",
                 item->GetPath().c_str());
      return false;
    }
  }
  ...
  bool DoRun(const std::shared_ptr<CFileItem>& item) override
  {
    ...
    for (const auto& itemToDelete : items)
    {
      if (itemToDelete->IsUsablePVRRecording())
        ...
        CLog::LogF(LOGERROR, "Cannot delete item '%s': no valid recording tag",
                   itemToDelete->GetPath().c_str());
    }
    return bReturn;
  }
};
...
private:
  bool DoRun(const std::shared_ptr<CFileItem>& item) override
  {
    ...
    {
      CLog::LogF(LOGERROR, "Cannot undelete item '%s': no valid recording tag",
                 item->GetPath().c_str());
      return false;
    }
  }
...
bool CPVRGUIActions::FindSimilar(const CFileItemPtr& item,
                                 CGUIWindow* windowToClose /* = nullptr */) const
{
  ...
  if (bRadio)
    windowSearch = CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIWindowPVRRadioSearch>(
        windowSearchId);
  else
    windowSearch = CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIWindowPVRTVSearch>(
        windowSearchId);
...
bool CPVRGUIActions::AddTimerRule(const std::shared_ptr<CFileItem>& item,
...
bool CPVRGUIActions::AddTimer(const std::shared_ptr<CFileItem>& item,
                              bool bCreateRule,
                              bool bShowTimerSettings,
                              bool bFallbackToOneShotTimer) const
{
  ...
  CPVRTimerInfoTagPtr newTimer(epgTag ? CPVRTimerInfoTag::CreateFromEpg(epgTag, bCreateRule)
                                               ...
                                               const std::string& title)
{
  if (m_actions.find(eAction) == m_actions.end())
  {
    ...
    case RECORD_INSTANTRECORDTIME:
      m_pDlgSelect->Add(
          StringUtils::Format(g_localizeStrings.Get(19090).c_str(),
                              m_iInstantRecordTime)); // Record next <default duration> minutes
      break;
    case RECORD_30_MINUTES:
      m_pDlgSelect->Add(
          StringUtils::Format(g_localizeStrings.Get(19090).c_str(), 30)); // Record next 30 minutes
      break;
    case RECORD_60_MINUTES:
      m_pDlgSelect->Add(
          StringUtils::Format(g_localizeStrings.Get(19090).c_str(), 60)); // Record next 60 minutes
      break;
    ...
    case RECORD_CURRENT_SHOW:
      m_pDlgSelect->Add(StringUtils::Format(g_localizeStrings.Get(19091).c_str(),
                                            title.c_str())); // Record current show (<title>)
      break;
    case RECORD_NEXT_SHOW:
      m_pDlgSelect->Add(StringUtils::Format(g_localizeStrings.Get(19092).c_str(),
                                            title.c_str())); // Record next show (<title>)
      break;
    case NONE:
    case ASK:
    default:
    ...
    if (bOnOff && !CServiceBroker::GetPVRManager().Timers()->IsRecordingOnChannel(*channel))
    {
      ...
      case ASK:
      {
        ...
        if (epgTag)
        {
          ...
          if (epgTagNext)
          {
            const std::string nextTitle =
                bLocked ? g_localizeStrings.Get(19266) /* Parental locked */ : epgTagNext->Title();
        ...
        default:
          CLog::LogF(
              LOGERROR,
              "Unknown instant record action selection (%d), defaulting to fixed length recording.",
      ...
      default:
        CLog::LogF(LOGERROR,
                   "Unknown instant record action setting value (%d), defaulting to fixed length "
                   "recording.",
                   iAction);
        break;
      }

      const CPVRTimerInfoTagPtr newTimer(
...
std::shared_ptr<CFileItem> CPVRGUIActions::GetTimerRule(
...
bool CPVRGUIActions::DeleteTimer(const CFileItemPtr& item,
                                 bool bIsRecording,
...
bool CPVRGUIActions::DeleteTimer(const CPVRTimerInfoTagPtr& timer,
                                 bool bIsRecording,
                                 bool bDeleteRule) const
{
...
bool CPVRGUIActions::ConfirmDeleteTimer(const CPVRTimerInfoTagPtr& timer, bool& bDeleteRule) const
{
  ...
  const CPVRTimerInfoTagPtr parentTimer(
...
bool CPVRGUIActions::RenameRecording(const CFileItemPtr& item) const
{
  ...
  if (!CGUIKeyboardFactory::ShowAndGetInput(strNewName, CVariant{g_localizeStrings.Get(19041)},
                                            false))
    return false;
...
bool CPVRGUIActions::SwitchToChannel(const CFileItemPtr& item, bool bCheckResume) const
{
  ...
  if (channel)
  {
    ...
    if (!bSwitchToFullscreen)
    {
      ...
      bSwitchToFullscreen =
          recording && CServiceBroker::GetPVRManager().IsPlayingRecording(recording);
    }
  ...
  if (result == ParentalCheckResult::SUCCESS)
  {
    ...
    StartPlayback(new CFileItem(channel),
                  m_settings.GetBoolValue(CSettings::SETTING_PVRPLAYBACK_SWITCHTOFULLSCREEN));
    return true;
  }
  else if (result == ParentalCheckResult::FAILED)
  {
    ...
    const std::string msg = StringUtils::Format(
        g_localizeStrings.Get(19035).c_str(),
        channelName.c_str()); // CHANNELNAME could not be played. Check the log for details.
...
bool CPVRGUIActions::SwitchToChannel(PlaybackType type) const
{
  ...
  else
  {
    ...
    const CPVRChannelGroupPtr channelGroup(
        CServiceBroker::GetPVRManager().GetPlayingGroup(bIsRadio));
    if (channelGroup)
    {
  ...
  CLog::LogF(LOGERROR,
             "Could not determine %s channel to playback. No last played channel found, and first "
             "channel of active group could also not be determined.",
             bIsRadio ? "Radio" : "TV");
  ...
  CGUIDialogKaiToast::QueueNotification(
      ...
      StringUtils::Format(
          g_localizeStrings.Get(19035).c_str(),
          g_localizeStrings.Get(bIsRadio ? 19021 : 19020)
              .c_str())); // Radio/TV could not be played. Check the log for details.
  return false;
}
...
bool CPVRGUIActions::ResetPVRDatabase(bool bResetEPGOnly)
{
  ...
  else
  {
    if (CheckParentalPIN() != ParentalCheckResult::SUCCESS ||
        !CGUIDialogYesNo::ShowAndGetInput(
            CVariant{19098}, // "Warning!"
            CVariant{
                19186})) // "All your TV related data (channels, groups, guide) will be cleared. Are you sure?"
      return false;
  }
  ...
  if (CServiceBroker::GetPVRManager().IsPlaying())
  {
    CLog::Log(LOGNOTICE, "PVR is stopping playback for %s database reset",
  ...
  const CPVREpgDatabasePtr epgDatabase(
  ...
  CLog::Log(LOGNOTICE, "Restarting the PVR Manager after %s database reset",
            bResetEPGOnly ? "EPG" : "PVR and EPG");
        ...
        if (cause)
        {
          if (cause->IsRecording())
          {
            text = StringUtils::Format(
                g_localizeStrings.Get(19691).c_str(), // "PVR is currently recording...."
                cause->Title().c_str(), cause->ChannelName().c_str());
          }
          else
          {
            ...
            text = StringUtils::Format(
                ...
                cause->Title().c_str(), cause->ChannelName().c_str(), dueStr.c_str());
          }
        }
        else
        {
          ...
          text = StringUtils::Format(
              g_localizeStrings.Get(19693).c_str(), // "Daily wakeup is due in...."
              dueStr.c_str());
        }
...
void CPVRChannelSwitchingInputHandler::GetChannelNumbers(std::vector<std::string>& channelNumbers)
{
  ...
  if (playingChannel)
  {
    const CPVRChannelGroupPtr group =
...
void CPVRChannelSwitchingInputHandler::SwitchToPreviousChannel()
{
  if (CServiceBroker::GetPVRManager().IsPlaying())
  {
    ...
    {
      const CPVRChannelGroupPtr group(
          CServiceBroker::GetPVRManager().ChannelGroups()->GetPreviousPlayedGroup());
      if (group)
      {
        CServiceBroker::GetPVRManager().SetPlayingGroup(group);
        const std::shared_ptr<CPVRChannel> channel =
            group->GetLastPlayedChannel(playingChannel->ChannelID());
        if (channel)
        {
