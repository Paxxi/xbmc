...
CPVRTimerTypePtr CPVRTimerType::CreateFromAttributes(unsigned int iMustHaveAttr,
                                                     unsigned int iMustNotHaveAttr,
                                                     int iClientId)
{
  const std::vector<std::shared_ptr<CPVRTimerType>> types = GetAllTypes();
  for (const auto& type : types)
  ...
  CLog::LogF(LOGERROR, "Unable to resolve timer type (0x%x, 0x%x, %d)", iMustHaveAttr,
...
void CPVRTimerType::InitDescription()
{
  ...
  m_strDescription =
      StringUtils::Format(g_localizeStrings.Get(prefixId).c_str(), m_strDescription.c_str());
}
...
void CPVRTimerType::InitLifetimeValues(const PVR_TIMER_TYPE& type)
{
  ...
  else if (SupportsLifetime())
  {
    ...
    for (int i = 1; i < 366; ++i)
    {
      m_lifetimeValues.push_back(std::make_pair(
...
void CPVRTimerType::InitPreventDuplicateEpisodesValues(const PVR_TIMER_TYPE& type)
{
  if (type.iPreventDuplicateEpisodesSize > 0)
  {
    for (unsigned int i = 0; i < type.iPreventDuplicateEpisodesSize; ++i)
    {
      ...
      m_preventDupEpisodesValues.push_back(
          std::make_pair(strDescr, type.preventDuplicateEpisodes[i].iValue));
    }
  ...
  else if (SupportsRecordOnlyNewEpisodes())
  {
    ...
    m_preventDupEpisodesValues.push_back(
        std::make_pair(g_localizeStrings.Get(815), 0)); // "Record all episodes"
    m_preventDupEpisodesValues.push_back(
        std::make_pair(g_localizeStrings.Get(816), 1)); // "Record only new episodes"
