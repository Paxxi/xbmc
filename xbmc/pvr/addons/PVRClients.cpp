...
void CPVRClients::UpdateAddons(const std::string& changedAddonId /*= ""*/)
{
  ...
  if (!addonsToCreate.empty() || !addonsToReCreate.empty() || !addonsToDestroy.empty())
  {
    ...
    for (const auto& addon : addonsToCreate)
    {
      ...
      {
        CLog::LogF(LOGERROR, "Failed to create add-on %s, status = %d", addon.first->Name().c_str(),
                   status);
        if (status == ADDON_STATUS_PERMANENT_FAILURE)
        {
          CServiceBroker::GetAddonMgr().DisableAddon(addon.first->ID());
          CJobManager::GetInstance().AddJob(new CPVREventlogJob(true, true, addon.first->Name(),
...
PVR_ERROR CPVRClients::GetCreatedClients(CPVRClientMap& clientsReady,
...
bool CPVRClients::GetTimers(CPVRTimersContainer* timers, std::vector<int>& failedClients)
{
  return ForCreatedClients(
             __FUNCTION__,
             [timers](const CPVRClientPtr& client) { return client->GetTimers(timers); },
...
PVR_ERROR CPVRClients::GetChannels(CPVRChannelGroupInternal* group, std::vector<int>& failedClients)
{
  return ForCreatedClients(
      __FUNCTION__,
      [group](const CPVRClientPtr& client) {
...
PVR_ERROR CPVRClients::GetChannelGroups(CPVRChannelGroups* groups, std::vector<int>& failedClients)
{
  return ForCreatedClients(
      __FUNCTION__,
      [groups](const CPVRClientPtr& client) { return client->GetChannelGroups(groups); },
      failedClients);
}
                                              ...
                                              std::vector<int>& failedClients)
{
  return ForCreatedClients(
      __FUNCTION__,
      [group](const CPVRClientPtr& client) { return client->GetChannelGroupMembers(group); },
      failedClients);
}
...
PVR_ERROR CPVRClients::ForCreatedClients(const char* strFunctionName,
                                         PVRClientFunction function) const
{
  std::vector<int> failedClients;
                                         ...
                                         std::vector<int>& failedClients) const
{
  ...
  for (const auto& clientEntry : clients)
  {
    ...
    {
      CLog::LogFunction(LOGERROR, strFunctionName, "PVR client '%s' returned an error: %s",
                        clientEntry.second->GetFriendlyName().c_str(),
                        CPVRClient::ToString(currentError));
