                       ...
                       CFileItemList& results)
{
  ...
  for (const auto& recording : recordings)
  {
    ...
    const std::string strCurrent =
        recParentPath.GetUnescapedSubDirectoryPath(recording->m_strDirectory);
    if (strCurrent.empty())
      continue;
...
bool CPVRGUIDirectory::FilterDirectory(CFileItemList& results) const
{
  if (!results.IsEmpty())
  {
    if (m_url.HasOption("view"))
    {
      ...
      {
        CLog::LogF(LOGERROR, "Unsupported value '%s' for channel list URL parameter 'view'",
                   view.c_str());
        return false;
      }
    }
...
bool CPVRGUIDirectory::GetChannelsDirectory(CFileItemList& results) const
{
  ...
  if (path.IsValid())
  {
    ...
    else if (path.IsChannelGroup())
    {
      ...
      {
        CLog::LogF(LOGERROR, "Unable to obtain members of channel group '%s'",
                   strGroupName.c_str());
        return false;
      }
                            ...
                            CFileItemList& results)
{
  ...
  bool bHideDisabled = CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
