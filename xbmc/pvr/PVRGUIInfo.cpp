          ...
          CSettings::SETTING_PVRPLAYBACK_SIGNALQUALITY))
  {
    ...
    if (!bIsPlayingRecording)
    {
      ...
      CServiceBroker::GetPVRManager().Clients()->GetCreatedClient(
          CServiceBroker::GetPVRManager().GetPlayingClientID(), client);
...
void CPVRGUIInfo::UpdateMisc(void)
{
  ...
  std::string strPlayingClientName =
      bStarted ? CServiceBroker::GetPVRManager().GetPlayingClientName() : "";
  bool bHasTVRecordings =
      bStarted && CServiceBroker::GetPVRManager().Recordings()->GetNumTVRecordings() > 0;
  bool bHasRadioRecordings =
      bStarted && CServiceBroker::GetPVRManager().Recordings()->GetNumRadioRecordings() > 0;
  ...
  bool bIsPlayingEncryptedStream =
      bStarted && CServiceBroker::GetPVRManager().IsPlayingEncryptedChannel();
  bool bHasTVChannels =
      bStarted && CServiceBroker::GetPVRManager().ChannelGroups()->GetGroupAllTV()->HasChannels();
  bool bHasRadioChannels =
      bStarted &&
      CServiceBroker::GetPVRManager().ChannelGroups()->GetGroupAllRadio()->HasChannels();
  bool bCanRecordPlayingChannel =
      bStarted && CServiceBroker::GetPVRManager().CanRecordOnPlayingChannel();
  bool bIsRecordingPlayingChannel =
      bStarted && CServiceBroker::GetPVRManager().IsRecordingOnPlayingChannel();
  bool bIsPlayingActiveRecording =
      bStarted && CServiceBroker::GetPVRManager().IsPlayingActiveRecording();
  std::string strPlayingTVGroup =
      (bStarted && bIsPlayingTV)
          ? CServiceBroker::GetPVRManager().GetPlayingGroup(false)->GroupName()
          : "";
  std::string strPlayingRadioGroup =
      (bStarted && bIsPlayingRadio)
          ? CServiceBroker::GetPVRManager().GetPlayingGroup(true)->GroupName()
          : "";
                                            ...
                                            std::string& strValue) const
{
  ...
  if (timer)
  {
    ...
    case LISTITEM_COMMENT:
      strValue = timer->GetStatus(CServiceBroker::GetGUI()->GetWindowManager().GetActiveWindow() ==
  ...
  if (item->IsPVRChannel() || item->IsEPG() || item->IsPVRTimer())
  {
    ...
    case LISTITEM_EPG_EVENT_TITLE:
      ...
      if (strValue.empty() && !CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
                              ...
                              std::string& strValue) const
{
  ...
  {
    const CPVREpgInfoTagPtr epgTag =
  ...
  case PVR_EPG_EVENT_SEEK_TIME:
    strValue =
        m_timesInfo.GetEpgEventSeekTime(g_application.GetAppPlayer().GetSeekHandler().GetSeekSize(),
  ...
  case VIDEOPLAYER_CAN_RESUME_LIVE_TV:
    if (item->IsPVRRecording())
    {
      ...
      const std::shared_ptr<CPVREpgInfoTag> epgTag =
          CServiceBroker::GetPVRManager().EpgContainer().GetTagById(epg, recording->BroadcastUid());
