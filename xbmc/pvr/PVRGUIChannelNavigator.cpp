...
void CPVRGUIChannelNavigator::SelectNextChannel(ChannelSwitchMode eSwitchMode)
{
  if (!CServiceBroker::GetGUI()
           ...
           .GetShowInfo() &&
      eSwitchMode == ChannelSwitchMode::NO_SWITCH)
  {
...
CPVRChannelPtr CPVRGUIChannelNavigator::GetNextOrPrevChannel(bool bNext)
{
  ...
  if (bPlayingTV || bPlayingRadio)
  {
    const CPVRChannelGroupPtr group =
        CServiceBroker::GetPVRManager().GetPlayingGroup(bPlayingRadio);
    if (group)
    {
      CSingleLock lock(m_critSection);
      return bNext ? group->GetNextChannel(m_currentChannel)
...
void CPVRGUIChannelNavigator::SelectChannel(const CPVRChannelPtr channel,
                                            ChannelSwitchMode eSwitchMode)
{
  ...
  {
    int iTimeout = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
    ...
    if (iTimeout > 0)
    {
      ...
      m_iChannelEntryJobId =
          CJobManager::GetInstance().AddJob(job, dynamic_cast<IJobCallback*>(job));
    }
    else
    {
      // instant switch
...
{
  return IsPreview() && CServiceBroker::GetGUI()
                            ->GetInfoManager()
                            .GetInfoProviders()
                            .GetPlayerInfoProvider()
                            .GetShowInfo();
