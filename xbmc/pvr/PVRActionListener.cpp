...
CPVRActionListener::CPVRActionListener()
{
  ...
  CServiceBroker::GetSettingsComponent()->GetSettings()->RegisterCallback(
      ...
      {CSettings::SETTING_PVRPARENTAL_ENABLED, CSettings::SETTING_PVRMANAGER_RESETDB,
       CSettings::SETTING_EPG_RESETEPG, CSettings::SETTING_PVRMANAGER_CLIENTPRIORITIES,
       CSettings::SETTING_PVRMANAGER_CHANNELMANAGER, CSettings::SETTING_PVRMANAGER_GROUPMANAGER,
       CSettings::SETTING_PVRMANAGER_CHANNELSCAN, CSettings::SETTING_PVRMENU_SEARCHICONS,
       CSettings::SETTING_PVRCLIENT_MENUHOOK, CSettings::SETTING_EPG_FUTURE_DAYSTODISPLAY});
}
...
bool CPVRActionListener::OnAction(const CAction& action)
{
  ...
  case ACTION_PREV_ITEM:
  {
    ...
    CServiceBroker::GetPVRManager().GUIActions()->SeekBackward(
  ...
  case ACTION_CHANNEL_UP:
  {
    ...
    CServiceBroker::GetPVRManager().GUIActions()->GetChannelNavigator().SelectNextChannel(
        GetChannelSwitchMode(action.GetID()));
    return true;
  }
  ...
  case ACTION_CHANNEL_DOWN:
  {
    ...
    CServiceBroker::GetPVRManager().GUIActions()->GetChannelNavigator().SelectPreviousChannel(
        GetChannelSwitchMode(action.GetID()));
    return true;
  }
  ...
  case ACTION_CHANNEL_SWITCH:
  {
    ...
    const CPVRChannelGroupPtr selectedGroup = CServiceBroker::GetPVRManager()
                                                  .ChannelGroups()
                                                  ->Get(currentChannel->IsRadio())
                                                  ->GetSelectedGroup();
    const std::shared_ptr<CPVRChannel> channel =
        selectedGroup->GetByChannelNumber(CPVRChannelNumber(iChannelNumber, iSubChannelNumber));

    if (!channel)
      return false;

    CServiceBroker::GetPVRManager().GUIActions()->SwitchToChannel(
        std::make_shared<CFileItem>(channel), false);
    return true;
  }
...
void CPVRActionListener::OnSettingChanged(std::shared_ptr<const CSetting> setting)
{
  ...
  {
    CServiceBroker::GetPVRManager().Clients()->SetEPGTimeFrame(
