...
{
  const std::shared_ptr<CFileItem> parentTimer =
      CServiceBroker::GetPVRManager().GUIActions()->GetTimerRule(item);
  if (parentTimer)
    ...
    if (timerType)
    {
      if (item.GetEPGInfoTag())
      {
        if (timerType->IsReminder())
          return g_localizeStrings.Get(timerType->IsReadOnly() ? 829
                                                               : 830); /* View/Edit reminder */
        else
          return g_localizeStrings.Get(timerType->IsReadOnly() ? 19241
                                                               : 19242); /* View/Edit timer */
      }
      else
