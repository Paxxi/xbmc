...
const std::string CLibInputSettings::SETTING_INPUT_LIBINPUTKEYBOARDLAYOUT =
    "input.libinputkeyboardlayout";
  ...
  : m_libInputHandler(handler)
{
  ...
  settings->GetSettingsManager()->RegisterSettingOptionsFiller("libinputkeyboardlayout",
  ...
  if (!rootElement)
  {
    CLog::Log(LOGWARNING, "CLibInputSettings: missing or invalid XML root element in: %s",
              xkbFile.c_str());
    return;
  }
  ...
  {
    CLog::Log(LOGWARNING, "CLibInputSettings: unexpected XML root element %s in: %s",
  ...
  {
    CLog::Log(LOGWARNING, "CLibInputSettings: unexpected XML child element %s in: %s",
...
void CLibInputSettings::SettingOptionsKeyboardLayoutsFiller(std::shared_ptr<const CSetting> setting,
