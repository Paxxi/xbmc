...
void CLibInputKeyboard::ProcessKey(libinput_event_keyboard* e)
{
  ...
  xkb_state_component modtype =
      xkb_state_component(XKB_STATE_MODS_DEPRESSED | XKB_STATE_MODS_LATCHED);
  if (xkb_state_mod_index_is_active(m_state, m_modindex[0], modtype) &&
      ((keysym != XBMCK_LCTRL) || !pressed))
    mod |= XBMCKMOD_CTRL;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[0], modtype) &&
      ((keysym != XBMCK_RCTRL) || !pressed))
    mod |= XBMCKMOD_CTRL;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[1], modtype) &&
      ((keysym != XBMCK_LALT) || !pressed))
    mod |= XBMCKMOD_ALT;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[1], modtype) &&
      ((keysym != XBMCK_RALT) || !pressed))
    mod |= XBMCKMOD_ALT;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[2], modtype) &&
      ((keysym != XBMCK_LSHIFT) || !pressed))
    mod |= XBMCKMOD_SHIFT;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[2], modtype) &&
      ((keysym != XBMCK_RSHIFT) || !pressed))
    mod |= XBMCKMOD_SHIFT;
  if (xkb_state_mod_index_is_active(m_state, m_modindex[3], modtype) &&
  ...
  if (xkb_state_led_index_is_active(m_state, m_ledindex[0]) &&
      ((keysym != XBMCK_NUMLOCK) || !pressed))
  {
    m_leds |= LIBINPUT_LED_NUM_LOCK;
    mod |= XBMCKMOD_NUM;
  }
  if (xkb_state_led_index_is_active(m_state, m_ledindex[1]) &&
      ((keysym != XBMCK_CAPSLOCK) || !pressed))
  {
    m_leds |= LIBINPUT_LED_CAPS_LOCK;
    mod |= XBMCKMOD_CAPS;
  }
  if (xkb_state_led_index_is_active(m_state, m_ledindex[2]) &&
  ...
  if (pressed && xkb_keymap_key_repeats(m_keymap, xkbkey))
  {
    ...
    {
      CLog::Log(LOGDEBUG, "CLibInputKeyboard::%s - using delay: %ims repeat: %ims", __FUNCTION__,
...
void CLibInputKeyboard::GetRepeat(libinput_device* dev)
{
  ...
  if (fd < 0)
  {
    CLog::Log(LOGERROR, "CLibInputKeyboard::%s - failed to open %s (%s)", __FUNCTION__, sysname,
              strerror(errno));
  }
  else
  {
    ...
    if (ret < 0)
      CLog::Log(LOGDEBUG, "CLibInputKeyboard::%s - could not get key repeat for %s (%s)",
                __FUNCTION__, sysname, strerror(errno));

    CLog::Log(LOGDEBUG, "CLibInputKeyboard::%s - delay: %ims repeat: %ims for %s (%s)",
