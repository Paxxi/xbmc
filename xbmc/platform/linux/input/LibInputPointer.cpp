...
void CLibInputPointer::ProcessButton(libinput_event_pointer* e)
{
  ...
  CLog::Log(LOGDEBUG,
            "CLibInputPointer::%s - event.type: %i, event.button.button: %i, event.button.x: %i, "
            "event.button.y: %i",
...
void CLibInputPointer::ProcessMotion(libinput_event_pointer* e)
{
  ...
  CLog::Log(LOGDEBUG,
            "CLibInputPointer::%s - event.type: %i, event.motion.x: %i, event.motion.y: %i",
...
{
  m_pos.X = static_cast<int>(libinput_event_pointer_get_absolute_x_transformed(
      e, CServiceBroker::GetWinSystem()->GetGfxContext().GetWidth()));
  m_pos.Y = static_cast<int>(libinput_event_pointer_get_absolute_y_transformed(
...
void CLibInputPointer::ProcessAxis(libinput_event_pointer* e)
{
  ...
  CLog::Log(LOGDEBUG, "CLibInputPointer::%s - scroll: %s, event.button.x: %i, event.button.y: %i",
