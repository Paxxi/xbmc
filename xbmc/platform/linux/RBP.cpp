...
bool CRBP::Initialize()
{
  ...
  m_gui_resolution_limit =
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt("videoscreen.limitgui");
...
void CRBP::LogFirmwareVersion()
{
  ...
  CLog::Log(LOGNOTICE, "ARM mem: %dMB GPU mem: %dMB MPG2:%d WVC1:%d", m_arm_mem, m_gpu_mem,
...
unsigned char* CRBP::CaptureDisplay(
    int width, int height, int* pstride, bool swap_red_blue, bool video_only)
{
...
uint32_t CRBP::WaitVsync(uint32_t target)
{
  ...
  if ((signed)(m_vsync_count - target) < 0)
    CLog::Log(LOGDEBUG, "CRBP::%s no  vsync %d/%d display:%x(%x) delay:%d", __FUNCTION__,
              m_vsync_count, target, m_display, display, delay.MillisLeft());
...
static int mbox_open()
{
  ...
  if (file_desc < 0)
    CLog::Log(LOGERROR, "%s: Can't open device file: %s (%d)", __FUNCTION__, DEVICE_FILE_NAME,
              file_desc);
...
{
  m_numbytes = numbytes;
  m_vcsm_handle = vcsm_malloc_cache(
      numbytes,
      static_cast<VCSM_CACHE_TYPE_T>(
          0x80 | static_cast<unsigned>(cached ? VCSM_CACHE_TYPE_HOST : VCSM_CACHE_TYPE_NONE)),
      const_cast<char*>("CGPUMEM"));
  if (m_vcsm_handle)
