...
void COMXCoreTunnel::Initialize(COMXCoreComponent* src_component,
                                unsigned int src_port,
                                COMXCoreComponent* dst_component,
...
OMX_ERRORTYPE COMXCoreTunnel::Deestablish(bool noWait)
{
  ...
  if (m_src_component->GetComponent())
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(
          LOGERROR,
          "COMXCoreTunnel::Deestablish - Error disable port %d on component %s omx_err(0x%08x)",
  ...
  if (m_src_component->GetComponent())
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreTunnel::Deestablish - Error WaitForCommand port %d on component %s "
                "omx_err(0x%08x)",
...
OMX_ERRORTYPE COMXCoreTunnel::Establish(bool enable_ports /* = true */,
                                        bool disable_ports /* = false */)
{
  OMX_ERRORTYPE omx_err = OMX_ErrorNone;
  ...
  if (m_src_component->GetState() == OMX_StateLoaded)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreTunnel::Establish - Error setting state to idle %s omx_err(0x%08x)",
  ...
  if (m_src_component->GetComponent() && disable_ports)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreTunnel::Establish - Error disable port %d on component %s omx_err(0x%08x)",
                m_src_port, m_src_component->GetName().c_str(), (int)omx_err);
    }
  }
  ...
  if (m_src_component->GetComponent() && disable_ports)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreTunnel::Establish - Error WaitForCommand port %d on component %s "
                "omx_err(0x%08x)",
                m_dst_port, m_src_component->GetName().c_str(), (int)omx_err);
      return omx_err;
    }
  ...
  {
    omx_err = m_DllOMX->OMX_SetupTunnel(m_src_component->GetComponent(), m_src_port,
  ...
  if (m_src_component->GetComponent() && enable_ports)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreTunnel::Establish - Error enable port %d on component %s omx_err(0x%08x)",
                m_src_port, m_src_component->GetName().c_str(), (int)omx_err);
      return omx_err;
    }
  }
...
OMX_ERRORTYPE COMXCoreComponent::DisableAllPorts()
{
  ...
  OMX_INDEXTYPE idxTypes[] = {OMX_IndexParamAudioInit, OMX_IndexParamImageInit,
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    if (omx_err == OMX_ErrorNone)
    {
      ...
      for (j = 0; j < ports.nPorts; j++)
      {
        ...
        omx_err =
            OMX_SendCommand(m_handle, OMX_CommandPortDisable, ports.nStartPortNumber + j, NULL);
        if (omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR,
                    "COMXCoreComponent::DisableAllPorts - Error disable port %d on component %s "
                    "omx_err(0x%08x)",
...
OMX_ERRORTYPE COMXCoreComponent::WaitForEvent(OMX_EVENTTYPE eventType, long timeout)
{
  ...
  while (true)
  {
    for (std::vector<omx_event>::iterator it = m_omx_events.begin(); it != m_omx_events.end(); ++it)
    {
      ...
      if (event.eEvent == OMX_EventError && event.nData1 == (OMX_U32)OMX_ErrorSameState &&
          event.nData2 == 1)
      {
#ifdef OMX_DEBUG_EVENTS
        CLog::Log(LOGDEBUG,
...
OMX_ERRORTYPE COMXCoreComponent::WaitForCommand(OMX_U32 command, OMX_U32 nData2, long timeout)
{
  ...
  while (true)
  {
    for (std::vector<omx_event>::iterator it = m_omx_events.begin(); it != m_omx_events.end(); ++it)
    {
      ...
      else if (event.eEvent == OMX_EventCmdComplete && event.nData1 == command &&
               event.nData2 == nData2)
      {
...
bool COMXCoreComponent::Initialize(const std::string& component_name, OMX_INDEXTYPE index)
{
  ...
  {
    CLog::Log(LOGERROR,
              "COMXCoreComponent::Initialize - could not get port_param for component %s "
              "omx_err(0x%08x)\n",
              component_name.c_str(), (int)omx_err);
  }
...
bool COMXCoreComponent::Deinitialize()
{
  ...
  if (m_handle)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "COMXCoreComponent::Deinitialize - failed to free handle for component %s "
                "omx_err(0x%08x)",
                m_componentName.c_str(), omx_err);
    }
    m_handle = NULL;
...
OMX_ERRORTYPE COMXCoreComponent::DecoderEmptyBufferDoneCallback(OMX_HANDLETYPE hComponent,
                                                                OMX_PTR pAppData,
                                                     ...
                                                     OMX_PTR pEventData)
{
  ...
  {
    CLog::Log(LOGDEBUG,
              "COMXCoreComponent::%s - %s Ignoring expected event: eEvent(0x%x), nData1(0x%x), "
              "nData2(0x%x), pEventData(0x%p)\n",
              __func__, GetName().c_str(), eEvent, nData1, nData2, pEventData);
    m_ignore_error = OMX_ErrorNone;
    return OMX_ErrorNone;
  }
  ...
  case OMX_EventCmdComplete:
    ...
    case OMX_CommandStateSet:
      ...
      default:
#if defined(OMX_DEBUG_EVENTHANDLER)
        CLog::Log(LOGDEBUG, "%s::%s %s - Unknown OMX_Statexxxxx, state(%d)\n", CLASSNAME, __func__,
...
void COMXCore::Deinitialize()
{
  if (m_is_open)
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "COMXCore::Deinitialize - OMXCore failed to deinit, omx_err(0x%08x)",
                omx_err);
    }
    m_DllOMX->Unload();
  }
