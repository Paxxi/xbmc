...
bool CDBusReserve::AcquireDevice(const std::string& device)
{
  ...
  if (res == -1)
  {
    CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): Request name failed: %s - %s",
              device.c_str(), error.Name().c_str(), error.Message().c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): Request name returned unknown code %d",
              device.c_str(), res);
    return false;
  }

  msg.reset(
      dbus_message_new_method_call(service.c_str(), object.c_str(), interface, "RequestRelease"));
  if (!msg)
  {
  ...
  {
    CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): Failed to append arguments",
              device.c_str());
    return false;
  }
  ...
  if (!reply)
  {
    if (error.Name() == DBUS_ERROR_TIMED_OUT || error.Name() == DBUS_ERROR_UNKNOWN_METHOD ||
        error.Name() == DBUS_ERROR_NO_REPLY)
      CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): RequestRelease was denied on call",
                device.c_str());
    else
      CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): RequestRelease call failed: %s - %s",
                device.c_str(), error.Name().c_str(), error.Message().c_str());
  ...
  {
    CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): Failed to get reply arguments",
              device.c_str());
    return false;
  }
  ...
  if (!allowed)
  {
    CLog::Log(LOGERROR, "CDBusReserve::AcquireDevice(%s): RequestRelease was denied",
              device.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGERROR,
              "CDBusReserve::AcquireDevice(%s): Request name failed after release: %s - %s",
              device.c_str(), error.Name().c_str(), error.Message().c_str());
    return false;
  }

  m_devs.push_back(device);
  CLog::Log(LOGDEBUG, "CDBusReserve::AcquireDevice(%s): Successfully reserved audio",
            device.c_str());
  return true;
}
...
bool CDBusReserve::ReleaseDevice(const std::string& device)
{
  ...
  if (it == m_devs.end())
  {
    CLog::Log(LOGDEBUG, "CDBusReserve::ReleaseDevice(%s): device wasn't acquired here",
              device.c_str());
    return false;
  }
  m_devs.erase(it);
