...
CZeroconfAvahi::~CZeroconfAvahi()
{
  ...
  if (mp_poll)
  {
    ...
    AvahiTimeout* lp_timeout;
    {
      ...
      m_shutdown = true;
      lp_timeout = cp_apoll->timeout_new(cp_apoll, &tv, shutdownCallback, this);
    }
...
{
  CLog::Log(LOGDEBUG, "CZeroconfAvahi::doPublishService identifier: %s type: %s name:%s port:%i",
  ...
  tServiceMap::mapped_type p_service_info(
      new CZeroconfAvahi::ServiceInfo(fcr_type, fcr_name, f_port, txtList));
  ...
  {
    CLog::Log(LOGDEBUG,
              "CZeroconfAvahi::doPublishService: client not running, queued for publishing");
  }
  return true;
}
...
bool CZeroconfAvahi::doForceReAnnounceService(const std::string& fcr_identifier)
{
  ...
  if (it != m_services.end() && it->second->mp_group)
  {
    ...
    if ((avahi_entry_group_update_service_txt_strlst(
            it->second->mp_group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, AvahiPublishFlags(0),
            it->second->m_name.c_str(), it->second->m_type.c_str(), NULL, it->second->mp_txt)) >= 0)
      ret = true;
  }
...
void CZeroconfAvahi::clientCallback(AvahiClient* fp_client, AvahiClientState f_state, void* fp_data)
{
  ...
  case AVAHI_CLIENT_FAILURE:
    CLog::Log(LOGINFO, "CZeroconfAvahi::clientCallback: client failure. avahi-daemon stopped? "
                       "Recreating client...");
  ...
  case AVAHI_CLIENT_S_REGISTERING:
    ...
    CLog::Log(LOGDEBUG,
              "CZeroconfAvahi::clientCallback: uiuui; coll or reg, anyways, resetting groups");
  ...
  case AVAHI_CLIENT_CONNECTING:
    CLog::Log(
        LOGINFO,
        "CZeroconfAvahi::clientCallback: avahi server not available. But may become later...");
    break;
  }
}

void CZeroconfAvahi::groupCallback(AvahiEntryGroup* fp_group,
                                   AvahiEntryGroupState f_state,
                                   void* fp_data)
{
  CZeroconfAvahi* p_instance = static_cast<CZeroconfAvahi*>(fp_data);
  //store our thread ID and check for shutdown -> check details in destructor
  ...
  case AVAHI_ENTRY_GROUP_COLLISION:
  {
    ...
    if (it != p_instance->m_services.end())
    {
      ...
      avahi_free(alt_name);
      CLog::Log(LOGNOTICE, "CZeroconfAvahi::groupCallback: Service name collision. Renamed to: %s",
...
{
  assert(fp_client);
  CLog::Log(LOGDEBUG, "CZeroconfAvahi::addService() named: %s type: %s port:%i",
  ...
  {
    if (!(fp_service_info->mp_group =
              avahi_entry_group_new(fp_client, &CZeroconfAvahi::groupCallback, this)))
    {
      CLog::Log(LOGDEBUG, "CZeroconfAvahi::addService() avahi_entry_group_new() failed: %s",
  ...
  if (avahi_entry_group_is_empty(fp_service_info->mp_group))
  {
    if ((ret = avahi_entry_group_add_service_strlst(
             fp_service_info->mp_group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, AvahiPublishFlags(0),
             fp_service_info->m_name.c_str(), fp_service_info->m_type.c_str(), NULL, NULL,
             fp_service_info->m_port, fp_service_info->mp_txt)) < 0)
    {
      if (ret == AVAHI_ERR_COLLISION)
      {
        ...
        avahi_free(alt_name);
        CLog::Log(LOGNOTICE, "CZeroconfAvahi::addService: Service name collision. Renamed to: %s",
                  fp_service_info->m_name.c_str());
        addService(fp_service_info, fp_client);
        return;
      }
      CLog::Log(LOGERROR,
                "CZeroconfAvahi::addService(): failed to add service named:%s@$(HOSTNAME) type:%s "
                "port:%i. Error:%s :/ FIXME!",
                fp_service_info->m_name.c_str(), fp_service_info->m_type.c_str(),
  ...
  {
    CLog::Log(LOGERROR, "CZeroconfAvahi::addService(): Failed to commit entry group! Error:%s",
              avahi_strerror(ret));
