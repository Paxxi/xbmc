...
{
  explicit ScopedEventLoopBlock(AvahiThreadedPoll* fp_poll)
    : mp_poll(fp_poll)
  {
    avahi_threaded_poll_lock(mp_poll);
  }

  ~ScopedEventLoopBlock() { avahi_threaded_poll_unlock(mp_poll); }
...
bool CZeroconfBrowserAvahi::doAddServiceType(const std::string& fcr_service_type)
{
  ...
  else
  {
    CLog::Log(
        LOGINFO,
        "CZeroconfBrowserAvahi::doAddServiceType client not available. service browsing queued");
    return true;
  }
}
...
bool CZeroconfBrowserAvahi::doResolveService(CZeroconfBrowser::ZeroconfService& fr_service,
                                             double f_timeout)
{
  {
    ...
    {
      CLog::Log(LOGERROR, "CZeroconfBrowserAvahi::doResolveService called with undiscovered "
                          "service, resolving is NOT possible");
      return false;
    }
    //start resolving
    ...
    if (!avahi_service_resolver_new(mp_client, it->second.interface, it->second.protocol,
                                    ...
                                    AvahiLookupFlags(0), resolveCallback, this))
    {
      CLog::Log(LOGERROR,
                "CZeroconfBrowserAvahi::doResolveService Failed to resolve service '%s': %s\n",
...
void CZeroconfBrowserAvahi::clientCallback(AvahiClient* fp_client,
                                           AvahiClientState f_state,
                                           void* fp_data)
{
  ...
  {
    CLog::Log(LOGINFO, "CZeroconfBrowserAvahi::clientCallback: client failure. avahi-daemon "
                       "stopped? Recreating client...");
  ...
  case AVAHI_CLIENT_CONNECTING:
    CLog::Log(LOGINFO, "CZeroconfBrowserAvahi::clientCallback: avahi server not available. But may "
                       "become later...");
    break;
  }
}
void CZeroconfBrowserAvahi::browseCallback(AvahiServiceBrowser* browser,
                                           ...
                                           void* fp_data)
{
  ...
  case AVAHI_BROWSER_NEW:
  {
    CLog::Log(
        LOGDEBUG,
        "CZeroconfBrowserAvahi::browseCallback NEW: service '%s' of type '%s' in domain '%s'\n",
        name, type, domain);
    //store the service
    ...
    if (p_instance->m_all_for_now_browsers.find(browser) !=
        p_instance->m_all_for_now_browsers.end())
      update_gui = true;
    break;
  }
  case AVAHI_BROWSER_REMOVE:
  {
    ...
    CLog::Log(
        LOGDEBUG,
        "CZeroconfBrowserAvahi::browseCallback REMOVE: service '%s' of type '%s' in domain '%s'\n",
        name, type, domain);
    //if this browser already sent the all for now message, we need to update the gui now
    if (p_instance->m_all_for_now_browsers.find(browser) !=
        p_instance->m_all_for_now_browsers.end())
  ...
  case AVAHI_BROWSER_ALL_FOR_NOW:
    ...
    if (!success)
      CLog::Log(LOGDEBUG,
                "CZeroconfBrowserAvahi::browseCallback AVAHI_BROWSER_ALL_FOR_NOW sent twice?!");
    update_gui = true;
    break;
  }
  if (update_gui)
  {
    ...
    CServiceBroker::GetGUI()->GetWindowManager().SendThreadMessage(message);
    CLog::Log(LOGDEBUG,
              "CZeroconfBrowserAvahi::browseCallback sent gui update for path zeroconf://");
  }
}
...
CZeroconfBrowser::ZeroconfService::tTxtRecordMap GetTxtRecords(AvahiStringList* txt)
{
  ...
  for (i = txt; i; i = i->next)
  {
    ...
    recordMap.insert(std::make_pair(std::string(key), std::string(value)));
...
void CZeroconfBrowserAvahi::resolveCallback(AvahiServiceResolver* r,
                                            AvahiIfIndex interface,
                                            AvahiProtocol protocol,
                                            ...
                                            void* userdata)
{
  ...
  case AVAHI_RESOLVER_FAILURE:
    CLog::Log(LOGERROR,
              "CZeroconfBrowserAvahi::resolveCallback Failed to resolve service '%s' of type '%s' "
              "in domain '%s': %s\n",
              name, type, domain,
  ...
  case AVAHI_RESOLVER_FOUND:
  {
    ...
    CLog::Log(LOGDEBUG,
              "CZeroconfBrowserAvahi::resolveCallback resolved service '%s' of type '%s' in domain "
              "'%s':\n",
              name, type, domain);
...
AvahiServiceBrowser* CZeroconfBrowserAvahi::createServiceBrowser(
    const std::string& fcr_service_type, AvahiClient* fp_client, void* fp_userdata)
{
  assert(fp_client);
  AvahiServiceBrowser* ret = avahi_service_browser_new(
      ...
      (AvahiLookupFlags)0, browseCallback, fp_userdata);
  if (!ret)
  {
    CLog::Log(
        LOGERROR,
        "CZeroconfBrowserAvahi::createServiceBrowser Failed to create service (%s) browser: %s",
