...
bool CWin32StorageProvider::Eject(const std::string& mountpath)
{
  ...
  HANDLE hVolume = CreateFile(strVolFormat.c_str(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
  ...
  long res = DeviceIoControl(hVolume, IOCTL_STORAGE_GET_DEVICE_NUMBER, NULL, 0, &sdn, sizeof(sdn),
  ...
  CM_Get_Parent(&DevInst, DevInst,
                0); // disk's parent, e.g. the USB bridge, the SATA controller....
    ...
    do
    {
          ...
          GetDiskFreeSpaceEx((strDrive.c_str()), nullptr, &ULTotal, &ULTotalFree))
      {
        strRet = KODI::PLATFORM::WINDOWS::FromW(StringUtils::Format(
            L"%s %d MB %s", strDrive.c_str(), int(ULTotalFree.QuadPart / (1024 * 1024)),
...
void CWin32StorageProvider::GetDrivesByType(VECSOURCES& localDrives,
                                            ...
                                            bool bonlywithmedia)
{
  ...
  if (dwStrLength != 0)
  {
    ...
    do
    {
      ...
      if (strWdrive.size() >= 2 && strWdrive.substr(0, 2) != L"A:" &&
          strWdrive.substr(0, 2) != L"B:")
      ...
      if (uDriveType > DRIVE_UNKNOWN &&
          ((eDriveType == ALL_DRIVES &&
            (uDriveType == DRIVE_FIXED || uDriveType == DRIVE_REMOTE || uDriveType == DRIVE_CDROM ||
             uDriveType == DRIVE_REMOVABLE)) ||
           (eDriveType == LOCAL_DRIVES &&
            (uDriveType == DRIVE_FIXED || uDriveType == DRIVE_REMOTE)) ||
           ...
           (eDriveType == DVD_DRIVES && (uDriveType == DRIVE_CDROM))))
      {
        ...
        else
        {
          ...
          {
          case DRIVE_CDROM:
            share.strName = StringUtils::Format("%s (%s)", share.strPath.c_str(),
                                                g_localizeStrings.Get(218).c_str());
            break;
          case DRIVE_REMOVABLE:
            if (share.strName.empty())
              share.strName = StringUtils::Format("%s (%s)", g_localizeStrings.Get(437).c_str(),
                                                  share.strPath.c_str());
            break;
          default:
            if (share.strName.empty())
              ...
              share.strName =
                  StringUtils::Format("%s (%s)", share.strPath.c_str(), share.strName.c_str());
            break;
          }
        }
...
DEVINST CWin32StorageProvider::GetDrivesDevInstByDiskNumber(long DiskNumber)
{
  ...
  while (true)
  {
    ...
    {
      pspdidd =
          (PSP_DEVICE_INTERFACE_DETAIL_DATA)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwSize);
      if (pspdidd == NULL)
        continue;
      ...
      if (res)
      {
        HANDLE hDrive = CreateFile(pspdidd->DevicePath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                                   OPEN_EXISTING, NULL, NULL);
