...
{
public:
  inline CharVecBuffer(const XbmcCommons::Buffer& buf)
    : XbmcCommons::Buffer(buf){};
...
public:
  ...
  CMediaDrmCryptoSession(const std::string& UUID,
                         const std::string& cipherAlgo,
                         const std::string& macAlgo);
  virtual ~CMediaDrmCryptoSession();

  // Interface methods
  XbmcCommons::Buffer GetKeyRequest(const XbmcCommons::Buffer& init,
  ...
  // Crypto methods
  XbmcCommons::Buffer Decrypt(const XbmcCommons::Buffer& cipherKeyId,
                              const XbmcCommons::Buffer& input,
                              const XbmcCommons::Buffer& iv) override;
  XbmcCommons::Buffer Encrypt(const XbmcCommons::Buffer& cipherKeyId,
                              const XbmcCommons::Buffer& input,
                              const XbmcCommons::Buffer& iv) override;
  XbmcCommons::Buffer Sign(const XbmcCommons::Buffer& macKeyId,
                           const XbmcCommons::Buffer& message) override;
  bool Verify(const XbmcCommons::Buffer& macKeyId,
              const XbmcCommons::Buffer& message,
              const XbmcCommons::Buffer& signature) override;

private:
  static CCryptoSession* Create(const std::string& UUID,
                                const std::string& cipherAlgo,
