                                               ...
                                               double f_timeout)
{
  ...
  {
    CLog::Log(LOGERROR, "ZeroconfBrowserAndroid: DNSServiceResolve returned (error = %ld)",
  ...
  while (it.hasNext())
  {
    ...
    CLog::Log(LOGDEBUG, "ZeroconfBrowserAndroid: TXT record %s = %s (%d)", key.c_str(),
              value.c_str(), vv.size());
...
void CZeroconfBrowserAndroid::addDiscoveredService(
    CZeroconfBrowserAndroidDiscover* browser, CZeroconfBrowser::ZeroconfService const& fcr_service)
{
  CSingleLock lock(m_data_guard);
  ...
  if (browserIt == m_discovered_services.end())
  {
    ...
    browserIt =
        m_discovered_services
            .insert(make_pair(browser, std::vector<std::pair<ZeroconfService, unsigned int>>()))
            .first;
  }
  //search this service
...
void CZeroconfBrowserAndroid::removeDiscoveredService(
    CZeroconfBrowserAndroidDiscover* browser, CZeroconfBrowser::ZeroconfService const& fcr_service)
{
  CSingleLock lock(m_data_guard);
  tDiscoveredServicesMap::iterator browserIt = m_discovered_services.find(browser);
...
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidDiscover::onDiscoveryStarted type: %s",
            serviceType.c_str());
  m_isActive = true;
}
...
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidDiscover::onDiscoveryStopped type: %s",
            serviceType.c_str());
  m_isActive = false;
}
...
{
  //store the service
  CZeroconfBrowser::ZeroconfService s(serviceInfo.getServiceName(), serviceInfo.getServiceType(),
                                      "local");
  ...
  while (it.hasNext())
  {
    ...
    CLog::Log(LOGDEBUG, "ZeroconfBrowserAndroid::onServiceFound: TXT record %s = %s (%d)",
              key.c_str(), value.c_str(), vv.size());
  }

  CLog::Log(LOGDEBUG,
            "CZeroconfBrowserAndroidDiscover::onServiceFound found service named: %s, type: %s, "
            "domain: %s",
...
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidDiscover::onServiceLost name: %s, type: %s",
...
void CZeroconfBrowserAndroidDiscover::onStartDiscoveryFailed(const std::string& serviceType,
                                                             int errorCode)
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidDiscover::onStartDiscoveryFailed type: %s, error: %d",
...
void CZeroconfBrowserAndroidDiscover::onStopDiscoveryFailed(const std::string& serviceType,
                                                            int errorCode)
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidDiscover::onStopDiscoveryFailed type: %s, error: %d",
            serviceType.c_str(), errorCode);
  m_isActive = false;
}
...
void CZeroconfBrowserAndroidResolve::onResolveFailed(const jni::CJNINsdServiceInfo& serviceInfo,
                                                     int errorCode)
{
  CLog::Log(LOGDEBUG,
            "CZeroconfBrowserAndroidResolve::onResolveFailed name: %s, type: %s, error: %d",
...
{
  CLog::Log(LOGDEBUG, "CZeroconfBrowserAndroidResolve::onServiceResolved name: %s, type: %s",
            serviceInfo.getServiceName().c_str(), serviceInfo.getServiceType().c_str());
  m_errorCode = -1;
