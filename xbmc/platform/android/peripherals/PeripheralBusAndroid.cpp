...
bool CPeripheralBusAndroid::InitializeProperties(CPeripheral& peripheral)
{
  ...
  {
    CLog::Log(LOGWARNING,
              "CPeripheralBusAndroid: failed to initialize properties for peripheral \"%s\"",
  ...
  CLog::Log(LOGDEBUG, "CPeripheralBusAndroid: Initializing device %d \"%s\"", deviceId,
  ...
  if (!state.Initialize(device))
  {
    CLog::Log(
        LOGWARNING,
        "CPeripheralBusAndroid: failed to initialize the state for input device \"%s\" with ID %d",
...
void CPeripheralBusAndroid::OnInputDeviceAdded(int deviceId)
{
  const std::string deviceLocation = GetDeviceLocation(deviceId);
  {
    ...
    {
      CLog::Log(LOGINFO,
                "CPeripheralBusAndroid: ignoring added input device with ID %d because we already "
                "know it",
                deviceId);
      return;
    }
    ...
    {
      CLog::Log(LOGWARNING,
                "CPeripheralBusAndroid: failed to add input device with ID %d because it couldn't "
                "be found",
                deviceId);
      return;
    }
...
void CPeripheralBusAndroid::OnInputDeviceChanged(int deviceId)
{
  ...
  const std::string deviceLocation = GetDeviceLocation(deviceId);
  {
         ...
         ++result)
    {
      if (result->m_strLocation == deviceLocation)
      {
        ...
        {
          CLog::Log(LOGWARNING,
                    "CPeripheralBusAndroid: failed to update input device \"%s\" with ID %d "
        ...
        CLog::Log(LOGINFO, "CPeripheralBusAndroid: input device \"%s\" with ID %d updated",
                  result->m_strDeviceName.c_str(), deviceId);
        changed = true;
        break;
      }
  ...
  if (changed)
    ...
    CLog::Log(LOGWARNING,
              "CPeripheralBusAndroid: failed to update input device with ID %d because it couldn't "
              "be found",
              deviceId);
}
...
void CPeripheralBusAndroid::OnInputDeviceRemoved(int deviceId)
{
  ...
  const std::string deviceLocation = GetDeviceLocation(deviceId);
  {
         ...
         ++result)
    {
      if (result->m_strLocation == deviceLocation)
      {
        CLog::Log(LOGINFO, "CPeripheralBusAndroid: input device \"%s\" with ID %d removed",
                  result->m_strDeviceName.c_str(), deviceId);
  ...
  if (removed)
  {
    ...
    CLog::Log(LOGWARNING,
              "CPeripheralBusAndroid: failed to remove input device with ID %d because it couldn't "
              "be found",
              deviceId);
}
...
bool CPeripheralBusAndroid::OnInputDeviceEvent(const AInputEvent* event)
{
  ...
  {
    CLog::Log(LOGWARNING,
              "CPeripheralBusAndroid: ignoring input event for unknown input device with ID %d",
              deviceId);
    return false;
  }
...
void CPeripheralBusAndroid::LogInputDevice(const CJNIViewInputDevice& device)
{
  ...
  for (int index = 0; index < motionRanges.size(); ++index)
  {
    ...
    CLog::Log(LOGDEBUG, "    Has axis: %s (%d)", CAndroidJoystickTranslator::TranslateAxis(axisId),
              axisId);
...
std::vector<std::pair<int, const char*>> CPeripheralBusAndroid::GetInputSources()
{
  ...
  sources.erase(
      std::remove_if(sources.begin(), sources.end(),
                     [](const std::pair<int, const char*>& source) { return source.first == 0; }),
      sources.end());
