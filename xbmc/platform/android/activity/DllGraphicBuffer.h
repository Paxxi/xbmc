...
public:
  ...
  virtual void GraphicBufferCtor(
      void*, uint32_t w, uint32_t h, uint32_t format, gfxImageUsage usage) = 0;
...
class DllGraphicBuffer : public DllDynamic, DllGraphicBufferInterface
{
  ...
  DEFINE_METHOD5(void,
                 GraphicBufferCtor,
                 (void* p1, uint32_t p2, uint32_t p3, uint32_t p4, gfxImageUsage p5))
  ...
  RESOLVE_METHOD_RENAME(_ZNK7android13GraphicBuffer15getNativeBufferEv,
                        GraphicBufferGetNativeBuffer)
  END_METHOD_RESOLVE()
};
