                         ...
                         unsigned int srcPitch /* 0 */)
{
  ...
  if (D3D11_USAGE_DEFAULT == usage &&
      DX::Windowing()->IsFormatSupport(format, D3D11_FORMAT_SUPPORT_RENDER_TARGET))
...
bool CD3DTexture::CreateInternal(const void* pixels /* nullptr */, unsigned int srcPitch /* 0 */)
{
  ...
  if (m_mipLevels == 0 &&
      DX::Windowing()->IsFormatSupport(m_format, D3D11_FORMAT_SUPPORT_MIP_AUTOGEN))
  {
  ...
  CD3D11_TEXTURE2D_DESC textureDesc(m_format, m_width, m_height, 1, m_mipLevels, m_bindFlags,
                                    m_usage, m_cpuFlags, 1, 0, miscFlags);
  ...
  HRESULT hr =
      pD3DDevice->CreateTexture2D(&textureDesc, (!autogenmm && pixels) ? &initData : nullptr,
                                  m_texture.ReleaseAndGetAddressOf());
  if (SUCCEEDED(hr) && autogenmm)
  {
    pD3D11Context->UpdateSubresource(
        m_texture.Get(), 0, nullptr, pixels,
    ...
    DXGI_FORMAT format /* = DXGI_FORMAT_UNKNOWN */)
{
  ...
  if (!m_views[format])
  {
    ...
    HRESULT hr = DX::DeviceResources::Get()->GetD3DDevice()->CreateShaderResourceView(
        m_texture.Get(), &cSRVDesc, view.GetAddressOf());
    if (SUCCEEDED(hr))
...
bool CD3DTexture::LockRect(UINT subresource, D3D11_MAPPED_SUBRESOURCE* res, D3D11_MAP mapType) const
{
  if (m_texture)
  {
    ...
    return (S_OK == DX::DeviceResources::Get()->GetImmediateContext()->Map(
                        m_texture.Get(), subresource, mapType, 0, res));
  }
  return false;
}
...
void CD3DTexture::SaveTexture()
{
  if (m_texture)
  {
    ...
    if (textureDesc.Usage != D3D11_USAGE_STAGING ||
        0 == (textureDesc.CPUAccessFlags & D3D11_CPU_ACCESS_READ))
    {
      ...
      if (FAILED(DX::DeviceResources::Get()->GetD3DDevice()->CreateTexture2D(&stagingDesc, NULL,
                                                                             &texture)))
        return;
...
ID3D11RenderTargetView* CD3DTexture::GetRenderTargetInternal(unsigned idx)
{
  ...
  if (!m_renderTargets[idx])
  {
    CD3D11_RENDER_TARGET_VIEW_DESC cRTVDesc(D3D11_RTV_DIMENSION_TEXTURE2DARRAY, DXGI_FORMAT_UNKNOWN,
                                            0, idx, 1);
    if (FAILED(DX::DeviceResources::Get()->GetD3DDevice()->CreateRenderTargetView(
            m_texture.Get(), &cRTVDesc, m_renderTargets[idx].ReleaseAndGetAddressOf())))
    {
      CLog::LogF(LOGWARNING, "cannot create texture view.");
    }
  }
                           ...
                           SHADER_METHOD options)
{
  ...
  Vertex verts[4] = {
      {XMFLOAT3(points[0].x, points[0].y, 0), xcolor, XMFLOAT2(coords.x1, coords.y1),
       XMFLOAT2(0.0f, 0.0f)},
      {XMFLOAT3(points[1].x, points[1].y, 0), xcolor, XMFLOAT2(coords.x2, coords.y1),
       XMFLOAT2(0.0f, 0.0f)},
      {XMFLOAT3(points[2].x, points[2].y, 0), xcolor, XMFLOAT2(coords.x2, coords.y2),
       XMFLOAT2(0.0f, 0.0f)},
      {XMFLOAT3(points[3].x, points[3].y, 0), xcolor, XMFLOAT2(coords.x1, coords.y2),
       XMFLOAT2(0.0f, 0.0f)},
  };
...
bool CD3DEffect::SetTexture(LPCSTR handle, CD3DTexture& texture)
{
  if (m_effect)
  {
    ID3DX11EffectShaderResourceVariable* var =
        m_effect->GetVariableByName(handle)->AsShaderResource();
    if (var->IsValid())
...
bool CD3DEffect::CreateEffect()
{
  ...
  hr = D3DX11CompileEffectFromMemory(
      m_effectString.c_str(), m_effectString.length(), "", &definemacros[0], this, dwShaderFlags, 0,
      DX::DeviceResources::Get()->GetD3DDevice(), m_effect.ReleaseAndGetAddressOf(), &pError);
  ...
  else if (pError)
  {
    ...
    CLog::Log(LOGERROR,
              "CD3DEffect::CreateEffect(): call to D3DXCreateEffect() failed with %" PRId32, hr);
  return false;
}
...
bool CD3DBuffer::Map(void** data)
{
  if (m_buffer)
  {
    ...
    if (SUCCEEDED(DX::DeviceResources::Get()->GetD3DContext()->Map(
            m_buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &resource)))
    {
      *data = resource.pData;
      return true;
    }
...
bool CD3DBuffer::CreateBuffer(const void* pData)
{
  ...
  return (S_OK == DX::DeviceResources::Get()->GetD3DDevice()->CreateBuffer(
                      &bDesc, (pData ? &initData : nullptr), m_buffer.ReleaseAndGetAddressOf()));
}
                              ...
                              unsigned int vertexLayoutSize)
{
  ...
  if (FAILED(D3DStripShader(code, codeLength, D3DCOMPILER_STRIP_REFLECTION_DATA,
                            m_VSBuffer.ReleaseAndGetAddressOf())))
  {
    CLog::LogF(LOGERROR, "failed to load the vertex shader.");
    return false;
  }
...
bool CD3DVertexShader::CreateInternal()
{
  ...
  if (FAILED(pDevice->CreateVertexShader(m_VSBuffer->GetBufferPointer(),
                                         m_VSBuffer->GetBufferSize(), nullptr,
  ...
  if (FAILED(pDevice->CreateInputLayout(m_vertexLayout, m_vertexLayoutSize,
                                        m_VSBuffer->GetBufferPointer(), m_VSBuffer->GetBufferSize(),
...
bool CD3DPixelShader::Create(const void* code, size_t codeLength)
{
  ...
  // trick to load bytecode into ID3DBlob
  if (FAILED(D3DStripShader(code, codeLength, D3DCOMPILER_STRIP_REFLECTION_DATA,
                            m_PSBuffer.ReleaseAndGetAddressOf())))
  {
    CLog::LogF(LOGERROR, "Failed to load the vertex shader.");
    return false;
  }
...
bool CD3DPixelShader::CreateInternal()
{
  ...
  // Create the vertex shader
  if (FAILED(pDevice->CreatePixelShader(m_PSBuffer->GetBufferPointer(), m_PSBuffer->GetBufferSize(),
