...
public:
  ...
  CGUIControlProfilerItem(CGUIControlProfiler* pProfiler,
...
#define GUIPROFILER_VISIBILITY_BEGIN(x) \
  { \
    if (CGUIControlProfiler::IsRunning()) \
      CGUIControlProfiler::Instance().BeginVisibility(x); \
  }
#define GUIPROFILER_VISIBILITY_END(x) \
  { \
    if (CGUIControlProfiler::IsRunning()) \
      CGUIControlProfiler::Instance().EndVisibility(x); \
  }
#define GUIPROFILER_RENDER_BEGIN(x) \
  { \
    if (CGUIControlProfiler::IsRunning()) \
      CGUIControlProfiler::Instance().BeginRender(x); \
  }
#define GUIPROFILER_RENDER_END(x) \
  { \
    if (CGUIControlProfiler::IsRunning()) \
      CGUIControlProfiler::Instance().EndRender(x); \
  }
