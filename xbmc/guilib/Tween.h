...
protected:
  ...
  float easeOut(float time, float start, float change, float duration) const
  {
    ...
    return (a * pow(2.0f, -10 * time) * sin((time * duration - s) * (2 * M_PI) / p)) + change +
           start;
  }
  ...
  float easeInOut(float time, float start, float change, float duration) const
  {
    ...
    {
      time--;
      return -.5f * (a * pow(2.0f, 10 * (time)) * sin((time * duration - s) * (2 * M_PI) / p)) +
             start;
    }
    time--;
    return a * pow(2.0f, -10 * (time)) * sin((time * duration - s) * (2 * M_PI) / p) * .5f +
           change + start;
  }
};
