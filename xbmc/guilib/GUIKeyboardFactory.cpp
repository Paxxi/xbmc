...
void CGUIKeyboardFactory::keyTypedCB(CGUIKeyboard* ref, const std::string& typedString)
{
  if (ref)
  {
    ...
    case FILTERING_SEARCH:
      ...
      CApplicationMessenger::GetInstance().SendGUIMessage(
...
#if defined(TARGET_DARWIN_EMBEDDED)
  kb = CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogKeyboardTouch>(
      WINDOW_DIALOG_KEYBOARD_TOUCH);
#else
  kb = CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogKeyboardGeneric>(
                                               ...
                                               unsigned int autoCloseMs /* = 0 */)
{
  ...
  if (1 > iRetries && strHeading.size())
    ...
    strHeadingTemp =
        StringUtils::Format("%s - %i %s", g_localizeStrings.Get(12326).c_str(),
