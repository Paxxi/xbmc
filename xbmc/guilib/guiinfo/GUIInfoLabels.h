...
#define SYSTEM_ALWAYS_TRUE \
  125 // useful for <visible fade="10" start="hidden">true</visible>, to fade in a control
#define SYSTEM_ALWAYS_FALSE \
  126 // used for <visible fade="10">false</visible>, to fade out a control (ie not particularly useful!)
...
#define SLIDESHOW_EXIF_DATE \
  (SLIDESHOW_LABELS_START + 19) /* Implementation only to just get localized date */
...
#define LISTITEM_PICTURE_COPYRIGHT_NOTICE \
  (LISTITEM_START + 54) // => SLIDESHOW_IPTC_COPYRIGHT_NOTICE
...
#define INFOFLAG_LISTITEM_POSITION \
  (static_cast<uint32_t>(1 << 26)) // ListItem lookups based on cursor position
...
#define INFOFLAG_LISTITEM_CONTAINER \
  (static_cast<uint32_t>(1 << 29)) // Lookup the item in given container
