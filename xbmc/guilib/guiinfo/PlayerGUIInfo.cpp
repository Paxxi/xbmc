...
float CPlayerGUIInfo::GetSeekPercent() const
{
  ...
  float fPercent = fPercentPlayTime +
                   fPercentPerSecond * g_application.GetAppPlayer().GetSeekHandler().GetSeekSize();
...
std::string CPlayerGUIInfo::GetCurrentSeekTime(TIME_FORMAT format) const
{
  ...
  return StringUtils::SecondsToTimeString(
      g_application.GetTime() + g_application.GetAppPlayer().GetSeekHandler().GetSeekSize(),
      format);
}
...
bool CPlayerGUIInfo::InitCurrentItem(CFileItem* item)
{
  ...
  {
    CLog::Log(LOGDEBUG, "CPlayerGUIInfo::InitCurrentItem(%s)",
                              ...
                              std::string* fallback) const
{
  ...
  case PLAYER_SEEKOFFSET:
  {
    std::string seekOffset = StringUtils::SecondsToTimeString(
        std::abs(m_seekOffset / 1000), static_cast<TIME_FORMAT>(info.GetData1()));
    if (m_seekOffset < 0)
      value = "-" + seekOffset;
  ...
  case PLAYER_TIME_SPEED:
  {
    ...
    if (speed != 1.0)
      value = StringUtils::Format(
          "%s (%ix)", GetCurrentPlayTime(static_cast<TIME_FORMAT>(info.GetData1())).c_str(),
  ...
  case PLAYER_SEEKSTEPSIZE:
  {
    ...
    std::string strSeekSize =
        StringUtils::SecondsToTimeString(abs(seekSize), static_cast<TIME_FORMAT>(info.GetData1()));
    if (seekSize < 0)
      value = "-" + strSeekSize;
    if (seekSize > 0)
      value = "+" + strSeekSize;
...
std::vector<std::pair<float, float>> CPlayerGUIInfo::GetCutList(CDataCacheCore& data,
                                                                time_t duration) const
{
  ...
  {
    if (cut.action != EDL::Action::CUT && cut.action != EDL::Action::COMM_BREAK)
      continue;
...
std::vector<std::pair<float, float>> CPlayerGUIInfo::GetChapters(CDataCacheCore& data,
                                                                 time_t duration) const
{
  std::vector<std::pair<float, float>> ranges;
