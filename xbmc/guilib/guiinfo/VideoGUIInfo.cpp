...
int CVideoGUIInfo::GetPercentPlayed(const CVideoInfoTag* tag) const
{
  ...
  if (bookmark.IsPartWay())
    return std::lrintf(static_cast<float>(bookmark.timeInSeconds) / bookmark.totalTimeInSeconds *
                       100.0f);
  else
    return 0;
}
...
bool CVideoGUIInfo::InitCurrentItem(CFileItem* item)
{
  if (item && item->IsVideo())
  {
    ...
    CLog::Log(LOGDEBUG, "CVideoGUIInfo::InitCurrentItem(%s)",
                             ...
                             std::string* fallback) const
{
  ...
  if (tag)
  {
    ...
    case LISTITEM_DURATION:
    {
      ...
      {
        value =
            StringUtils::SecondsToTimeString(iDuration, static_cast<TIME_FORMAT>(info.GetData4()));
        return true;
      }
      break;
    }
