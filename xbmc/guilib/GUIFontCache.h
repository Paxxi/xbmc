...
struct CGUIFontCacheKeysMatch
{
  ...
  {
    return a.m_text == b.m_text && a.m_colors == b.m_colors && a.m_alignment == b.m_alignment &&
           a.m_scrolling == b.m_scrolling && a.m_maxPixelWidth == b.m_maxPixelWidth &&
           Match(a.m_pos, a.m_matrix, b.m_pos, b.m_matrix, a.m_scrolling) &&
           a.m_scaleX == b.m_scaleX && a.m_scaleY == b.m_scaleY;
  }
};
...
struct CGUIFontCacheStaticPosition
{
  float m_x;
  float m_y;
  CGUIFontCacheStaticPosition(float x, float y)
    : m_x(x)
    , m_y(y)
  {
  }
...
struct CGUIFontCacheDynamicPosition
{
  ...
  CGUIFontCacheDynamicPosition(float x, float y, float z)
    : m_x(x)
    , m_y(y)
    , m_z(z)
  {
                  ...
                  bool scrolling)
{
  ...
  return (scrolling || diffX - floorf(diffX) < 2 * FONT_CACHE_DIST_LIMIT) &&
         ...
         diffZ - floorf(diffZ) < 2 * FONT_CACHE_DIST_LIMIT && a_m.m[0][0] == b_m.m[0][0] &&
         a_m.m[1][1] == b_m.m[1][1] && a_m.m[2][2] == b_m.m[2][2];
